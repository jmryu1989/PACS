...
- 클릭 좌표는 고정 OHIF와 같은 방식(표시 요소의 `getBoundingClientRect()` 기준)으로 계산한다. 영역 안의 자식 요소를 눌러도 같은 점을 가리키며, 표시 영역을 확인할 수 없으면 계산하지 않고 거절한다.
...
- 고정 렌더러는 이미 시작한 요청을 취소하지 못한다. 그래서 모든 대기에 상한을 두고, 상한 안에 끝나지 않은 요청이 있는 화면은 **보류**로 표시해 그 화면에 새 요청(되돌리기 포함)을 보내지 않는다. 종료·취소도 상한 안에서 끝나며 확인하지 못한 되돌리기를 성공으로 적지 않는다.
