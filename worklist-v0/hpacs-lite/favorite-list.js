...
window.KinFavoriteList = function (app) {
  ...
  function accept(value){
    ...
    if(!Number.isInteger(value.revision)||!Array.isArray(value.folders))throw new Error('즐겨찾기 목록 형식 오류');
    const folder=value.folders.find(f=>f.id===scope.id);
    if(folder&&(typeof folder.name!=='string'||!Array.isArray(folder.uids)||folder.uids.some(uid=>typeof uid!=='string')))throw new Error('즐겨찾기 폴더 형식 오류');
    ...
    scope={...scope,revision:value.revision,name:folder?.name??scope.name,uids:new Set(folder?.uids??[]),
      status:folder?'':'폴더가 삭제되었습니다. 범위를 해제하거나 다른 폴더를 선택하세요.'};changed();
  }
  return {
    apply(value,id){
      if(!app.identity()||JSON.stringify(value?.owner)!==identity())throw new Error('즐겨찾기 계정이 바뀌었습니다');
      if(!value.folders?.some(f=>f.id===id))throw new Error('폴더를 찾을 수 없습니다');
    ...
    label(){return scope?(owns()?'즐겨찾기: '+scope.name+(scope.status?' · '+scope.status:' · 기존 검색과 함께 적용'):'즐겨찾기: 계정 확인 필요'):'';},
    async refresh(){
      if(!scope)return;
      ...
      try{const value=await app.api('GET','/favorite-folders',undefined,local.signal);if(ticket===sequence)accept(value);}
