...
(function(root){
  'use strict';
  const defaults=()=>({version:1,newStudies:false,emergency:false,volume:0.3});
  function normalize(value){
    if(!value||typeof value!=='object'||Array.isArray(value)||Object.keys(value).sort().join(',')!=='emergency,newStudies,version,volume'||value.version!==1||typeof value.newStudies!=='boolean'||typeof value.emergency!=='boolean'||![0.1,0.3,0.6].includes(value.volume))return null;
    return {...value};
  }
  function observer(){
    let previous=null;
  ...
  function mount({button,owner,available}){
    const bound=owner(),key=bound&&'kin-worklist-alerts:v1:'+bound,model=observer();
    let ended=false,settings=defaults(),audio=null,armed=false,busy=false,channel,storage,feedback='',last='Waiting for List',generation=0;
    ...
    function render(){
      ...
      dialog.querySelector('#alerts-list-state').textContent=last;dialog.querySelector('#alerts-feedback').textContent=feedback;
    }
    ...
    async function sound(testing){
      ...
      try{
        if(!testing&&armed){armed=false;stopTones();await audio.suspend();}
          ...
          armed=true;if(testing&&tone(false))feedback='시험음 재생을 요청했습니다. 실제 스피커와 음량을 확인하세요.';
        }
    ...
    fresh.onchange=urgent.onchange=volume.onchange=()=>{
      if(!live()){render();return;}
      settings={version:1,newStudies:fresh.checked,emergency:urgent.checked,volume:Number(volume.value)};
    ...
    function end(){if(ended)return;ended=true;generation++;armed=false;busy=false;model.reset();stopTones();if(audio){audio.onstatechange=null;audio.close().catch(()=>{});}channel?.close();if(dialog.open)dialog.close();render();}
    ...
    return {observe(rows){
      if(!live())return;
      try{
        const result=model.observe(rows);last=result.baseline?'List Baseline Ready':'New to List '+result.newStudies+' · Emergency Changes '+result.emergency;
        if(!result.baseline&&!document.hidden&&(settings.newStudies&&result.newStudies>0||settings.emergency&&result.emergency>0)){
          tone(settings.emergency&&result.emergency>0);
        }
      }catch(e){model.reset();last='Alerts Unverified';feedback=e.message;}
      render();
    },end};
  }
  const api={defaults,normalize,observer,mount};
