...
globalThis.kinViewerJobPrint = function ({ api, authenticate, live, editor }) {
  ...
  async function prepare() {
    ...
    try { await bounded(async signal => {
      ...
      // The MPR renderer assumes a fixed, vacancy-free cell list, so a version 7 plane
      // layout is named and refused here instead of reaching either renderer.
  ...
  function open(item) {
    ...
    heading.textContent = item.version===7?'MPR Plane Layout · 출력 미지원':[4,5,6].includes(item.version)?(item.snapshot?'Current MPR Output':'Saved MPR Output'):item.snapshot ? '현재 비교 화면 출력 · 저장 안 함' : '저장한 비교 영상 출력';
