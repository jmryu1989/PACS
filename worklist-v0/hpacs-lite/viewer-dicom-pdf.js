...
(function(root){
  ...
  function create(services,options={}){
    ...
    let ended=false,mounted=false,hostTimer=null,channel=null,boundOwner=null,ownerError=null,source=null,generation=0,request=null,sourceRequest=null,ownerRequest=null,pendingWindow=null,nativeRetryNeeded=false,nativeRetryPending=false;
    const subscriptions=[];
    ...
    async function resolveSource(value,ticket){
      ...
      const work=(async()=>{
        ...
        if(!sourceLive())return;source={...value,url:url.href,orthancId:lookup.id,owner:first};button.textContent='Open Source PDF';button.disabled=false;status.textContent='Ready · 브라우저 PDF 도구에서 페이지 이동·검색·인쇄를 사용할 수 있습니다.';
      })();
    ...
    async function open(){
      if(ended||request||!boundOwner)return;if(nativeRetryNeeded){if(nativeRetryPending)return;nativeRetryPending=true;options.onRetry?.();button.textContent='Retry Source PDF';button.disabled=true;status.textContent='Checking native PDF…';return;}if(!source){options.onRetry?.();button.disabled=true;button.textContent='Retry Source PDF';refresh();return;}
    ...
    function nativeFailure(error){if(ended)return;nativeRetryNeeded=true;nativeRetryPending=false;button.textContent='Retry Source PDF';button.disabled=!!request||!boundOwner;status.textContent=error?.message||'원본 PDF 표시를 완료하지 못했습니다. 다시 시도하세요.';}
    function nativeReady(){if(ended||!nativeRetryNeeded)return;nativeRetryNeeded=false;nativeRetryPending=false;button.textContent='Open Source PDF';const ready=!!source&&sameOwner(boundOwner,source.owner);button.disabled=!ready;status.textContent=ready?'Ready · 브라우저 PDF 도구에서 페이지 이동·검색·인쇄를 사용할 수 있습니다.':'Checking source path…';}
