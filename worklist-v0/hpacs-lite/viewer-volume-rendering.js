window.kinCreateVolumeRendering=function({target,permitted,alive,owner,notice=()=>{},storage}){
  ...
  const presetControls=el('fieldset',undefined,dialog);el('legend','Personal Display Presets',presetControls);const presetHelp=el('p','',presetControls),presetHelpText='현재 기관과 계정에 한정해 이 브라우저에만 저장됩니다. 편집값은 Apply Display로 적용한 뒤 저장하세요.';presetHelp.textContent=presetHelpText;
  ...
  let ended=false,operation=null,drag=null,knots=[],librarySnapshot=null,libraryStale=false,libraryFailure=null;const groups=[controls,cropControls,transferControls,presetControls],writeButtons=[savePreset,replacePreset,deletePreset];
  ...
  return {open,dispose(){ended=true;clearInterval(timer);window.removeEventListener('storage',storageChanged);observer.disconnect();close();dialog.remove();}};
};
