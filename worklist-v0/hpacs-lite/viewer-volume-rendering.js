window.kinCreateVolumeRendering=function({target,permitted,alive,owner,notice=()=>{}}){
  ...
  const cropControls=el('fieldset',undefined,dialog);el('legend','Crop Bounds',cropControls);el('p','I/J/K 원본 복셀 번호를 양 끝 포함 범위로 입력하세요.',cropControls);const cropInputs={};
  ...
  const transferControls=el('fieldset',undefined,dialog);el('legend','Transfer Function',transferControls);el('p','Custom에서는 HU, 색상, 불투명도 점을 HU가 작은 순서로 입력하세요.',transferControls);const transferMode=select('Transfer Mode',['Preset','Custom'],transferControls),knotHost=el('div',undefined,transferControls);knotHost.className='kin-vr-knots';
  const addKnot=el('button','Add Knot',transferControls),removeKnot=el('button','Remove Knot',transferControls),closeButton=el('button','Close VR',dialog);
  ...
  let ended=false,operation=null,drag=null,knots=[];const groups=[controls,cropControls,transferControls];
  const disableControls=value=>groups.forEach(group=>group.disabled=value);
  ...
  function applyDisplay(rethrow=false){const op=operation;if(!op?.view)return;let changed=false;try{
    ...
    const custom=transferMode.value==='Custom'?KinVolumeRendering.validateTransferKnots(knots):null,property=op.view.getActors()[0].actor.getProperty();changed=true;
    ...
    else{op.view.setProperties({preset:preset.value});const curve=property.getScalarOpacity(0);for(let i=0;i<curve.getSize();i++){const node=[];curve.getNodeValue(i,node);node[1]*=scale/100;curve.setNodeValue(i,node);}}
    property.setShade(shade.checked);render(op);status.textContent='VR 표시 조건을 적용했습니다.';
  }catch(error){if(rethrow===true)throw error;if(changed)fail(op,error);else status.textContent=error.message;}}
  // The pinned vtkAbstractMapper only calls isA while adding, then getOrigin/getNormal
  // from getClippingPlaneInDataCoords; keeping this local avoids an unavailable vtk global.
  function planeObject(definition){const origin=[...definition.origin],normal=[...definition.normal];return {isA:name=>name==='vtkPlane',getOrigin:()=>origin,getNormal:()=>normal};}
  ...
  function applyCrop(rethrow=false,clear=false){const op=operation;if(!op?.view)return;let changed=false;try{
    ...
    const definitions=full||clear?[]:KinVolumeRendering.cropPlanes(bounds,op.dimensions,op.imageData),next=definitions.map(planeObject),previous=[...mapper.getClippingPlanes()];changed=true;
  ...
  apply.onclick=()=>applyDisplay();applyCropButton.onclick=()=>applyCrop();reset.onclick=()=>{const op=operation;if(!op?.view)return;try{check(op);op.view.setCamera(structuredClone(op.base));resetEditors(op);applyDisplay(true);if(typeof op.mapper.getClippingPlanes==='function'&&typeof op.mapper.addClippingPlane==='function'&&typeof op.mapper.removeAllClippingPlanes==='function')applyCrop(true,true);status.textContent='처음 VR 표시로 돌아왔습니다.';}catch(e){fail(op,e);}};
