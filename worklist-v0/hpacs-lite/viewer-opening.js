...
(function (root) {
  ...
  const defaults = () => ({ version: 3, listTarget: 'window', includePrior: true, maxWindows: 1, autoLoad: false, reuseClean: false });
  function normalize(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value) ||
        Object.keys(value).sort().join(',') !== (value.version === 1 ? 'includePrior,listTarget,version' : value.version === 2 ? 'includePrior,listTarget,maxWindows,version' : 'autoLoad,includePrior,listTarget,maxWindows,reuseClean,version') ||
        ![1, 2, 3].includes(value.version) || !['window', 'workspace'].includes(value.listTarget) ||
    ...
    const autoLoad = value.version < 3 ? false : value.autoLoad;
    const reuseClean = value.version < 3 ? false : value.reuseClean;
    if (typeof autoLoad !== 'boolean' || typeof reuseClean !== 'boolean') return null;
    return { version: 3, listTarget: value.listTarget, includePrior: value.includePrior, maxWindows, autoLoad, reuseClean };
  }
  ...
  function mount({ button, session }) {
    ...
    dialog.innerHTML = '<h2 id="image-opening-title">Image Opening</h2>' +
      ...
      '<p>이 목록에서 관리하는 별도 창 수입니다. 2개 이상이면 다른 검사를 새 창에 유지합니다. 제한을 줄여도 열린 창을 자동으로 닫지 않습니다.</p>' +
      '<label><input id="image-opening-reuse" type="checkbox"> Reuse Clean Window at Limit</label>' +
      '<p>2개 이상 제한이 가득 찼을 때 저장 중도, 미저장 작업도, 열린 대화상자도 없는 창 하나를 다시 사용합니다. 제한 1에서는 적용되지 않습니다. 끄면 기존 창을 직접 닫습니다. 영상 전체를 열람했다는 뜻은 아닙니다.</p>' +
      '<label><input id="image-opening-autoload" type="checkbox"> Open on Selection</label>' +
      '<p>목록 행이나 이전/다음 검사로 새 검사를 선택할 때 영상을 엽니다. 이미 열린 창의 미저장 작업·대화상자와 창 수 제한을 유지합니다.</p>' +
    ...
    const target = dialog.querySelector('#image-opening-target'), prior = dialog.querySelector('#image-opening-prior'), limit = dialog.querySelector('#image-opening-limit'), automatic = dialog.querySelector('#image-opening-autoload');
    const reuse = dialog.querySelector('#image-opening-reuse');
    ...
    function render(note) {
      ...
      automatic.checked = value.autoLoad;
      reuse.checked = value.reuseClean; reuse.disabled = !current() || value.maxWindows === 1;
      button.disabled = target.disabled = prior.disabled = limit.disabled = automatic.disabled = reset.disabled = !current();
    ...
    target.onchange = prior.onchange = limit.onchange = automatic.onchange = reuse.onchange = () => save({ version: 3, listTarget: target.value, includePrior: prior.checked, maxWindows: Number(limit.value), autoLoad: automatic.checked, reuseClean: reuse.checked });
