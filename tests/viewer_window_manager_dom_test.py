...
const KinViewerDisplayLayout = {screens:()=>[],identity:()=>''};
const initMonitorPermission = async()=>{}, cacheMonitorScreens=()=>{}, popupRect=()=>null;
const monitorPermissionGranted = ()=>false, rememberOhifRect=()=>true, watchOhifRect=()=>{};
...
window.__makePopup = ({href='https://example.test/ohif/viewer?StudyInstanceUIDs=1.2.1', pending=null,
  kind='viewer',ready=true,busy=false,dirty=false,owner=sessionOwner,coverage=true}={}) => {
  ...
  viewerWindows.attach(choice,popup);ohifPopupSlots.set(popup,choice.index);ohifPopupSequences.set(popup,1);
