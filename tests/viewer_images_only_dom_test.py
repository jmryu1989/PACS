...
class ViewerImagesOnlyDOMTest(unittest.TestCase):
    ...
    def test_frame_navigation_is_allowed_but_source_list_change_exits(self):
        ...
        self.page.wait_for_function("()=>document.fullscreenElement===null", timeout=2000)
    ...
    def test_exit_rejection_keeps_visible_retry_and_owned_fullscreen(self):
        ...
        expect(self.page.locator("#kin-images-only-exit")).to_have_attribute("data-position", "left-middle")
        ...
        expect(self.page.locator("#kin-images-only-exit")).to_be_hidden()
        expect(self.page.locator(".kin-viewer-identity")).to_be_visible()
        expect(self.page.locator("#kin-images-only-status")).to_contain_text("Esc")
        ...
        expect(self.page.locator("#kin-images-only-status")).to_have_text("Images Only를 종료했습니다.")

    def test_exit_retry_is_immediately_clickable_inside_the_temporary_shield(self):
        self.page.locator("#kin-images-only-enter").click()
        self.page.evaluate("exitMode='reject'")
        self.page.locator("#kin-images-only-exit").click()
        expect(self.page.locator("#kin-images-only-exit")).to_contain_text("Retry")
    ...
    def test_zero_size_fullscreen_target_hides_exit_until_geometry_recovers(self):
        self.page.locator("#kin-images-only-enter").click()
        self.page.evaluate("exitMode='reject';viewportElement.style.width='0';viewportElement.style.height='0';document.dispatchEvent(new Event('rendered-image'))")
        expect(self.page.locator("#kin-images-only-exit")).to_be_hidden()
        expect(self.page.locator("#kin-images-only-status")).to_contain_text("Esc")
        self.page.evaluate("viewportElement.style.width='800px';viewportElement.style.height='600px';document.dispatchEvent(new Event('rendered-image'))")
        expect(self.page.locator("#kin-images-only-exit")).to_be_visible()
    ...
    def test_cancelled_pending_rejection_releases_owner_and_old_exit_callback_is_stale(self):
        ...
        self.page.evaluate("viewportElement.querySelector('#pixels').dispatchEvent(new MouseEvent('dblclick',{bubbles:true,cancelable:true}))")
        self.assertEqual(1, self.page.evaluate("oneUp"), "an old shield must not block a newer ownership record")
