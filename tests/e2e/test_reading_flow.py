...
class ReadingFlowE2E(ReadingWorkspaceE2E):
 ...
 def test_flow_02_keyboard_roundtrip_retains_report_and_viewer(self):
  ...
  p.get_by_role('button',name='판독문 작성',exact=True).click()
  ...
  expect(p.get_by_role('button',name='검사 목록',exact=True)).to_be_focused()
 ...
 def test_flow_03_queue_edges_filter_and_modal_guard(self):
  ...
  expect(p.get_by_role('button',name='이전 검사',exact=True)).to_be_disabled()
  expect(p.get_by_role('button',name='영상으로',exact=True)).to_be_enabled(timeout=60000)
  ...
  expect(p.get_by_role('button',name='다음 검사',exact=True)).to_be_disabled()
  ...
  p.get_by_role('button',name='검사 정보·상용구',exact=True).click()
  p.get_by_role('button',name='목록 화면으로',exact=True).click();p.locator('#m-reading').click()
  expect(p.get_by_role('button',name='검사 정보·상용구',exact=True)).to_have_attribute('aria-expanded','false')
