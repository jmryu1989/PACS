...
class ReadingWorkspaceE2E(ViewerJobReportE2E):
 ...
 def choose(self,p,f):
  if not p.locator('.left').is_visible():p.get_by_role('button',name='검사 목록',exact=True).click()
  self.select(p,f)
 ...
 def test_workspace_01_real_comparison_report_and_separate_window(self):
  ...
  expect(p.locator('#findings')).to_have_value('CURRENT '+a.uid)
  p.get_by_role('button',name='판독문 작성',exact=True).click();expect(p.locator('#findings')).to_be_focused()
 ...
 def test_workspace_02_draft_navigation_and_mode_resume(self):
  ...
  p.get_by_role('button',name='목록 화면으로',exact=True).click();expect(p.locator('#reading-viewer')).not_to_be_visible()
  ...
  p.get_by_role('button',name='다음 검사' if direction==1 else '이전 검사',exact=True).click()
 ...
 def test_workspace_03_unsaved_viewer_hidden_and_restored(self):
  ...
  expect(f.get_by_label('작업 제목',exact=True)).to_have_value('KEEP UNSAVED VIEWER')
  ...
  p.get_by_role('button',name='미저장 영상 작업 버리고 열기',exact=True).click()
  expect(f.get_by_label('작업 제목',exact=True)).to_have_value('KEEP UNSAVED VIEWER')
 ...
 def test_workspace_04_failed_document_retry_and_session_end(self):
  ...
  p.get_by_role('button',name='영상 다시 열기',exact=True).click()
  expect(p.locator('#reading-status')).to_have_text('영상 작업공간 연결됨',timeout=60000)
 ...
 def test_workspace_05_narrow_layout_and_context_controls(self):
  ...
  p.get_by_role('button',name='검사 정보·상용구',exact=True).click();expect(p.locator('.rw')).to_be_visible()
  p.get_by_role('button',name='검사 정보·상용구',exact=True).click();expect(p.locator('.rw')).not_to_be_visible()
  p.get_by_role('button',name='판독문 작성',exact=True).click();expect(p.locator('#findings')).to_be_focused()
  expect(p.locator('#findings')).to_have_value('CURRENT '+a.uid)
  self.shot(p,'narrow')
 ...
 def test_workspace_06_restore_changes_comparison_identity(self):
  ...
  expect(f.locator('#kin-viewer-jobs-status')).to_contain_text('복원했습니다',timeout=45000)
  p.get_by_role('button',name='목록 화면으로',exact=True).click();p.locator('#m-reading').click()
