...
class ViewerLayoutE2E(ThumbnailSeriesE2E):
    ...
    def action(self,page,name,message):
        button=page.get_by_role('button',name='최근 배치 '+name,exact=True)
    ...
    def test_layout_01_native_placement_and_recent_save(self):
        ...
        expect(page.get_by_role('button',name='최근 배치 저장',exact=True)).to_be_visible()
        page.get_by_role('button',name='최근 배치 저장',exact=True).click()
        ...
        expect(page.get_by_role('button',name='최근 배치 복원',exact=True)).to_be_in_viewport()
        box=page.locator('#kin-viewer-layout').bounding_box();history=page.locator('#kin-viewer-history').bounding_box()
        self.assertLessEqual(box['x']+box['width'],history['x'])
    ...
    def test_layout_03_owner_device_and_delayed_permission(self):
        ...
        stale=self.launch(page.context.new_page(),[f])
        self.relog(page,'doctor2');self.launch(page,[f]);self.action(page,'복원','없습니다')
        stale.get_by_role('button',name='최근 배치 저장',exact=True).click()
        ...
        page.get_by_role('button',name='최근 배치 복원',exact=True).click()
        expect(page.get_by_role('button',name='최근 배치 복원',exact=True)).to_be_disabled()
        ...
        before=self.cells(page);page.get_by_role('button',name='최근 배치 복원',exact=True).click()
        expect(page.locator('#kin-viewer-layout-status')).to_contain_text('세션이 변경')
        expect(page.get_by_role('button',name='최근 배치 저장',exact=True)).to_be_disabled()
