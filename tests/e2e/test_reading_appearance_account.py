...
class AppearanceAccountE2E(ReadingAppearanceE2E):
 ...
 def test_roam_01_explicit_two_browser_restore_preserves_work(self):
  ...
  expect(other.locator('#appearance-account-status')).to_have_text('표시 설정을 계정에 저장했습니다.')
 ...
 def test_roam_05_legacy_load_retains_local_font_color_then_upgrades(self):
  ...
  p.locator('#appearance-account-save').click();expect(p.locator('#appearance-account-status')).to_have_text('표시 설정을 계정에 저장했습니다.')
  saved=p.request.get(self.stack.api+'/reading-appearance').json()['sizes'];self.assertEqual(saved['version'],3);self.assertEqual(saved['fonts']['current'],'serif');self.assertEqual(saved['colors']['current'],'cool')
 ...
 def test_roam_04_failed_write_empty_load_and_wrong_owner_response(self):
  ...
  expect(p.locator('#appearance-account-status')).to_have_text('계정에 저장된 표시 설정이 없습니다.')
