...
class HeldMeasurementE2E(MeasurementReadbackE2E):
    def held(self, p, f, label='보관 <img src=x onerror=window.bad=1> 수정'):
        ...
        row.get_by_label('주석 문구').fill(label)
    ...
    def test_01_discard_hidden_held_edit_without_server_write(self):
        ...
        row.get_by_text('보관 중인 수정 보기',exact=True).click()
        ...
        discard=row.get_by_role('button',name='보관 수정 버리기',exact=True)
        self.guard(p,True)
        ...
        expect(row).not_to_contain_text('보관 중인 수정 보기')
        self.guard(p,False)
        self.assertEqual(p.evaluate("()=>cornerstoneTools.annotation.state.getAllAnnotations().filter(a=>a.metadata.toolName==='Length').length"),0)
        self.assertEqual(writes,[]);self.assertEqual(self.durable(f),saved)
        p.get_by_role('button',name='새로고침',exact=True).click()
        expect(row).to_contain_text('숨김');expect(row).not_to_contain_text('보관 중인 수정 보기')
        self.guard(p,False)
    ...
    def test_02_recheck_on_another_frame_recovers_held_content(self):
        ...
        expect(row.get_by_role('button',name='원본 다시 확인',exact=True)).to_be_enabled()
        self.guard(p,True)
        ...
        row.get_by_role('button',name='원본 다시 확인',exact=True).click()
        expect(row.get_by_label('주석 문구')).to_have_value('원본 확인 뒤 이어 쓸 수정')
        expect(row).to_contain_text('영상으로 이동해')
        row.get_by_role('button',name='영상으로 이동',exact=True).click()
    ...
    def test_03_pending_restore_blocks_old_discard_handler(self):
        ...
        p.evaluate('''()=>window.heldOldDiscard=[...document.querySelectorAll('#kin-viewer-history button')]
            .find(b=>b.textContent==='보관 수정 버리기')''')
        requests=[]; routes=[]
        def lost(route):
        ...
        with p.expect_request(lambda request:request.method=='POST' and request.url.endswith('/revisions')):
            row.get_by_role('button',name='복원',exact=True).click()
        expect(row.get_by_role('button',name='보관 수정 버리기',exact=True)).to_be_disabled()
        dialogs=[]
        ...
        with p.expect_response(lambda r:r.request.method=='POST' and r.url.endswith('/revisions')) as response:
            row.get_by_role('button',name='같은 요청 재시도',exact=True).click()
        ...
        self.assertEqual(row.get_by_role('button',name='보관 수정 버리기',exact=True).count(),0)
    ...
    def test_04_discard_unverified_copy_keeps_other_unsaved_item(self):
        ...
        other=self.draw_length(p);other.get_by_label('주석 문구').fill('다른 미저장 항목')
        saved=self.durable(f);writes=[]
        p.on('request',lambda request:writes.append(request.url) if request.method in ['POST','PATCH','DELETE'] else None)
        p.once('dialog',lambda dialog:dialog.accept())
        row.get_by_role('button',name='보관 수정 버리기',exact=True).click()
        expect(row).to_contain_text('재확인 필요');expect(row).not_to_contain_text('보관 중인 수정 보기')
        expect(other.get_by_label('주석 문구')).to_have_value('다른 미저장 항목')
    ...
    def test_05_repeated_restore_conflicts_keep_original_held_edit(self):
        ...
        expect(row).to_contain_text('저장됨 r4');self.guard(p,True)
        ...
        expect(p.locator('svg.svg-layer')).to_contain_text('mm')
        row.get_by_role('button',name='저장',exact=True).click()
        expect(row).to_contain_text('저장됨 r6')
    ...
    def test_06_discard_after_conflict_adopts_known_latest_head(self):
        ...
        row.get_by_role('button',name='보관 수정 버리기',exact=True).click()
        expect(row).to_contain_text('저장됨 r4')
        expect(row.get_by_role('button',name='최신판 기준으로 내 수정 유지')).to_have_count(0)
        expect(row).not_to_contain_text('보관 중인 수정 보기')
