...
class ReadingDockE2E(ReadingWorkspaceE2E):
 ...
 def test_dock_01_resize_switch_and_retained_job_save(self):
  ...
  expect(f.get_by_label('Job Title',exact=True)).to_have_value('DOCK RETAINED JOB')
  p.set_viewport_size(dict(width=1680,height=1100));self.bounds(f);canvas_ready(f,2)
  self.click_job(f,'Save New Job','저장했습니다')
 ...
 def test_dock_02_hidden_unsaved_work_still_blocks_navigation(self):
  ...
  job=f.get_by_role('button',name='Comparison',exact=True);job.click()
  f.get_by_label('Job Title',exact=True).fill('HIDDEN UNSAVED');job.click()
  ...
  job.click();expect(f.get_by_label('Job Title',exact=True)).to_have_value('HIDDEN UNSAVED')
