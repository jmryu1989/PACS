...
class ColumnsRoamingE2E(WorklistColumnsE2E):
    ...
    def test_roam_columns_01_two_browsers_explicit_apply_and_clear(self):
        ...
        self.column(page,'age').locator('input[type=checkbox]').uncheck()
        ...
        expect(self.column(second,'age').locator('input[type=checkbox]')).not_to_be_checked()
        expect(second.locator('#heads [data-key="age"]')).to_have_count(1)
        ...
        expect(self.column(other,'age').locator('input[type=checkbox]')).to_be_checked()
        ...
        self.assertIsNone(self.remote(second)['columns']);expect(self.column(second,'age').locator('input[type=checkbox]')).not_to_be_checked()
    ...
    def test_roam_columns_02_conflict_and_network_failure(self):
        ...
        self.column(peer,'age').locator('input[type=checkbox]').uncheck();self.action(peer,'save','편집값을 계정에 저장했습니다')
        ...
        expect(self.column(page,'desc').locator('input[type=checkbox]')).not_to_be_checked();expect(page.locator('#wc-server-save')).to_be_disabled()
        ...
        expect(self.column(page,'age').locator('input[type=checkbox]')).not_to_be_checked();expect(self.column(page,'desc').locator('input[type=checkbox]')).to_be_checked()
        page.route('**/api/worklist-columns',lambda r:r.abort())
        self.column(page,'sex').locator('input[type=checkbox]').uncheck();page.locator('#wc-server-save').click()
        expect(page.locator('#wc-server-status')).to_contain_text('응답을 확인할 수 없습니다')
        expect(self.column(page,'sex').locator('input[type=checkbox]')).not_to_be_checked()
    ...
    def test_roam_columns_03_delayed_read_edit_close_and_session(self):
        page=self.login();self.open_columns(page);self.action(page,'inspect','저장된 열 설정이 없습니다')
        self.column(page,'age').locator('input[type=checkbox]').uncheck();self.action(page,'save','편집값을 계정에 저장했습니다')
        ...
        self.column(page,'desc').locator('input[type=checkbox]').uncheck();route,response=held.pop();route.fulfill(response=response)
