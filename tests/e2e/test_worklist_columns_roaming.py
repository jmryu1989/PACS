...
class ColumnsRoamingE2E(WorklistColumnsE2E):
    ...
    def test_roam_columns_01_two_browsers_explicit_apply_and_clear(self):
        ...
        expected=page.locator('#heads th').evaluate_all('els=>els.map(e=>e.dataset.key)')
        index=expected.index('techNote');self.assertGreater(index,0)
        expected[index-1],expected[index]=expected[index],expected[index-1];expected.remove('age')
        ...
        self.assertEqual(second.locator('#heads th').evaluate_all('els=>els.map(e=>e.dataset.key)'),expected)
