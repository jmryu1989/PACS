...
class ViewerRecoveryE2E(HeldMeasurementE2E):
    ...
    def test_recovery_01_comparison_holds_drafts_and_held_edits(self):
        ...
        draft=self.draw_length(p);draft.get_by_label('주석 문구').fill('B 새 측정')
        ...
        self.switch(p,a)
        p.once('dialog',lambda d:d.dismiss());p.get_by_role('button',name='보관 작업 버리기',exact=True).click()
        p.get_by_role('button',name='보관 작업 재개',exact=True).click()
        expect(p.locator('#kin-viewer-history')).to_contain_text('A 보관 수정')
        self.switch(p,b);p.get_by_role('button',name='보관 작업 재개',exact=True).click()
        row=p.locator('#kin-viewer-history section[data-kind=length]')
        expect(row.get_by_label('주석 문구')).to_have_value('B 새 측정')
    ...
    def test_recovery_02_busy_hide_conflict_has_no_fake_held_edit(self):
        ...
        p.once('dialog',lambda d:d.accept('같은 항목 숨김'));row.get_by_role('button',name='숨김',exact=True).click()
        expect(row).to_contain_text('서버에 다른 판');row.get_by_role('button',name='최신판 기준으로 내 수정 유지').click()
        expect(row).not_to_contain_text('보관 중인 수정 보기');self.guard(p,False)
        p.once('dialog',lambda d:d.accept('최신 내용 복원'));row.get_by_role('button',name='복원',exact=True).click()
    ...
    def test_recovery_03_committed_denied_receipt_preserves_other_study_and_uuid(self):
        ...
        self.switch(p,b);row=self.draw_length(p);row.get_by_label('주석 문구').fill('B 결과 미확인')
        seen=[]
        ...
        self.switch(p,a);p.get_by_role('button',name='보관 작업 재개',exact=True).click()
        ...
        self.switch(p,b);p.get_by_role('button',name='보관 작업 재개',exact=True).click()
        row=p.locator('#kin-viewer-history section').filter(has=p.get_by_role('button',name='같은 요청 재시도',exact=True))
    ...
    def test_recovery_05_resume_uses_current_source_verdict_without_discarding_draft(self):
        ...
        self.switch(p,a)
        row=self.draw_length(p);row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('저장 완료')
        row.get_by_role('button',name='편집',exact=True).click();row.get_by_label('주석 문구').fill('원본 미확인 중에도 보존')
        ...
        expect(row.get_by_label('주석 문구')).to_have_value('원본 미확인 중에도 보존')
