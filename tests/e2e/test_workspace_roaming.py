...
class WorkspaceRoamingE2E(WorkspacePersistenceE2E):
    ...
    def open_menu(self,page):
        ...
        expect(page.get_by_role('button',name='서버 배치 불러오기',exact=True)).to_be_enabled()
    ...
    def test_roam_01_two_browsers_all_panels_and_report(self):
        ...
        self.action(page,'서버에 배치 저장','계정에 저장했습니다');self.assertEqual(self.remote(page)['layout'],layout)
        ...
        self.action(y,'서버 배치 불러오기','불러왔습니다');self.assertEqual(self.stored(y),layout);self.same_sizes(y,layout);self.shot(y,'portrait-restored')
        ...
        self.action(y,'서버 배치 초기화','현재 창의 배치는 유지');self.assertIsNone(self.remote(y)['layout']);self.assertEqual(self.stored(y),layout)
    ...
    def test_roam_02_owner_switch_new_login_and_conflict(self):
        x=self.device();a=self.sign_in(x);self.action(a,'서버에 배치 저장','저장했습니다');saved_a=self.remote(a)
        b=self.sign_in(x,'doctor2');self.assertIsNone(self.remote(b)['layout']);self.mode_is(b,'auto')
        self.open_menu(a);a.get_by_role('button',name='서버에 배치 저장',exact=True).click()
        expect(a.locator('#workspace-server-status')).to_contain_text('세션이 변경');expect(a.get_by_role('button',name='서버에 배치 저장',exact=True)).to_be_disabled()
        b.locator('#layout-toggle').click();self.action(b,'서버에 배치 저장','저장했습니다');saved_b=self.remote(b)
        by=self.sign_in(self.device(),'doctor2');self.mode_is(by,'auto');self.action(by,'서버 배치 불러오기','불러왔습니다');self.mode_is(by,'portrait')
        ay=self.sign_in(self.device());self.assertEqual(self.remote(ay),saved_a);self.action(ay,'서버 배치 불러오기','불러왔습니다')
        by.locator('#layout-toggle').click();self.action(by,'서버에 배치 저장','저장했습니다')
        before=self.stored(b);self.action(b,'서버에 배치 저장','다른 창에서');self.assertEqual(self.stored(b),before)
        self.action(b,'서버 배치 초기화','다른 창에서');self.assertIsNotNone(self.remote(b)['layout'])
        self.action(b,'서버 배치 불러오기','불러왔습니다');self.mode_is(b,'landscape')
        self.action(b,'서버 배치 초기화','초기화했습니다');self.assertIsNone(self.remote(b)['layout']);self.assertEqual(self.remote(ay),saved_a)
    ...
    def test_roam_03_delayed_load_and_session_end(self):
        page=self.sign_in(self.device());page.locator('#layout-toggle').click();self.action(page,'서버에 배치 저장','저장했습니다')
        ...
        page.get_by_role('button',name='서버 배치 불러오기',exact=True).click();expect(page.get_by_role('button',name='서버 배치 불러오기',exact=True)).to_be_disabled()
        ...
        page.get_by_role('button',name='서버 배치 불러오기',exact=True).click();page.wait_for_timeout(100);self.assertEqual(len(pending),1)
        response=pending[0].fetch();other=page.context.new_page();other.goto(self.stack.proxy+'/worklist/hpacs-lite/main.html')
        expect(other.locator('#dbstat')).to_contain_text('DB 연결됨');self.sign_out(other)
        expect(page.locator('#workspace-server-status')).to_contain_text('세션이 변경');pending[0].fulfill(response=response)
        self.assertEqual(self.stored(page),before);expect(page.get_by_role('button',name='서버 배치 불러오기',exact=True)).to_be_disabled()

    def test_roam_04_failure_storage_denial_and_csrf(self):
        page=self.sign_in(self.device());page.locator('#layout-toggle').click();self.action(page,'서버에 배치 저장','저장했습니다');remote=self.remote(page)
        ...
        page.route(pattern,lambda r:r.fulfill(status=200,json=bad));self.action(page,'서버 배치 불러오기','형식을 확인할 수 없습니다');self.assertEqual(self.stored(page),before);page.unroute(pattern)
        page.route(pattern,lambda r:r.fulfill(status=500,json=dict(message='Synthetic failure')));self.action(page,'서버에 배치 저장','완료됐을 수 있으니');self.assertEqual(self.stored(page),before);page.unroute(pattern)
        ...
        page.reload();expect(page.locator('#dbstat')).to_contain_text('DB 연결됨');self.action(page,'서버 배치 불러오기','이 창에만 적용됨');self.mode_is(page,'portrait');self.assertIsNone(self.stored(page))
        self.assertEqual(self.remote(page),remote)
        page.route(pattern,lambda r:r.fulfill(status=403,json=dict(message='Synthetic denied')));self.open_menu(page);page.get_by_role('button',name='서버 배치 불러오기',exact=True).click()
        expect(page.locator('#workspace-server-status')).to_contain_text('접근 권한이 없습니다');expect(page.get_by_role('button',name='서버에 배치 저장',exact=True)).to_be_disabled();self.mode_is(page,'portrait')
    ...
    def test_roam_05_account_changes_while_old_read_is_pending(self):
        ...
        self.action(page,'서버에 배치 저장','저장했습니다');page.locator('#layout-reset').click();before=self.stored(page)
        ...
        expect(page.get_by_role('button',name='서버 배치 불러오기',exact=True)).to_be_disabled()
