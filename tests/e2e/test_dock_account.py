...
class DockAccountE2E(AppearanceAccountE2E):
 ...
 def test_dock_account_01_cross_browser_explicit_restore_and_live_work(self):
  ...
  other=self.login();g=self.workspace(other,a);g.get_by_role('button',name='Comparison',exact=True).click();g.get_by_label('Job Title',exact=True).fill('KEEP ROAM DOCK TITLE');other.locator('#findings').fill('KEEP ROAM REPORT')
  ...
  self.assertEqual(ViewerTechNoteE2E.snapshot(self,g),before);expect(g.get_by_label('Job Title',exact=True)).to_have_value('KEEP ROAM DOCK TITLE');expect(other.locator('#findings')).to_have_value('KEEP ROAM REPORT');self.assertEqual(self.jobs(a),[]);expect(other.locator('#reading-dock-status')).not_to_contain_text('다른 창')
