...
class StudyTagsE2E(WorklistE2E):
 ...
 def test_tags_02_browser_assignment_search_and_report(self):
  ...
  self.account();p=self.login();self.select(p,a);self.open_toolbar_group(p,'#study-tag-open');p.locator('#study-tag-open').click();p.locator('#study-tag-new').fill('SYNTHETIC <tag>')
  ...
  self.select(p,b);self.open_toolbar_group(p,'#study-tag-open');p.locator('#study-tag-open').click();p.locator('#study-tag-add').click();expect(p.locator('#study-tag-count')).to_contain_text('2건');p.locator('#study-tag-close').click()
  self.select(p,c);p.locator('#findings').fill('KEEP TAG REPORT');self.open_toolbar_group(p,'#study-tag-open');p.locator('#study-tag-open').click();p.locator('#study-tag-apply').click();expect(p.locator('#rows tr[data-uid]')).to_have_count(2);expect(p.locator('#findings')).to_have_value('KEEP TAG REPORT')
  expect(p.locator('#filterlist')).to_contain_text('태그: [개인] SYNTHETIC <tag>');self.open_toolbar_group(p,'#study-tag-clear');p.locator('#study-tag-clear').click();expect(p.locator('#rows tr[data-uid]')).to_have_count(3)
  other=self.login();self.select(other,a);self.open_toolbar_group(other,'#study-tag-open');other.locator('#study-tag-open').click();expect(other.locator('#study-tag-name')).to_have_value('SYNTHETIC <tag>');expect(other.locator('#study-tag-assigned')).to_contain_text('부여된')
 ...
 def test_tags_03_shared_ui_and_revocation(self):
  a=self.fixture();admin=self.account('jmryu');tid=str(uuid.uuid4());self.change(self.body(admin,'create',tid,scope='institution',name='SYNTHETIC institution'),'jmryu');self.account();p=self.login();self.select(p,a);self.open_toolbar_group(p,'#study-tag-open');p.locator('#study-tag-open').click();p.locator('#study-tag-scope').select_option('institution')
 ...
 def test_tags_04_lost_response_conflict_and_late_session(self):
  a=self.fixture();self.account();p=self.login();self.select(p,a);self.open_toolbar_group(p,'#study-tag-open');p.locator('#study-tag-open').click();expect(p.locator('#study-tag-status')).to_contain_text('최신 태그')
  def lost(route):
