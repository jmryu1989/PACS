...
class VolumePreferencesE2E(VolumeSyncE2E):
 ...
 def test_properties_06_account_new_browser_restores_after_modal_preserving_work(self):
  ...
  panel.get_by_role('button',name='Save MPR Preferences',exact=True).click();p.bring_to_front();self.open_toolbar_group(p,'#reading-appearance-open');p.locator('#reading-appearance-open').click();expect(p.locator('#appearance-account-save')).to_be_enabled();p.locator('#appearance-account-save').click();expect(p.locator('#appearance-account-status')).to_contain_text('계정에 저장했습니다')
  ...
  self.open_toolbar_group(other,'#reading-appearance-open')
