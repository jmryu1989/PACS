...
class ViewerOpeningE2E(ReadingWorkspaceE2E):
    ...
    def test_opening_01_window_workspace_prior_and_retained_work(self):
        ...
        self.choose(p, a)
        self.assertEqual(self.choice(p), dict(version=3, listTarget='window', includePrior=True, maxWindows=1, autoLoad=False, reuseClean=False))
    ...
    def test_opening_02_owner_relogin_other_browser_and_corruption(self):
        ...
        self.assertEqual(self.choice(b), dict(version=3, listTarget='window', includePrior=True, maxWindows=1, autoLoad=False, reuseClean=False))
        self.sign_out(b)
        a = self.sign_in(context)
        self.assertEqual(self.choice(a), chosen)
        other = self.sign_in(self.device())
        self.assertEqual(self.choice(other), dict(version=3, listTarget='window', includePrior=True, maxWindows=1, autoLoad=False, reuseClean=False))
