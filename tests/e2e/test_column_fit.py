...
class ColumnFitE2E(ColumnsRoamingE2E):
 ...
 def test_fit_04_explicit_reading_style_precedence_survives_width_apply(self):
  ...
  self.open_columns(p);self.fit(p,'name');p.locator('#wc-save').click();expect(cell).to_have_css('font-size','16px');expect(cell).to_have_css('color','rgb(215, 243, 255)');p.reload();expect(p.locator('#dbstat')).to_contain_text('DB 연결됨');expect(self.cell(p,a.uid,'name')).to_have_css('font-size','16px')
