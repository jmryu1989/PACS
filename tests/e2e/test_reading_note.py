...
class ReadingNoteE2E(ReadingWorkspaceE2E):
 ...
 def test_reading_note_01_shortcut_preserves_viewer_and_report(self):
  ...
  f.get_by_role('button',name='Comparison',exact=True).click();f.get_by_label('Job Title',exact=True).fill('UNSAVED VIEWER NOTE ROUNDTRIP')
  ...
  expect(f.get_by_label('Job Title',exact=True)).to_be_focused()
 ...
 def test_reading_note_04_bridge_failure_keeps_images(self):
  ...
  f.get_by_role('button',name='Comparison',exact=True).click();f.get_by_label('Job Title',exact=True).fill('KEEP VIEWER THROUGH ASSET RETRY')
  ...
  expect(f.get_by_label('Job Title',exact=True)).to_have_value('KEEP VIEWER THROUGH ASSET RETRY')
