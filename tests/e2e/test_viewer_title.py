...
class ViewerTitleE2E(ViewerIdentityE2E):
 def test_title_01_current_prior_preferences_and_embedded_name(self):
  ...
  p.locator('#findings').fill('KEEP TITLE REPORT');v.get_by_label('Job Title',exact=True).fill('KEEP JOB TITLE')
  ...
  expect(p.locator('#findings')).to_have_value('KEEP TITLE REPORT');expect(v.get_by_label('Job Title',exact=True)).to_have_value('KEEP JOB TITLE');self.assertEqual(self.snapshot(v),before)
