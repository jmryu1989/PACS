...
class ViewerHistoryE2E(ThumbnailRequestsE2E):
    ...
    def draw(self,p,label):
        ...
        expect(row.get_by_label('주석 문구')).to_have_value(label)
        return row
    ...
    def test_01_save_new_login_edit_hide_restore(self):
        ...
        row=self.draw(p,label);row.get_by_role('button',name='저장',exact=True).click()
        ...
        row.get_by_role('button',name='편집',exact=True).click();row.get_by_label('주석 문구').fill('수정한 주석')
        row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('저장됨 r2')
        p.once('dialog',lambda d:d.accept('합성 숨김 사유'));row.get_by_role('button',name='숨김',exact=True).click();expect(row).to_contain_text('저장됨 r3 · 숨김')
        ...
        p.once('dialog',lambda d:d.accept('합성 복원 사유'));row.get_by_role('button',name='복원',exact=True).click();expect(row).to_contain_text('저장됨 r4')
        row.get_by_role('button',name='이력',exact=True).click();expect(row).to_contain_text('합성 숨김 사유')
    ...
    def test_02_key_multiframe_new_login_and_readonly(self):
        ...
        row=p.locator('#kin-viewer-history section').filter(has_text='US 세 번째 프레임');expect(row).to_be_visible();row.get_by_role('button',name='영상으로 이동').click()
        ...
        row=p.locator('#kin-viewer-history section').filter(has_text='US UI key');row.get_by_role('button',name='영상으로 이동').click()
        p.wait_for_function("sop=>cornerstone.getRenderingEngines().filter(e=>e.id!=='_thumbnails').flatMap(e=>e.getViewports()).some(v=>v.getCurrentImageId?.().includes(sop+'/frames/3'))",arg=str(ds.SOPInstanceUID))
        w2,p2=self.open_viewer(f,'doctor2');expect(p2.locator('#kin-viewer-history')).to_contain_text('읽기 전용')
        self.assertEqual(p2.locator('#kin-viewer-history section').get_by_role('button',name='편집',exact=True).count(),0)
        self.assertEqual(p2.locator('#kin-viewer-history section').get_by_role('button',name='숨김',exact=True).count(),0)
    ...
    def test_03_network_retry_conflict_and_quota(self):
        ...
        row.get_by_role('button',name='같은 요청 재시도').click();expect(row).to_contain_text('저장 완료');self.assertEqual(seen[0],seen[1]);self.assertEqual(len(self.saved(f)),1)
        head=self.saved(f)[0];row.get_by_role('button',name='편집',exact=True).click();row.get_by_label('키 제목').fill('내 수정 보존')
        ...
        expect(row.get_by_label('키 제목')).to_have_value('내 수정 보존');expect(row.get_by_role('button',name='저장',exact=True)).to_be_disabled()
        row.get_by_role('button',name='최신판 기준으로 내 수정 유지').click();row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('저장됨 r3')
        self.assertEqual(self.saved(f)[0]['item']['title'],'내 수정 보존')
        row.get_by_role('button',name='편집',exact=True).click();row.get_by_label('키 제목').fill('503 보존')
        ...
        row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('저장 결과를 확인하지 못했습니다')
        expect(row.get_by_label('키 제목')).to_have_value('503 보존');p.unroute('**/viewer-items/*/revisions',unavailable)
        row.get_by_role('button',name='같은 요청 재시도').click();expect(row).to_contain_text('저장됨 r4')
        ...
        try:
            row.get_by_role('button',name='편집',exact=True).click();row.get_by_label('키 제목').fill('한도 실패 보존')
            row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('저장 공간 한도');expect(row.get_by_label('키 제목')).to_have_value('한도 실패 보존')
        ...
        row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('서버에 다른 판')
        row.get_by_role('button',name='최신판 기준으로 내 수정 유지').click();expect(row).to_contain_text('미저장 수정은 보관')
        p.once('dialog',lambda d:d.accept('숨김 충돌 복원'));row.get_by_role('button',name='복원',exact=True).click()
        expect(row.get_by_label('키 제목')).to_have_value('한도 실패 보존')
        row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('저장됨 r7')

    def test_04_readonly_arrow_revocation_and_401(self):
        f=self.fixture();w,p=self.open_viewer(f);row=self.draw(p,'권한 주석');row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('저장 완료')
        w2,p2=self.open_viewer(f,'doctor2');expect(p2.locator('#kin-viewer-history')).to_contain_text('읽기 전용')
    ...
    def test_06_large_oblique_pan_zoom_coordinates(self):
        ...
        row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('저장 완료')
        points=self.saved(f)[0]['item']['points'];row.get_by_role('button',name='편집',exact=True).click()
        ...
        p.keyboard.press('h');row.get_by_label('주석 문구').fill('큰 좌표 유지');row.get_by_role('button',name='저장',exact=True).click();expect(row).to_contain_text('저장됨 r2')
