...
class ViewerJobAnnotationsE2E(ViewerJobPrintE2E):
 ...
 def test_annotations_02_visible_output_old_revision_and_coordinates(self):
  ...
  p.get_by_label('작업 제목',exact=True).fill('주석 이력 보존')
  self.click_job(p,'주석 함께 새 비교 작업 저장','주석 이력도 함께 고정');job=self.jobs(f)[0]
  ...
  p.keyboard.press('1');self.click_job(p,'이 작업 복원','최신 이력입니다');self.assertEqual(self.pngs(p),before)
 ...
 def test_annotations_03_native_manual_values_pdf_and_forged_baseline(self):
  ...
  p.get_by_label('작업 제목',exact=True).fill('수치가 확인되는 과거 작업');self.click_job(p,'주석 함께 새 비교 작업 저장','고정했습니다')
  ...
  self.post(f,command)
  self.click_job(p,'작업 목록 새로고침','목록입니다');p.get_by_role('button',name='저장 영상 출력',exact=True).first.click()
 ...
 def test_annotations_06_original_replacement_with_frozen_measurement(self):
  def save_with_annotation(p,f):
   ...
   p.get_by_label('작업 제목',exact=True).fill('원본 교체와 주석 고정');self.click_job(p,'주석 함께 새 비교 작업 저장','고정했습니다')
 ...
 def test_annotations_10_roi_budget_and_accumulator_exception_restore(self):
  ...
  p.get_by_label('작업 제목',exact=True).fill('ROI 예외 복구');self.click_job(p,'주석 함께 새 비교 작업 저장','고정했습니다')
  ...
  try:
   p.get_by_role('button',name='저장 영상 출력',exact=True).click();expect(p.locator('#kin-job-print [role=status]')).to_contain_text('owned calculator fault',timeout=45000)
  ...
  p.get_by_label('작업 제목',exact=True).fill('ROI 누적 계산 한도');self.click_job(p,'주석 함께 새 비교 작업 저장','고정했습니다')
  p.get_by_role('button',name='저장 영상 출력',exact=True).first.click();expect(p.locator('#kin-job-print [role=status]')).to_contain_text('수치 재확인 범위가 너무 큽니다',timeout=45000)
  expect(p.locator('#kin-job-print').get_by_role('button',name='인쇄 / PDF')).to_be_disabled()
 ...
 def test_annotations_09_response_containment_and_cache_refusal(self):
  ...
  for bad in variants:
   ...
   try:
    p.get_by_role('button',name='저장 영상 출력',exact=True).click();expect(p.locator('#kin-job-print [role=status]')).to_contain_text('주석',timeout=45000)
    expect(p.locator('#kin-job-print').get_by_role('button',name='인쇄 / PDF')).to_be_disabled()
  ...
  try:
   p.get_by_role('button',name='저장 영상 출력',exact=True).click();expect(p.locator('#kin-job-print [role=status]')).to_contain_text('메모리가 부족',timeout=45000)
 ...
 def test_annotations_07_grid_blank_dpr_and_long_legend_pdf(self):
  ...
  p.get_by_label('작업 제목',exact=True).fill('네 셀 고정 주석');self.click_job(p,'주석 함께 새 비교 작업 저장','고정했습니다')
  for dpr in [2,1.25]:
