...
class DockPreferencesE2E(ReadingWorkspaceE2E):
 ...
 def test_dock_pref_02_reload_owner_isolation_reset_corrupt(self):
  a,b=self.pair();p=self.login();f=self.workspace(p,a);f.get_by_role('button',name='Measurements',exact=True).click();f.locator('#kin-dock-placement').select_option('top');saved=self.stored(p);self.assertEqual(len(saved),1)
  ...
  f.locator('#kin-dock-reset').click();expect(f.locator('#kin-dock-placement')).to_have_value('bottom');expect(f.get_by_role('button',name='Measurements',exact=True)).to_have_attribute('aria-expanded','false')
 ...
 def test_dock_pref_03_failed_storage_other_tab_and_small_screen(self):
  ...
  # The portrait workspace scrolls vertically; bring the embedded viewer into view.
  p.locator('#reading-frame').scroll_into_view_if_needed()
  ...
  f.get_by_role('button',name='Comparison',exact=True).click();f.locator('#kin-dock-placement').select_option('top');expect(f.locator('#kin-dock-preference-status')).to_contain_text('이 창에만 적용');self.assertEqual(self.stored(p),{});canvas_ready(f,2);self.bounds(f,True)
