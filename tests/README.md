...
- 계약: 새 읽기 route `GET /api/admin/metrics` 하나(`admin.controller.ts` → `PacsService.adminMetrics`). `need('admin')`·`inst()` 뒤 검사 접근 조건을 읽고, 제한된 계정은 403 `ADMIN_METRICS_RESTRICTED`(범위 밖 검사를 세어 주는 창이라 `/statistics`와 같은 이유), 읽는 동안 조건이 바뀌면 409 `STUDY_ACCESS_CHANGED`다. clinician-only는 guard 기본 거절(allowlist 밖)이며 clinician fixture는 바꾸지 않았다. `AdminController` 접두어를 `admin`으로 올리고 회원 네 경로를 `users…` 핸들러로 옮겼다 — 접두어+핸들러를 이어 붙인 URL과 route key는 그대로다. `invariants_live.py` ROUTES에 `("GET", "admin/metrics"): Route(Kind.TENANT)` 한 행을 더했다.
