...
- 조회: SQL은 표에 있는 action이면서 detail에 이 기관 문자열이 있거나 target이 기관인 action이면 target이 이 기관인 행만 후보로 넘기고(보이는 행을 빠뜨리지 않는 넓은 조건), 정확한 판정은 `readAuditPage`가 쪽을 자르기 **전에** 한다. `total`·다음 쪽 여부·빈 목록은 보이는 행만 센다. 다음 쪽 값(`next` → `after`)은 첫 쪽이 읽은 가장 큰 id(기준)와 마지막 행 id를 AES-256-GCM(프로세스 키, 10분, 기관+계정 묶음)으로 봉인한 값이라 행 id의 틈으로 숨긴 행 수를 알 수 없고, 위조·다른 계정·만료·API 재시작 전 값은 409 `ADMIN_AUDIT_CURSOR_EXPIRED`다. `limit`(1–100, 기본 25)·`after` 밖의 질의는 400 `ADMIN_AUDIT_QUERY_INVALID`, DB 읽기 실패는 503 `ADMIN_AUDIT_UNAVAILABLE`(빈 목록이 아님)이다. 검사 접근이 제한된 관리자는 403 `ADMIN_AUDIT_RESTRICTED`(검사 행에 overlay 같은 검사 내용이 실리므로, 운영 지표와 같은 이유 — 읽는 사람의 관문이며 귀속에 쓰지 않음), 읽는 동안 조건이 바뀌면 409 `STUDY_ACCESS_CHANGED`다.
