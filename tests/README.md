...
REQ-D09-OUTPUT-IDENTITY → RISK-D09-REPORT-MISATTRIBUTION → TEST-D09-OUTPUT-IDENTITY: `node --test tests/viewer_job_print_identity_test.cjs`의 순수 27건은 현재/비교 검사 명칭, 실제 달력 날짜와 선후 관계, 판독문 선택 항목, 환자·검사 식별행, 보고서별 named page와 margin-box CSS, 미확정 편집문 구분, 추가기재 라벨과 출력 원본 읽기의 전송 복구를 검사한다(이전 「14건」은 첫 묶음 기준이라 낡은 수였다). `python tests/viewer_job_print_pages_test.py`의 격리 Chromium 15건은 현재보다 이후·이전·같은 날짜와 미상 날짜, 긴 식별정보의 하단 여백, 단일 검사, named-page 미지원 거절, 미확정 편집문 출력·거절, 추가기재 표기와 잘못된 `action` 거절 및 preview 무쓰기를 실제 PDF 페이지에서 확인하며, 같은 파일의 순수 2건이 PDF 추출 순서 규칙을 고정한다. validate의 runtime과 measurements job은 각각 `record-run.py`로 원문과 소스 해시를 남기고 합성 artifact만 게시한다. 실제 프린터의 물리 여백·색·DPI 수용은 별도다.
