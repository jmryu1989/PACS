...
- 매트릭스: `tests/clinician_policy_fixtures.json`의 `route_matrix`가 통합 head 9e7f698의 모든 (method, template) 110개에 S5-U6b의 1개, S5-U4a의 6개와 S5-U4c의 5개를 더한 122개를 한 행씩 가진다 — public 4 + session 2 + business 16 + denied 100. denied는 빼기로 만들지 않고 근거 묶음마다 이름을 적는다(report 17·viewer 14·operations 9·workspace 32·admin 10·consultation 5·connect 9·gateway 4, 뜻은 `basis_legend`). 104행 기준선 이후 추가된 18개는 `post_baseline`에 결정·단위·커밋·근거와 함께 따로 적는다: `GET clinician/studies`·`GET clinician/studies/:uid/report`(business, S5-U1b), `POST studies/:uid/dictation`(denied, S3-ASR 판독문 작성 경로), `POST gateway/receipt`·`GET gateway/retry-requests`(denied, Gateway client-credentials 전용), `POST studies/:uid/gateway-retry`(denied, 기사의 Now Retry), `GET admin/metrics`(denied, S5-U6b 운영 지표, need admin), 질문 route 6개 `GET questions`·`GET questions/:id`·`GET studies/:uid/questions`·`POST studies/:uid/questions`·`POST questions/:id/entries`·`POST questions/:id/close`(business, S5-U4a), 영상 요청 route 5개 `GET image-requests`·`GET image-requests/:id`·`GET studies/:uid/image-requests`·`POST studies/:uid/image-requests`·`POST image-requests/:id`(business, S5-U4c).
- 순수(TEST-S5-U1c-INVENTORY, `python -B tests/clinician_policy_test.py`, 23건): test_05는 컨트롤러 − 매트릭스 = ∅, 매트릭스 − 컨트롤러 = ∅, 두 곳에 있는 행 없음, 컨트롤러 = `invariants_live.py` ROUTES(양방향), 개수(`route_matrix.counts`, S5-U6b `GET admin/metrics`·S5-U4a 질문 6행·S5-U4c 요청 5행 뒤 122/4/2/16/100), 기준선 이후 경로마다 정확히 한 `post_baseline` 행을 단언한다. 새 route는 매트릭스 행(session/business면 live 사례까지)을 붙이기 전에는 이 시험과 live test_01이 실패한다. test_13은 승인 토큰 × 모든 route 모델에서 legacy 역할이 하나라도 있으면 clinician 게이트를 만나지 않고 clinician-only는 denied 행을 하나도 지나지 않음을, 소스에서 `clinicianOnly(` 호출이 guard·`viewer.controller.ts`·`report-preview.controller.ts`·`pacs.service.ts`(`/statistics`) 네 곳뿐이고 역할 존재 검사(`includes('clinician')`)가 없음을 고정한다. test_14는 live 매트릭스 데이터(allow 행마다 positive·wrong_role·wrong_tenant, 설계상 비거절은 pending/invalid logout 예외 하나)와 live 모듈 배선을 본다.
...
- 계약: 새 읽기 route `GET /api/admin/metrics` 하나(`admin.controller.ts` → `PacsService.adminMetrics`). `need('admin')`·`inst()` 뒤 검사 접근 조건을 읽고, 제한된 계정은 403 `ADMIN_METRICS_RESTRICTED`(범위 밖 검사를 세어 주는 창이라 `/statistics`와 같은 이유), 읽는 동안 조건이 바뀌면 409 `STUDY_ACCESS_CHANGED`다. clinician-only는 guard 기본 거절(allowlist 밖)이며 clinician 허용 목록은 바꾸지 않았다. S5-U1c 병합 뒤 `route_matrix`에는 `GET admin/metrics`가 denied(admin) 행과 `post_baseline` 행으로 들어가 111 = 4 + 2 + 5 + 100이 되었고, S5-U4a 질문 6행과 합친 뒤에는 117 = 4 + 2 + 11 + 100이다. `AdminController` 접두어를 `admin`으로 올리고 회원 네 경로를 `users…` 핸들러로 옮겼다 — 접두어+핸들러를 이어 붙인 URL과 route key는 그대로다. `invariants_live.py` ROUTES에 `("GET", "admin/metrics"): Route(Kind.TENANT)` 한 행을 더했다.
