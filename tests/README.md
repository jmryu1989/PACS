...
- 뷰어(`config/ohif.js`, 즉시 검토 경로): 서버 `/me`의 업무 역할이 clinician뿐인 세션(`kinViewerClinicianOnly`, `api/src/clinician-policy.ts` clinicianOnly와 같은 규칙)에서 Measurements & Key Images는 `GET studies/:uid/viewer-items?limit=100`과 받은 서명 cursor 그대로만 읽고(includeHidden·recheck 없음, 서버는 숨긴 항목 요청을 400으로 거절), 모든 쪽의 uid·`reportVersion`이 같을 때만 그린다(다르면 받은 쪽도 버림). 확정 전은 withheld, 항목 없음은 empty, 404/409/400은 서버 문구·HTTP 상태·코드로 failed, 403은 denied(`Recheck Access`는 다시 읽기)이고 상태는 `#kin-viewer-history[data-read-only]`에 남는다. 행은 `Read-only`와 `Go to Image`뿐이며 Length·Angle·Ellipse ROI·Add Key Image·Download SR·Store SR·Edit·Save·Hide·History·Recheck Source·Remeasure를 두지 않는다. 수동 측정 도구는 새 측정을, SR 명령은 생성·저장을 거절하고, 뷰어 도구로 그린 표식을 미저장 항목으로 받지 않는다(저장된 측정은 잠근 채 그린다). Findings·Job·Tech Note 모듈은 붙이지 않고(`kinViewerReadOnlySession`의 `/me` 한 번), Recent Layout·Hanging Protocol 대신 `Viewer Status` 상태 줄만 남긴다. 한 번 clinician-only로 답한 문서는 끝날 때까지 읽기 전용이다. `/me`를 받지 못하면 모듈은 예전처럼 붙고 그 안의 쓰기는 각 모듈의 판독의 역할 확인과 서버가 막는다. 컨트롤을 두지 않는 것은 화면일 뿐 권한이 아니다 — 쓰기 거절은 S5-U1a/U1b 서버가 한다.
- 시험(TEST-S5-U2b-DOM): `python -B tests/clinician_viewer_dom_test.py` — 16건. 홈은 배포 파일(`clinician.html`·`clinician.js`·`auth.js`)을 합성 origin에서 그대로 싣고, 뷰어는 `config/ohif.js` 전체를 cornerstone·도구 그룹·grid·서비스 스텁 페이지에 싣는다(Findings·Job·Tech Note·Hanging Protocol은 마운트만 기록하는 스텁). 01 Open Viewer·Compare의 창 이름·opener·주소, 팝업 차단·UID 형식, 02 같은 서버 환자 키만 후보(같은 이름·화면 ID가 같아진 다른 환자·다른 기관 같은 원본 ID·키 없음 제외, 화면에서 고친 같은 키 포함), 03 A→B→A와 목록 새로고침 뒤 남은 Compare 버튼, 04 영어 컨트롤/한국어 설명·피하는 말·12px·24px·Enter, 05 clinician-only 뷰어의 읽기 쿼리·Read-only 행·저장 측정 잠금 표시·쓰기 컨트롤 없음·도구와 SR 거절·모듈 미마운트, 05b 판독의·혼합 세션의 쓰기 패널과 모든 모듈, `/me` 무응답, 06 loading/empty/withheld/failed(404·409·400·다른 검사 답·다른 판 쪽)/denied와 Refresh, 07 비교 검사를 사이에 둔 A→B→A(이미 오던 A의 늦은 답은 새 읽기를 기다리는 동안에도 그리지 않음). 대조군(같은 테스트 안의 변형 파일): 이름·표시 ID로 묶는 홈은 다른 환자를 후보로 내고, 누를 때 키를 다시 보지 않는 홈은 다른 환자 쌍을 열며, 읽기 전용 도구막대·모듈 관문을 뺀 뷰어는 쓰기 컨트롤·모듈을 내고, 판 번호 고정을 뺀 뷰어는 다른 판 쪽을 이어 그리고, UID만 보는 뷰어는 A의 늦은 답을 그린다. CI는 measurements의 S5-U2a DOM 단계 다음 단계가 같은 인터프리터로 돌린다.
- 미실행·한계: hosted CI, 실제 OHIF·Orthanc 스택(`tests/e2e/test_prior_selection.py`·`tests/e2e/test_related_scope.py` 회귀는 hosted 전용)과 nginx `authz/dicom`·`dicom/lookup` 경로, 기존 뷰어 e2e. OHIF 기본 도구막대의 다른 측정 도구(Bidirectional 등)와 화살표는 이 창에서 저장되지 않는 로컬 표식으로 남는다(KIN 패널이 받지 않고 SR·서버가 거절). 뷰어의 `Open Source PDF`는 `GET studies`(임상의 거절)를 읽어 임상의에게는 실패 문구가 나온다. `tests/clinician_home_dom_test.py` test_02는 U2a의 비활성 `Open Viewer`를 고정하고 있어 이 변경과 충돌한다(이 단위의 소유 경로 밖이라 고치지 않았다).
