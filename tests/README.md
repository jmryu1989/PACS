...
- 매트릭스: `tests/clinician_policy_fixtures.json`의 `route_matrix`가 통합 head 9e7f698의 모든 (method, template) 110개를 한 행씩 가진다 — public 4 + session 2 + business 5 + denied 99. denied는 빼기로 만들지 않고 근거 묶음마다 이름을 적는다(report 17·viewer 14·operations 9·workspace 32·admin 9·consultation 5·connect 9·gateway 4, 뜻은 `basis_legend`). 104행 기준선 이후 추가된 6개는 `post_baseline`에 결정·단위·커밋·근거와 함께 따로 적는다: `GET clinician/studies`·`GET clinician/studies/:uid/report`(business, S5-U1b), `POST studies/:uid/dictation`(denied, S3-ASR 판독문 작성 경로), `POST gateway/receipt`·`GET gateway/retry-requests`(denied, Gateway client-credentials 전용), `POST studies/:uid/gateway-retry`(denied, 기사의 Now Retry).
- 순수(TEST-S5-U1c-INVENTORY, `python -B tests/clinician_policy_test.py`, 23건): test_05는 컨트롤러 − 매트릭스 = ∅, 매트릭스 − 컨트롤러 = ∅, 두 곳에 있는 행 없음, 컨트롤러 = `invariants_live.py` ROUTES(양방향), 개수(`route_matrix.counts`, S5-U4a 질문 6행 뒤 116/4/2/11/99), 기준선 이후 경로마다 정확히 한 `post_baseline` 행을 단언한다. 새 route는 매트릭스 행(session/business면 live 사례까지)을 붙이기 전에는 이 시험과 live test_01이 실패한다. test_13은 승인 토큰 × 모든 route 모델에서 legacy 역할이 하나라도 있으면 clinician 게이트를 만나지 않고 clinician-only는 denied 행을 하나도 지나지 않음을, 소스에서 `clinicianOnly(` 호출이 guard·`viewer.controller.ts`·`report-preview.controller.ts`·`pacs.service.ts`(`/statistics`) 네 곳뿐이고 역할 존재 검사(`includes('clinician')`)가 없음을 고정한다. test_14는 live 매트릭스 데이터(allow 행마다 positive·wrong_role·wrong_tenant, 설계상 비거절은 pending/invalid logout 예외 하나)와 live 모듈 배선을 본다.
...
- 정책·선언: `CLINICIAN_BUSINESS_ROUTES`와 `tests/clinician_policy_fixtures.json`의 `business_routes`·`allowed_additions`에 6행, `route_matrix` 116 = public 4 + session 2 + business 11 + denied 99(거절 수 불변), `post_baseline` 6행(b4d9989), `route_metadata_cases` 6건. `invariants_live.py` ROUTES에 6행을 `Kind.TENANT`로 선언했다.
