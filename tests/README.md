...
- 미실행·한계: 이 host에는 node·tsc가 없어 Nest build·cjs 시험은 hosted `kin-api:ci`에서만 돈다. live 11건과 policy live, `production_image_test.py`(컨테이너), 복원 producer/consumer, hosted CI는 실행하지 않았다. 복원 receipt는 합성 행이 3,560 bytes 늘었고(34자리 UID로 측정) catalog는 약 13 KB로 모델링해 전체 약 203 KB/256 KiB로 추정했다(hosted 실측 전). clinician-only 회원의 검사 단위 StudyAccess 철회는 시험하지 않았다(`target()` 미확장, D33 OQ-8 b). 이 단위 경로 밖의 `tests/clinician_read_serializer_test.cjs`:110-111이 clinician 비GET allowlist를 S5-U4a의 네 값으로 고정해 두 요청 쓰기 행 때문에 hosted measurements 단계가 실패한다 — 그 목록에 `'POST studies/:uid/image-requests', 'POST image-requests/:id'`를 더하는 후속이 필요하다(여기서는 고치지 않았다).
