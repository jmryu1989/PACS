...
REQ-D-VR-DISPLAY → RISK-D-VR-SOURCE/TRANSFER/GEOMETRY/LOSS → TEST-VR-DISPLAY. `node --test tests/volume_rendering_test.cjs` and `python tests/e2e/test_volume_rendering.py` verify manual CT volume rendering in a separate viewport: patient directions, rotation/zoom, native presets, opacity/shading/reset, known-box geometry and signed/rescaled HU equivalence. Failure, pending input, source/owner changes and close/retry preserve MPR pixels, marks and report input. The workflow returns to current MPR output and saved Job replay. Run fixture suites sequentially; direct regressions are orientation, batch, current MPR print and saved annotated output. The viewer shares the existing GL context but owns its actor, camera and lifecycle. This does not complete crop, editable transfer functions/preset persistence, sculpt, VOI overlays, VR batch, MR or large-volume support.
