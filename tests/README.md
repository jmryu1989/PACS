...
REQ-D-VR-CROP/TRANSFER → RISK-D-VR-GEOMETRY/TRANSFER/SOURCE/LOSS → TEST-VR-CROP/TRANSFER extends the same model suite with voxel-edge crop geometry and transfer-knot validation. E2E cases 11–12 cover manual I/J/K crop pixels, invalid bounds, custom HU/color/opacity, repeated global opacity, reset and reopen with MPR/source preservation. These added live cases are unexecuted until an isolated synthetic environment is verified; syntax checks do not establish renderer accuracy. Preset persistence, sculpt, VOI overlays, VR batch, MR and large-volume support remain incomplete.
