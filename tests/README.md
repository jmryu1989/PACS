...
- 실제 실행(2223a0d HEAD에서 커밋 뒤 기록, 로컬): `tests/clinician_read_serializer_test.cjs` **14/14**(Node v22.23.2, TS 원본), `tests/clinician_policy_test.py` **9/9**(routes 110 / public 4 / allowed 7 / denied 99). 미실행: `tests/clinician_read_live.py` 7건, U1b allowlist 기준 `tests/clinician_policy_live.py` 재실행, Nest build/tsc, 같은 최종 SHA의 candidate invariants·worklist, hosted CI.
