...
- live(TEST-S5-U1b-LIVE, hosted 합성 전용): `python scripts/run-tests.py --module tests/clinician_read_live.py --mode live --unit s5-u1b-clinician-read --timeout 900` — 허용 행마다 positive·wrong-role·wrong-tenant, 원격판독 열림/취소 경계, `/statistics` 닫힘, W→T→A→addendum→reset→P 판독 상태 전환, 회원 회수, 같은 사용자 A→B→A. 소유 계정·합성 검사만 쓰고 표시 항목 행은 fixture 정리 전에 자식부터 지운다.
...
- 실제 실행(96a1fd5에 커밋한 blob과 같은 작업 파일, 로컬): `tests/clinician_read_serializer_test.cjs` **12/12**(Node v22.23.2, TS 원본), `tests/clinician_policy_test.py` **9/9**(routes 110 / public 4 / allowed 7 / denied 99). 순수 serializer 시험은 validate `measurements` job의 `tmp/workspace-ui-ci/clinician-read-serializer` 단계(`node:22-alpine`, 네트워크 없음·읽기 전용 mount, `KIN_CLINICIAN_POLICY_MODULE=api/src/clinician-policy.ts` — 이 job은 kin-api:ci를 만들지 않는다)로 push·PR마다 실행하며 hosted 실행 결과는 아직 없다. 미실행: `tests/clinician_read_live.py` 5건(test_01 목록 좁힘·볼 수 있는 검사만 total, test_02 머리 판 확정→reset 뒤 상태만, test_03 뷰어 쌍·표시 항목·`/statistics`·원격판독 경계, test_04 회원 회수, test_05 같은 사용자 A→B→A), U1b allowlist 기준 `tests/clinician_policy_live.py` 재실행, Nest build/tsc, 같은 최종 SHA의 candidate invariants 83·worklist 15. live 모듈은 candidate 선택에 넣지 않고 hosted에서 `scripts/run-tests.py`로만 실행한다.
