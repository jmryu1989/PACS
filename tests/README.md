...
- 알려진 충돌·미실행: 새 경로가 `pacs.controller.ts`에 들어가 `tests/study_identity_source_test.py`의 컨트롤러 sha 고정, `tests/clinician_policy_test.py` test_01(allowlist = fixture)·test_09(`pacs.controller.ts`에 clinician 없음), `tests/clinician_read_serializer_test.cjs` 첫 시험(allowlist = fixture)이 실패한다 — fixture와 고정값은 이 단위 소유가 아니어서 고치지 않았다. hosted CI, live 4건, Nest build/tsc, `tests/e2e/test_related_scope.py`는 실행하지 않았다. U2b Comparison 칸에는 충돌 표식이 없다(타임라인 칸이 그보다 앞에 있다). 원본 생년월일·성별은 Orthanc가 검사마다 보관하는 환자 태그(QIDO 기본 응답, includefield로 명시하지 않음)에서 읽으며, 값이 비었거나 규칙으로 읽을 수 없는 검사는 같다고도 다르다고도 하지 않는다(`not_comparable`). 검사마다 다른 값이 실제로 오는지는 live 02가 hosted에서 처음 확인한다.
