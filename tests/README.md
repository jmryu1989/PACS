...
- 시험(TEST-S5-U2a-DOM): `python -B tests/clinician_home_dom_test.py` — 9건. 합성 origin에서 배포 파일(`clinician.html`·`clinician.js`·`index.html`·`auth.js`)을 그대로 싣고 `/api/me`·임상의 목록(서명 cursor 모양 쪽 나눔)·판독 읽기를 시험 안 저장소로 답한다. 01 착지(index.html 7역할 조합과 승인 대기, main.html의 clinician-only 넘김과 혼합 세션 boot, 근거 세 줄 고정), 02 식별자·확정본·key image·심은 작성자 칸 미표시·마크업 무실행, 03 확정 전 상태만(본문을 실어도 미표시)·형식 오류·다른 검사 답·404·403·Retry, 04 A→B→A, 05 대조군(UID만 보는 판·확인 없는 판은 늦은 답을 그려야 한다), 06 목록 세 상태·거절 표시·늦은 목록과 그 대조군, 07 세 쪽 cursor 전달·진행 표시·형식 오류·계정 바뀜, 08 영어 컨트롤/한국어 설명·피하는 말·수신 확인 문구 없음·대화상자 없음·12px·24px·키보드, 09 로그아웃·다른 탭 종료·승인 대기/무효·세션 없음. CI는 measurements의 S5-U5a DOM 단계 다음 단계가 같은 인터프리터로 돌린다.
