...
REQ-S3-ASR-U5-AUTHZ/PARITY/INERT/ORDER/INPUT/SESSION/PROOF → RISK-U5-WEAKER-COPY/TENANT/PRELIM-LEAK/OVER-REFUSAL/SIDE-EFFECT/HOLD-TOUCH/ORDER-LEAK/INPUT-DRIFT/CSRF/GATEWAY/FALSE-PROOF → TEST-S3-ASR-U5-LIVE·TEST-S3-ASR-U5-ORACLE: 받아쓰기 경로 `POST /api/studies/:uid/dictation`이 **판독문 초안 저장과 같은 사람을 같은 답으로 거절하고 같은 사람을 들여보내는지** 실스택에서 본다. 기존 REPORT 배터리는 이 경로에 `<500`만 본다. `invariants_live.py`의 여덟 시험 `test_dictation_u5_01`~`08`(T7만 `BffInvariantTests`)이 fixture 여덟 개에서 26건(거절 12·입력 4·대조 10)을 부른다: 역할, 기관(원격판독 수신 기관은 입장), 남의 살아 있는 점유 `REPORT_HELD`(점유자·만료 점유는 입장), 촬영 중 비응급(응급은 입장), 예비 판독 제3자(작성자·지정 상급자는 입장), StudyState 없는 uid, 인증 없음, 게이트웨이(정확한 경로 거절 본문은 `validGateway`를 지난 뒤에만 나온다), 쿠키의 CSRF 누락과 위조 세션(`인증 세션이 없습니다`), 그리고 권한 거절이 파서가 미뤄 둔 400/413보다 **먼저** 답하는지. 거절은 정확한 상태·본문, 그 uid의 감사 행 **0**(action 무관), `Report`·`ReportDraft`·`ReportVersion` 행과 `StudyState` 행이 **그 호출 바로 앞뒤로** 같음, `Cache-Control: no-store`, 응답에 fixture 비밀 없음으로 판정하고 같은 시험의 대조가 통과해야 센다. 엔진이 없는 CI에서 503 `DICTATION_NOT_CONFIGURED`는 **입장 증인일 뿐 거절의 증거가 아니다** — 대조와 입력 거절은 `dictation.request` 한 행(행위자 = 호출자, 키는 정확히 `bytes·seconds·ms·engine·outcome`, 전사문 없음)을 남긴다. 점유 만료(`backdate_hold`)·Verify·원격판독 지정은 사례 사이의 선언된 준비 단계이고 다음 사례의 직전 스냅숏에 들어간다. DB 관측은 스냅숏마다 합성 SELECT 한 번, 사례당 두 번이다. 판정표·기대 본문·문제 태그는 `.env`를 읽지 않는 stdlib 모듈 `tests/dictation_refusal_oracle.py`에 있다. `python -B tests/dictation_refusal_oracle_test.py`(hanging-protocols 잡의 기록 단계)는 26 벡터와 **정확한 태그 집합**을 요구하는 돌연변이로 판정기를 검사하고, 기대 문구를 `api/src`의 해당 줄에, 실스택 시험의 배선(사례 순서·키·A1/A3/A4/A5)을 AST로 고정한다. 실스택 로그에는 시험마다 `U5-PRECONDITION`, 호출마다 `U5-CASE`, 끝에 `U5-SUMMARY` 한 줄이 남으며 본문·토큰·비밀·오디오는 찍지 않는다. 선택 고정값은 `invariants_live` 71 → **79**, worklist 15는 그대로다. 이름 붙은 잔여: 추론 뒤 재관문 거절의 감사 outcome이 `DICTATION_ENGINE_FAILED`로 남음(DQ4, 별도 `api/src` 단위), 제한된 검사 접근 정책 404는 받아쓰기·`putReport` 모두 실스택 미관측(DQ5), 설정된 엔진이 필요한 200·BUSY·TIMEOUT·재관문은 컴파일 수준만, nginx 2 MB 초과 413 미관측, admin 전용(판독의 아님) 신원 없음. **실스택 여덟 건은 아직 실행되지 않았다 — 첫 실행은 호스티드 후보 CI다.**
...
`python tests/e2e/test_finding_worklist.py` (7개, 4는 S2-B2, 5는 S2-L, 6은 S3-U2b 실스택 인용, **7은 구조화 입력 창이 Image Findings 서랍과 함께 떠 있을 때의 실제 기하와 환자 ID 단축키**; 기존 `measurements` 프로필의 19번째 suite이며 `--class FindingWorklistE2E`가 클래스에 선언된 `test_*`를 전부 고르므로 새 메서드는 자동 선택된다. suite 수·per-suite 540초 상한·25분 profile deadline은 그대로다). 7번은 서랍을 연 채 실제 `Structured` 단추를 눌러 `GEN-1 TECHNIQUE`을 고르고, **1680×1100과 1366×768 두 뷰포트에서** 창과 서랍이 둘 다 보이는 동안 `#struct-value-text`·`#struct-apply`·`#struct-cancel`의 사각형과 `elementFromPoint` 적중을 재어 `R15-ENTRY` JSON 한 줄로 남긴다(실패해도 남긴다). 기하는 **두 번** 잰다 — 아무것도 누르기 전(`initial`)과 창이 열린 뒤 또는 실패 시점(`measured`) — 이고, `#b-structured`·`#reading-findings-open`·`#reading-related-region`·`.rbtns`·`.redit`의 사각형도 함께 싣는다. 화면에 없는 컨트롤은 0×0이라 `elementFromPoint(0,0)`가 엉뚱한 답을 주므로 항목마다 `present`를 붙여 **「가려짐」과 「아예 없음」을 구별**한다. 진입 단추의 적중은 **누르기 전에** 단언한다 — `click()`도 같은 사실을 알아내지만 20초를 다 쓰고 무엇이 가렸는지 말하지 않는다. **첫 호스티드 실행(run 35692815424)에서 1680×1100은 전 단계 통과, 1366×768은 창이 열리지조차 못했다**: 서랍이 `bottom:12px`+`max-height:min(62vh,640px)`라 위쪽 가장자리가 `높이−12−h`(1100에서 563, 768에서 280)로 뷰포트가 낮을수록 올라오는데 `.rbtns`(`main.html:236`)의 세로 위치는 뷰포트 높이와 무관해서, 대략 794px보다 낮은 화면에서는 서랍이 판독문 단추 줄을 덮는다(가로채는 요소는 서랍 자신의 `Reload Findings`·`Show Hidden`). **이것은 모달 z-index 문제가 아니며 창이 열리기도 전의 일이다.** 고친 방법은 상수가 아니라 **실측 경계**다: `#reading-findings`의 `max-height`에 `calc(100vh - 12px - var(--reading-findings-top, 0px))`를 한 항으로 더하고(`reading-workspace.css:77`, 기존 62vh·640px 상한과 12px 여백은 그대로), `reading-findings.js`가 패널이 **보이는 동안에만** `.report-p .redit`의 실제 `top`을 읽어 그 변수에 쓴다 — 단추 줄이 몇 줄로 접히든, 초안·인용 표시줄이 나타나든 그 값이 따라오므로 서랍의 윗변은 판독문 **본문 영역**에서 멈추고 단추 줄을 덮지 않는다. 변수가 없으면 항이 `100vh−12px`라 두 상한보다 커서 **예전 동작 그대로**다. 갱신원은 셋뿐이다 — `resize`(뷰포트), 캡처 `scroll`(판독 모드에서 `.right`가 스크롤되면 크기 변화 없이 위치만 바뀐다), `.redit`의 `ResizeObserver`(그 칸이 열의 `flex:1`이라 위쪽에서 접히거나 나타나는 모든 변화가 높이로 나타난다). 값이 같으면 쓰지 않고, 서랍이 흐름 밖이라 되먹임도 없다. 시험은 CSS 문자열을 따라 적지 않고 **렌더된 사각형**으로 `drawer.top ≥ rbtns.bottom`을 두 뷰포트에서, 열기 전과 창을 연 뒤 두 번 단언한다. 서랍은 한 번만 열고 다시 열지 않으므로 두 번째 뷰포트의 「열기 전」 단언이 곧 **이미 열린 서랍이 리사이즈를 따라오는가**에 대한 답이다. 창의 상자는 인라인 `width:620px`(`main.html:1023`)이고 서랍은 `right:12px`·`width:min(420px,100vw-24px)`(`reading-workspace.css:77`)라 가로 구간은 대략 1484px 아래에서만 겹친다 — **가림 여부는 측정 대상이지 단언이 아니며, 1366에서 실패하면 그것은 Astra에게 보고할 증거이지 z-index를 올리거나 서랍을 숨길 근거가 아니다.** 단축키 쪽은 `navigator.clipboard.writeText`를 세는 스텁으로 갈아끼워(권한 부여도 실제 쓰기도 없다) 창이 열린 채 `#struct-cancel`·`#struct-apply`에 초점이 있을 때 Control+Alt+c가 **아무것도 복사하지 않는 것**을 보이고, 실제 `Cancel` 클릭으로 닫은 뒤 판독문이 그대로임을 확인하고, 마지막에 `#copy-patient-id`에 초점을 두고 같은 키로 합성 `FINDNAV-…` ID가 정확히 한 번 복사되는 **양성**을 건다 — 그 양성이 없으면 두 음성은 공허하다. `Apply`는 한 번도 누르지 않는다. 워크리스트·통합 작업공간의 Image Findings는 선택한
...
`tests/viewer_opening_dom_test.py` covers REQ-D-WORKSPACE-OPENING → RISK-D-WORKSPACE-FOCUS-STEAL → TEST-VIEWER-OPENING-FOCUS-DOM in the `measurements` job (isolated Chromium, synthetic approved session object, no stack). `close()` restores focus before the queued close event, so the harness presses Done and moves focus to another button in one task. The pre-fix listener, derived from the shipped file by one exact block and pinned to blob `a97c477f`, must take focus back; if it does not, the case fails as a refuted hypothesis. The shipped listener must keep the moved focus. Focus left on the body, or with no restore target, still returns to the opener. An ended session never does. The old listener, loaded through the `KIN_VIEWER_OPENING_JS` override after an unmutated copy passes, must FAIL V1 on its own assertion text. Recorder JSON, source hashes and child logs go to `KIN_EVIDENCE_DIR`. This shows the mechanism; it does not prove the cause of a hosted `test_related_keyboard` failure.
...
S5-UI1 (D27) → RISK-SESSION-CONTROL-OFFSCREEN → TEST-S5-UI1-HEADER-DOM: `worklist_header_dom_test.py` renders the real `main.html` header with every linked stylesheet inlined in document order and no services, at 1024/1225/1280/1366/1680/1920 wide (plus reading mode at 1024/1225/1280/1366/1680/1920), with the admin-only Unmatched Studies item shown and a normal and a long institution/role string. Log out must be a native button whose whole box lies in the viewport, reached by Tab from the top of the document and activated by Enter and by a hit-tested click; the header box and every header item stay inside the viewport and the bar, no item overlaps another, and user/role text wraps rather than being clipped. The header wraps to a second row when narrow; the page-wide scrollWidth is not asserted because the template panel already exceeds 1024px without scripts on the base commit. It does not run the page script, so the real logout handler, Unmatched Studies visibility rule and live fonts are covered only by the hosted e2e suite.

S5-UI2 (D45, UXR-G-01) → RISK-S5-UI2-TOOLBAR (a control lost, duplicated or relabelled by the move; the page script changed; two rows from 1366px; an open group covering the list; the reading layout moving; a group the keyboard cannot open) → TEST-S5-UI2-TOOLBAR-DOM: the worklist toolbar keeps every control, id, label and handler but shows one row of seven groups — Search (ID or Name · Contains · Clear), Filters ▾ (study date; then Save Filter, Search & Saved Filters, Favorites, Study Tags, body parts; then the saved-search chips and the applied saved search with Edit Search on their own last line), Refresh with its Auto Refresh ▾ interval, View ▾ in three titled sections (Images: Image Opening, Viewer Windows, Move to Monitor; This Browser: Layout, Columns, Appearance & Tools, Reset Layout; Account: Account Layout), More ▾ (Consultations, Study Access, Worklist Alerts), the status texts, and the Search Mode row that `worklist-search.js` appends to the toolbar. Groups are native `<details name="kin-toolbar-menu">`: no script, one open at a time, Enter or Space on the summary. An open group becomes a full-width row inside the toolbar that pushes the list down instead of floating over it, because a `<details>` panel does not close on an outside click. In reading mode the toolbar keeps the 92px it always had there (reading-workspace.css caps it at 92px and the old toolbar filled it), so the reading layout G-LIVE-GEO measures does not move and opening a group never resizes the viewer. The closed Filters summary shows a dot while a study date, saved search, favorite list or tag filter is on. `worklist_toolbar_dom_test.py` (15 tests) pins the base commit 7a35570 by digest — the page script blocks, everything outside the head `<style>` block and the toolbar, the style block without the one S5-UI2 block (which sets no font), and the tag and text of the 36 base toolbar ids and of the 10 controls given ids — and checks the pins against git where the commit is in the clone. It places every base id in its declared group, counts the seven top-level groups, measures one 59px row (92px in reading mode) at 1366/1680/1920 in the normal, reading, portrait and reading-portrait layouts with empty, longest and overlong status texts, opens each group at its fullest (every control shown and enabled, twelve chips, a 400-character saved search) with the row staying put, the panel below it and above the list, and every control inside hit-testable and focusable, walks Tab left to right through the row and into an open group, and lets 1024/1280 wrap inside the viewport with every group still opening below the wrapped row. Hosted e2e: `WorklistE2E.open_toolbar_group(page, selector)` clicks the summary of each closed group holding a control, and every e2e step that clicks, selects, focuses or checks the visibility of a moved control calls it first. Not run by this test: the page script (role-based disabled and hidden stay its job), the hosted e2e suites and G-LIVE-GEO.

S5-UI2 fix1 (Astra S5-UI2-IMPROVE-R-001 F01–F03 and proposals 2–4, D51; markup and CSS only, the 51 page script blocks byte-identical): a status text cut short in the row — refresh stopped, an interval or search setting not saved, search criteria changed but not applied, results cleared — is read in full by pointing at the status group or the Search Mode row, or by keyboard focus inside either (the status group is a Tab stop between More and Search Mode). The same elements are drawn whole below the row, fixed to the group by CSS anchor positioning so the 92px reading toolbar's overflow does not cut them, taking no pointer so they close as soon as the pointer leaves and never block the list; nothing in the row moves and the closed row stays 59px (92px reading). No text is copied and no fixed `title` stands in for it. The Account Layout panel nested in View is placed in View's flow (`position: static !important` over its inline `position: fixed`), so the 12px/12px coordinates `workspace-roaming.js` writes when a resize or a finished request arrives while View is closed have no effect and the panel reopens under its summary. Filters and View are split into sections (proposals 2 and 3) with the resets set 12px apart at the end of their section; the Filters tooltip explains the dot. The DOM test adds the status texts by pointer and keyboard at 1366/1680/1920 in the four layouts, the Account Layout reopening after a hidden resize with the shipped `workspace-roaming.js` mounted, the nested Account Layout buttons in every open-View sweep, the sections and reset gaps, open groups at 1024/1280, and Shift+Tab back to a summary after the reading toolbar scrolled. The earlier recorded run `post-10-invariants-userfilter-pin` executed no assertion (its `python -c` body was split into argv) and is withdrawn as evidence; the `.userfilter` wrap pin in `invariants_live.py` is rechecked by an evidence script with a negative control.

S5-UI2 fix2 (hosted validate 36254174084; test only, `main.html` unchanged): the Account Layout case failed on the hosted runner in its four subtests because it asserted the intermediate `12px`/`12px` read immediately after `set_viewport_size()`. The hosted runner uses the same Playwright 1.60.0 and Chromium 148.0.7778.96 as the local runs, and that Chromium supports `<details name>`, `::details-content` and anchor positioning. The cause was timing: resize events (and the `toggle` event) reach the page as tasks after the Playwright call returns. The hosted read came before any of them and found the coordinates written when Account Layout opened with View open. A local probe shows the same effect when a read comes after zero or one of the two resize events. The case now closes View by its own summary instead of the shared `name`. It puts in a value `place()` never writes, waits until the window has the new size and the resize events have been delivered, and requires the script to have overwritten that value while View was closed. After View reopens it checks the result: `position: static`, and the same panel box under the written left/top, under `12px` and under ±4000px. It also checks that the panel is under its summary and inside View, the window and the toolbar, that the header and list are clear, and that Save, Load and Reset Account Layout can be hit-tested, focused and clicked. Removing the static rule, or replacing it with `relative` or `absolute`, fails all four subtests. Opening two groups at once, as a browser without `<details name>` would, leaves the row in place and stacks both panels below it, so no CSS fallback was added.

UI 백로그(사용자 결정 대기; 셋 다 페이지 스크립트 변경이 필요해 이 단위 밖이다):
- Search controls next to the search input: move `worklist-search.js`'s mount point so Search Mode, Search and Clear Results on Clear sit beside ID or Name (CSS `order` alone would break the Tab order).
- State on closed summaries: show the current interval or Manual on Refresh, an accessible summary of the applied conditions on Filters, and access state or open viewer windows on View/More.
- Outside click and Escape closing the groups: a separate behaviour contract including focus return and dialog hand-off.

S5-UI3 (D46, UXR-G-08; markup and CSS only, the 51 page script blocks byte-identical) → RISK-S5-UI3-REPORT-ACTIONS (a button lost, duplicated or changed by the move; the page script changed; the top row wrapping from 1366px or growing taller than one button row; an open More covering the report fields or the dictation pane; Dictate covered by the pane; a control the keyboard cannot reach) → TEST-S5-UI3-REPORT-ACTIONS-DOM: the report panel keeps all 18 buttons with their ids, labels, disabled states and handlers but shows seven controls — Approve (still the primary style), Save, Prelim, Dictate and More ▾ on one top row, Prev and Next on the footer row. More is a native `<details class="toolbar-menu" id="report-more">` (S5-UI2's classes, no `name`, no script; Enter or Space on the summary; accessible name "More Report Actions", Korean tooltip) whose panel becomes a row inside `.rbtns` that pushes the report fields down instead of floating over them, one section per line: Reading (Addendum, Transcribe), Status (Defer, Except, Mark CVR, Reset to Unread), Editor (Copy, Paste, Clear, Save as Template), Print and History (Print, History). Except and Mark CVR get ids (`b-except`, `b-mark-cvr`) and stay disabled placeholders. The Structured entry that the page script puts right before Print (`$("#b-print").before(button)`, the catalog is not empty) therefore opens first in Print and History. Dictate stays on the top row, a deviation from UXR-G-08's ⋯ list: `test_dictation_live.py` (G-LIVE-GEO) presses and measures it at 1366x768 and 1680x1100 with the dictation pane open, and an open More adds a panel of about 100px above the report fields at the 419px report column those widths give (the DOM test's geometry record), which is the room G5/G6 measure; on the top row the flow needs no menu. Closed, the top row is one button row and the footer one row where the base wrapped each to two at 419px. `report_actions_dom_test.py` (9 tests) pins the base ae04b19 by digest: `without_ui3()` puts the base bytes back into this unit's three regions (the button rows from the unit's comment, the footer row and the one CSS block) and the result must equal the base main.html byte for byte, checked against git where the commit is in the clone; `worklist_toolbar_dom_test.py` reads main.html through the same function, so S5-UI2's pins keep standing for everything outside both units. It compares the 18 buttons' tags, attributes and labels with the base (the two new ids taken off), places every button in its declared row or section, counts seven controls in view, allows one font rule (the summary's 11px, the size of the buttons beside it), measures one top row at 1366/1680/1920 in the normal, reading, portrait and reading-portrait layouts with the summary no taller than the buttons, opens More with and without the dictation pane at its cap (the top row staying put, the panel below it and above the fields and the pane, every control inside — Structured included — drawn, hit-testable and focusable, Dictate owning its centre outside the pane), lets 1024/1280 wrap with every control still reachable, and walks Tab through the row, into and out of an open More, with Enter and Space opening and closing it. Hosted e2e: every step that clicks or hit-tests a moved button (Print, History, Save as Template, Defer, Reset to Unread, Addendum, Structured) calls `open_toolbar_group(page, '#<id>')` first, and `report_dictation_input_dom_test.py` opens More before Structured. Not run by this test: the page script, the hosted e2e suites, G-LIVE-GEO and the Image Findings drawer bound (`reading-findings.js` follows the top of `.redit`, so an open More stays above the drawer; that is measured only on the hosted stack).

S5-UI3 백로그(UXR-G-08 중 이 단위가 하지 않은 것; 동작이나 페이지 스크립트가 바뀌어 사용자 결정 대기):
- Confirmation before signing with a 5-second wait (Changes L109): changes what Approve does.
- One editor and an icon toolbar for Copy, Paste, phrase insertion and Clear (L112, Reading L183-200), and the 3/12 top split (L111): editor behaviour and layout, not grouping.
- Dictate under More: only after G-LIVE-GEO is re-baselined with an open More, or the panel no longer costs the field region its height.
- Structured under Editor instead of Print and History: the page script places it before Print.
- Outside click and Escape closing More: the same behaviour contract as the S5-UI2 groups.
...
REQ-S4-U2-ORDER-RECONCILIATION (stage4 contract §4 S4-U2, P10/P11) → RISK-S4-U2-CROSS-TENANT-PAIR/TELE-PAIR/RESTRICTED-LEAK/INVENTED-ACCESSION/SEED-AS-SCHEDULED-PATIENT/FAILURE-AS-NO-IMAGES → TEST-S4-U2-SOURCE·CLIENT·SERVER·DOM·LIVE: the final page of `GET /studies` (the one that also carries `notObserved`) carries `orderReconciliation` = `{source:'engineering_only', orders}` or `null` when the enumeration could not be read. **Engineering only**: no authoritative site order source exists (G3), every order is a code seed without an accession, so nothing here closes architecture `:157/:158/:164` or IF-W09; the Order List heading says `Engineering Only` and the display shows only oid and state (no patient field, no scheduled time). Rule (`api/src/order-reconciliation.ts`): `Order.institutionId == caller == StudyState.institutionId` (tele-received studies are the sender's rows, so they never pair); an unlinked order with a non-blank accession lists own unlinked **observed** studies whose server-read `00080050` matches (3-valued relation, blank = `not_comparable`); a pair is a suggestion only, `/match` stays the only link; a restricted caller gets zero unlinked orders and zero candidates, and a linked order only for a permitted study. Schema: one nullable `Order.accession` (migration `20260924120000_order_accession`, no default, no backfill); provenance was not added — with no interface every row is engineering, and a column whose only reachable value is `seed` would state nothing. Client: `order-reconciliation.js` reads only the server answer that rides on the U1b observation (`applyObservation`/`markObservationUnavailable`); a failed or unreadable answer keeps the last one and says `관측 불가`; a cold failure shows nothing else; the offline `localStorage` seed never reaches it. Tests: `order_reconciliation_source_test.py` (stdlib: independent model over `order_reconciliation_vectors.json`, named wrong rules that must each fail a vector, source/migration pins), `order_reconciliation_test.cjs` (shipped module, runtime job), `order_reconciliation_server_test.cjs` (compiled rule and real `listStudies` over a fake store, `kin-api:ci`), `order_reconciliation_dom_test.py` (real sliced poll, measurements job) and `invariants_live.py` `test_s4u2_order_reconciliation_tenant_tele_restricted_seed_engineering_only` (real tokens, two institutions, tele, a restricted policy, psql-inserted synthetic orders removed by exact row; invariants 79 → **80**, candidate selection 101 → **102**). Not proved here: an authoritative order source, duplicate reconciliation (EG-7), accession mismatch on linked pairs and correction paths (S4-U5).

REQ-S4-U3-GATEWAY-RECEIPT (stage4 contract §2 axis C, §4 S4-U3, P2/P8/P9/P14; H-1 interim) → RISK-S4-U3-OWNERSHIP-LEAK/BODY-INSTITUTION/AGENT-CLOCK-ORDER/REPLAY-WRITE/EPOCH-ROLLOVER/AUDIT-FLOOD/FREE-TEXT-PHI/COMPLETE-AS-RECEIVED/NO-RECEIPT-AS-FAILURE → TEST-S4-U3-AGENT·SOURCE·SERVER·DOM·LIVE: `POST /api/gateway/receipt` (gateway credentials only, `needExact`) takes the closed body `{studyUid, phase, attempt, successCount, localCount, errorCode, epoch, seq}` — any other key (a timestamp, an institution, a reason) is 400; `0 ≤ successCount ≤ localCount ≤ 2^53−1`, `complete` needs `M == N`, `errorCode` is one of the agent's mapped codes exactly in `retry`/`failed` and null otherwise. A UID without the caller's own StudyState (absent or another institution's, including before announce) is one identical 404, decided before any order rule. Per study, under one advisory lock: nothing stored → store and audit `gateway.receipt.first` once; same `(epoch, seq, body)` → 200 with no write or audit; same key with another body → 409 `GATEWAY_RECEIPT_CONFLICT`; lower seq → 200 ignored; higher seq → store (audit `gateway.receipt.transition` only on a change into `complete`/`failed`); another epoch → 409 `GATEWAY_EPOCH_UNRECOGNISED`, stored row unchanged, at most one `gateway.receipt.epoch_unrecognised` audit per study per hour — no automatic rollover (H-1 HOLD; contract 6b stays deferred). The stored time is the server's reception time. Agent (`gateway/agent/agent.py`): the queue DB mints its epoch once; every write of a receipt field advances the per-study `seq` in the same statement; receipts go only after a successful announce, never carry the free reason (every `GatewayError` template maps to a code, unknown → `other`, enumerated by AST), and a receipt failure never changes the transfer; 400/404/409 are final, anything else stays owed with backoff. Schema: table `GatewayReceipt` (migration `20260924130000_gateway_receipt`, additive, CHECK backstops, cascades with its StudyState). Read surface: `GET /studies` rows carry `gatewayReceipt` (the S4-U1b label shape) only for the caller's own studies and only from its own credentials' rows; `null` is normal (`No Gateway Report`); bootstrap carries none; `complete`/`M == N` never promotes the observation. Tests: `gateway/agent/test_agent.py` (18, run by `invariants_live.py` `test_gateway_agent_queue_and_batch_contract`), `gateway_receipt_source_test.py` (stdlib model over `gateway_receipt_vectors.json`, named wrong rules, AST error mapping, pins), `gateway_receipt_server_test.cjs` (compiled rule and the real route/list over a fake store, `kin-api:ci`), `worklist_arrivals_dom_test.py` (real sliced poll) and `invariants_live.py` `test_s4u3_gateway_receipt_route_rules` (invariants 80 → **81**, candidate selection 102 → **103**; migrations 27 → 28, restore tables 37 → 38). Not proved here: end-to-end Gateway reliability with a hospital Orthanc (EG-1), authorised epoch rollover (H-1), offline vs not installed (EG-8), retry commands (S4-U4).

REQ-S4-U4-NOW-RETRY (stage4 contract §4 S4-U4, P7/Q4/N1/P14; U4 contract D1–D7 with review corrections R1–R6) → RISK-S4-U4-FAILED-REOPEN/ATTEMPT-RESET/RETRY-STORM/FALSE-DELIVERY-CLAIM/CROSS-TENANT/GATEWAY-ON-MEMBER-ROUTE/EPOCH-ROLLOVER/DUPLICATE-AUDIT/PHI-IN-POLL/INBOUND-PUSH → TEST-S4-U4-AGENT·SOURCE·SERVER·DOM·LIVE: `POST /api/studies/:uid/gateway-retry` takes an empty body only (`need(technician)`, so admin too; `kind === 'member'`; own institution). Absent, another institution's, tele-received and access-restricted studies get one byte-identical 404. Under U3's per-study advisory lock the request binds the stored `retry` receipt's `(epoch, seq)` in the append-only table `GatewayRetryRequest` (migration `20260924140000_gateway_retry_request`, FK to `GatewayReceipt`, cascades with it, never updated or deleted by the product). The first request writes one `gateway.retry.request` audit (requester, server time); a repeat returns `already_requested` with the first time and writes nothing. `failed` is 409 `GATEWAY_RETRY_UNSUPPORTED_F01` (F-01: the same bytes cannot succeed, and the UI shows that sentence with no control). Every other phase, or no receipt, is 409 `GATEWAY_RETRY_NOT_RETRY`. `GET /api/gateway/retry-requests?epoch=<uuid>` (`needExact` gateway; the query is exactly one lowercase epoch, and arrays or objects are 400) is read-only and answers `{studyUids}` only: one SQL statement keeps requests whose `(epoch, seq)` equals the stored `retry` receipt of this institution on a study of this institution, before `LIMIT 100`. There is no ack route; the Gateway's next receipt ends a request. Agent: a fixed 30 s poll (not configurable; first poll 30 s after start; skipped while receipts back off), a confirmation that re-sends the row's current U3 receipt and applies only on an exact `duplicate`, and then one compare-and-set `next_at=0 WHERE uid, phase='retry', seq, next_at>now`. `attempt`, `lastError`, the error code, sent SOPs, counts and seq are kept, and `pending_now` is never used. The inference holds only while one agent process writes its epoch (two hosts on one `queue.db` are H-1, outside this unit). Delivery is at least once while pending, and the effect is at most one `next_at` change per request. `Retry Requested` means stored, not retried: the next poll comes after any transfer in progress. Tests: `gateway/agent/test_agent.py` `RetryNowTests` (T1–T6, agent 18 → **25**), `gateway_retry_source_test.py` (stdlib model over `gateway_retry_vectors.json`, named wrong rules, AST single-writer/compare-and-set/cadence pins with mutants, route/SQL/migration/restore/client pins), `gateway_retry_server_test.cjs` (compiled rules and the real routes over a fake store, including an admin-only caller, `kin-api:ci`), `study_arrivals_test.cjs` (Now Retry action by phase substitution; `V.receipts` stays 9), `worklist_arrivals_dom_test.py` (control, one empty POST, stale answers dropped), the restore fixture (+1 table, row and CHECK/FK/PK probes) and `invariants_live.py` `test_s4u4_gateway_retry_route_rules` (invariants 81 → **82**, candidate selection 103 → **104**; migrations 28 → 29, restore tables 38 → 39). Not proved here: end-to-end with a hospital Orthanc (EG-1), epoch rollover or two-host epochs (H-1), reopening `failed` (H-2), cloud duplicate-STOW idempotency (EG-2).

REQ-S4-U5-STUDY-IDENTITY (stage4 contract §4 S4-U5, P10/P11/P12/Q2/Q5=A; U5 contract with review M-1–M-6 and N-1–N-9) → RISK-S4-U5-OV-AS-INPUT/CROSS-TENANT-PAIR/TELE-PAIR/RESTRICTED-LEAK/BLANK-OR-INVALID-READ-AS-RELATION/OVER-NORMALIZATION/STALE-RELATION/OPTIMISTIC-MODIFY/ORIG-REPAINT/OV-KEY-OR-MARKUP-INJECTION/RS-GATE-DRIFT/REPORT-ROW-DRIFT/IDENTITY-CLAIM → TEST-S4-U5-SOURCE·CLIENT·SERVER·DOM·LIVE: every `GET /studies` row carries `orderIdentity`, `null` unless the row is the caller's own study linked both ways to an own-institution order (`study.institutionId == order.institutionId == caller`, `study.orderOid == order.oid`, `order.studyUid == study.uid`, both matched); otherwise `{source:'engineering_only', oid, accession, patientId, patientName, birth, sex}` with each relation `match | mismatch | not_comparable`. **Engineering only and not a patient judgement**: `match` means two recorded strings are equal under the rule, nothing closes architecture `:164/:165`, and Q2's provenance column stays deferred by N3_Q2 (U2) until G3. Inputs are only the row's server-read QIDO tags and one tenant-pinned Order read per list or page (`ORDER_IDENTITY_SELECT` in `api/src/study-identity.ts`, taken before the access re-check); `StudyState.ov`/`orig` are never read and no Order value is sent. Rule: accession and Patient ID trimmed and otherwise exact (U2's rule), Patient Name = the PN **Alphabetic group** only with NFC, `^` as a space (so `KIM^CHUL SOO` equals `KIM CHUL^SOO`), whitespace runs collapsed and a-z folded; birth only as `YYYYMMDD`/`YYYY-MM-DD` naming a real Gregorian date (table-based leap years; `1962-02-30` is not comparable); sex only `M`/`F`; blank or unparseable on either side is `not_comparable`; Study Description is shown, never compared. Server containment (M-1/N-1): after every existing refusal, `PATCH ov` and a claimed `/match` `patient.orig` must be `null` or a plain object whose keys are `id,name,sex,birth,age,desc,ward,date,acc,modality` with text values (age may be a finite number), else 400 without a write; every existing status and message per caller and RS is unchanged (tech ov at P 403, the designated reviewer 400; match/unmatch 400 at T/P/A/H). Client: `study-identity.js` + the collapsed read-only `DICOM Identity` panel (`#study-identity`, tags from the response row, `Patient Name · Alphabetic` / `Alphabetic Absent`, `Linked Order <oid> · Engineering Only`, `No Linked Order`, `Tele-received · Not Compared`, `Unknown`, `Patient Mismatch`/`Accession Mismatch` badges, `관측 불가` keeps the last answer, guidance names Unmatch→Match at RS W and no report action elsewhere); the overlay repaints display fields only and the `matched`/`ts` cells are escaped; Modify Exam waits for the PATCH answer (a refusal keeps the modal and typed values, the toast carries the server wording); Unmatch repaints from the last server-read tags, never from `orig`; Match/Unmatch/Modify invalidate in-flight list answers; Order List `Refresh` re-reads the guarded `/bootstrap?states=omit` (latest answer only, same institution). Tests: `study_identity_source_test.py` (stdlib: vector-file well-formedness and source pins, not a model), `study_identity_test.cjs` (client model, runtime job), `study_identity_server_test.cjs` (compiled rule over every vector and the real `listStudies` over a fake store, `kin-api:ci`), `study_identity_dom_test.py` (real poll and correction paths, with b6a317c controls), and `invariants_live.py` `test_s4u5_order_identity_qido_only_w_gates_and_report_preservation` (invariants 82 → **83**, candidate selection 104 → **105**). Not proved here: a real site order interface (G3), other Orthanc versions' default QIDO fields, PN conventions at a real site.

REQ-S4-F01V-ABSENT-RECEIPT (EG1 contract §6 F-EG1-1/D-5; F01V contract with review amendment A-1 and constraints C-1–C-6) → RISK-S4-F01V-TENANT-LEAK/FALSE-CONTROL/STALE-ANSWER → TEST-S4-F01V-P/S/D/L: an own study that KIN never observed (announced, zero instances delivered) shows its last Gateway receipt and the existing Now Retry on its `Not Observed` item. On the page that completes `GET /studies`, after the unchanged absence list, one `GatewayReceipt` read pinned to the caller's institution (`studyUid in` the absent own UIDs, `institutionId: me`, no origin filter) runs before the access re-check, like the U2/U3/U5 tenant reads. An item gets `gatewayReceipt` (the unchanged U3 projection) only when such a receipt exists; otherwise it keeps exactly `{uid, origin, createdAt}`, so a stray receipt written by another institution's credentials is never shown, and tele-received or foreign studies are still never listed. No route, schema, agent, write or new patient field. The client copies the key only when it is sent and judges it by the unchanged U1b/U4 rules: no receipt keeps the item text byte-identical (no `No Gateway Report` in the list); an unreadable one gives the fixed sentence; `failed` gives M-of-N plus the F-01 sentence and never a control; `retry` with a bindable `epoch|agentSeq` offers `Now Retry` only to a technician (admin included) online, through one drawing shared with the worklist panel. `requestGatewayRetry` binds the clicked control and writes the answer to the element right after it. Items are kept per UID so a request in flight survives a poll; an item that leaves the list drops its token (its late answer is never written), one that comes back is built new (A→B→A), and the children are replaced only when the items change, so a redraw does not drop keyboard focus. `Retry Requested` still means stored, not retried. After this unit, the 3-key `notObserved` pin in `tests/e2e/test_study_arrivals.py` holds only while no own absent study in that stack carries a receipt (nothing there writes one), and Now Retry, which `gateway/README.md` places in the worklist, is also on the Not Observed list (`gateway/**` is outside this unit). Tests: `gateway_retry_source_test.py` `F01VPins` (T-P: the read line, its order, the conditional attach and copy, the shared drawing, two listeners and the request binding; mutants: no `institutionId: me`, read after the re-check, unconditional attach, no role gate) with the named pin moves `projectGatewayReceipt(` 1 → 2, `MAIN.count("gatewayReceipt")` 5 → 6, `#receipt-retry-note` selectors 2 → 1 and the U1b order `unchanged < notObserved` reversed to `notObserved < absent read < unchanged`; `gateway_receipt_server_test.cjs` (T-S, compiled over a fake store: projection, 3 keys, stray/tele/foreign, restored row, no second read for an unknown enumeration or a page that does not complete the list, the read before the policy re-check, no write); `worklist_arrivals_dom_test.py` (T-D, two methods including A-1: the same node across a poll and then `Retry Requested`, and leave-and-return with one POST and no note); `invariants_live.py` `test_gateway_announce_is_idempotent_and_preserves_dicom_origin` (T-L: failed and retry projections, 409 F-01, 200 requested, hallym 404 and no UID or epoch in its list, cleanup cascade; invariants stay **83**, candidate selection **105**). Not proved here: transferring an instance over 24 MiB (H-2 HOLD: the whole-study block and the F-01 sentence stay, and no recoverable `failed` producer exists), the F-01 capability placement (EG1 contract §6 D-6, open), and the EG1 end-to-end hospital Orthanc path.

REQ-S5-U1a-ROLE-DEFAULT-DENY → RISK-S5-CLINICIAN-WRITER-LEAK/UNCLASSIFIED-ROUTE/ROLE-LIST-DRIFT → TEST-S5-U1a-CLINICIAN-POLICY/CLINICIAN-LIVE: `clinician`은 `api/src/clinician-policy.ts` 한 곳의 역할 목록으로 guard·회원콘솔·Keycloak 클라이언트·렐름에 같은 커밋에서 들어간다. clinician만 가진 승인 회원은 Nest 데코레이터 메타데이터로 만든 `METHOD route/template` 키가 명시 allowlist(처음에는 `GET me`, `POST auth/logout`뿐, 업무 allowlist는 비어 있음)에 있을 때만 통과하고 나머지는 서비스 층 전에 `403 CLINICIAN_ROUTE_DENIED`다. public 4개, pending/invalid의 logout 예외, gateway 신원 닫힌 판정, CSRF 규칙, radiologist/technician/admin 단독·혼합 토큰의 기존 need/visible 경로는 바뀌지 않는다. `python tests/clinician_policy_test.py`(순수 stdlib)는 fixtures를 독립 모델로 판정하고 현재 컨트롤러 목록·`invariants_live.py` ROUTES·104행 기준선·allowlist를 대조하며 소스 핀을 검사한다. `tests/clinician_policy_live.py`는 hosted 합성 스택에서 clinician-only 토큰으로 public·allowlist 밖 모든 현재 경로가 guard 코드로 거절되고 감사 행이 남지 않음, pending/invalid 코드와 logout, 회원콘솔의 clinician 승인→혼합 역할→취소, 혼합/기존 역할 회귀를 검사한다(`scripts/run-tests.py --module tests/clinician_policy_live.py --mode live` 전용). 읽기 allow와 narrow DTO는 U1b, 전체 route 수용은 U1c이며, 이 단위는 임상의 화면·질문·전송을 포함하지 않는다. main 489c326 위 통합 head의 순수 시험 8건 실제 실행: 현재 컨트롤러 경로 **108** = public 4 + allowlist 2(`GET me`, `POST auth/logout`) + clinician-only 거절 **102**, 104행 기준선 이후 추가된 4개(`GET gateway/retry-requests`, `POST gateway/receipt`, `POST studies/:uid/dictation`, `POST studies/:uid/gateway-retry`)는 모두 거절, 기준선에서 사라진 경로 0. 시험은 숫자 상수가 아니라 구조식(거절 = 전체 − public − allowlist)으로 단언한다. 순수 시험은 validate `measurements` job의 stdlib 단계(`tmp/workspace-ui-ci/clinician-policy`)로 push·PR마다 실행하고, live 모듈은 candidate 선택(invariants 83 + worklist 15 + flow 7 = 105, 그대로)에 넣지 않으며 hosted에서 `scripts/run-tests.py`로만 실행한다(4건, 미실행).

REQ-S5-U5a-CLINICIAN-ROLE-ROUNDTRIP → RISK-S5-U5a-SILENT-ROLE-DROP/LABEL-RULE → TEST-S5-U5a-DOM/SERVICE: 회원 콘솔(`worklist-v0/hpacs-lite/admin.html`)의 저장은 PATCH `roles`로 회원 역할 전체를 바꾼다. 체크된 세 역할만 보내던 이전 화면은 `{clinician, radiologist}` 회원의 clinician을 오류 없이 뺐다. 역할 체크박스에 `clinician`(`임상의`)을 더했고, 이 화면이 그리지 않는 역할(서버 `APP_ROLES`가 화면보다 먼저 늘어난 경우)은 대화상자에 바꿀 수 없는 표시(체크·비활성·`name` 없음, 한국어 툴팁)로만 보이고 저장 목록에는 불러온 값 그대로 붙는다. 관리 역할 목록은 마크업의 체크박스 하나에서 읽는다. 저장은 대화상자를 열 때 보관한 값(승인 상태, 앞뒤 공백을 뺀 기관, 순서와 무관한 역할 집합)과 비교해 바뀐 필드만 보낸다(Astra S5-U5a-F01). 서버 `patchUser`는 `approvalState: APPROVED`·`institution`·`roles` 중 하나만 받아도 회원을 격리(세션 삭제·Keycloak 로그아웃)하고 받지 않은 필드는 서버의 현재 값으로 다시 쓰므로, 네 필드를 늘 보내던 저장은 아무것도 바꾸지 않아도 회원 세션을 끊었고 목록을 읽은 뒤 다른 관리자가 바꾼 기관·역할을 이 화면의 옛 값으로 되돌렸다. 바뀐 것이 없으면 요청 없이 대화상자에 `변경된 내용이 없어 저장하지 않았습니다.`를 보이고, 이미 APPROVED인 회원에게 `approvalState`를 다시 보내지 않는다. PENDING·INVALID 회원의 저장은 `approvalState: APPROVED`를 붙인 명시적 승인이고, `verificationOverride`는 체크했을 때만 변경과 함께 `true`로 간다. 역할을 바꿀 때만 전체 역할 집합(이 화면이 모르는 역할 포함)을 보낸다. 두 관리자가 같은 회원의 역할을 각각 바꾸는 경우는 서버에 버전 조건이 없어 나중 저장이 이긴다. 같은 단위에서 AGENTS §4에 따라 이 화면의 메뉴·버튼·상태명·패널 제목(진입 패킷 `wording_acceptance.english_required` 24개와 헤더의 `Worklist` 링크)을 영어로 바꾸고, 설명·툴팁·확인·오류 안내와 패킷이 제외한 필드 라벨·표 머리글·`역할` legend·역할 라벨(새 `임상의` 포함)·`전체 기관` 선택지는 한국어로 둔다. `tests/admin_member_roles_dom_test.py`(Playwright Chromium, 서버·네트워크·자격증명 없음)는 출하 `admin.html`·`auth.js`·`study-access-admin.js`를 합성 origin에서 그대로 싣고 `/api/me`·`/api/admin/users`를 시험 안 저장소(역할은 `AdminService.row()`처럼 정렬, `patchUser`처럼 받지 않은 필드는 현재 값 유지)로 답하며 모든 PATCH 본문을 기록한다. 답하지 않는 요청과 page error는 실패다. 01 clinician-only·혼합·legacy·정지 회원을 바꾸지 않고 저장하면(기관 앞뒤 공백, 역할 해제 후 재체크, 이메일 검증 예외만 체크한 경우 포함) 요청이 없고 한국어 안내와 함께 대화상자가 남으며 다시 열어도 같은 역할, 02 혼합 회원의 역할 변경은 `{roles}`만 보내고 clinician은 다른 역할을 바꿀 때 유지·관리자가 해제할 때만 제거, 03 모르는 역할의 읽기 전용 표시·무변경 비교에 포함·역할 변경 때 되돌려 보냄·다음 회원의 대화상자와 저장으로 새지 않음·마크업으로 해석되지 않음, 04 PENDING 회원을 키보드로 열어 clinician과 이메일 검증 예외로 승인(`approvalState`·`institution`·`roles`·`verificationOverride`), INVALID 회원은 역할 추가로 승인(`approvalState`·`roles`, 이미 가진 기관은 보내지 않음), 05 english_required 24개(정적 문구는 첫 목록 요청을 붙잡은 상태에서 읽음)와 버튼·제목·배지·헤더 링크의 한글 0, UXR-SP-34 단어 없음, 역할 라벨 12px 이상, 06 한국어 안내·확인·오류(공백 기관 거절, 서버 문구 그대로)와 회원 데이터 비번역, 07 대조군: 이전 역할 처리(clinician 체크박스 없음 + 이전 `admin.html:446` 제출)와 체크값만 보내는 제출을 같은 경로로 싣고 역할이 조용히 빠지는 정확한 출력을 단언, 08 목록을 읽은 뒤 저장소가 바뀐 상태에서 기관만 바꾸면 `{institution}`만 가고 다른 관리자가 더한 역할이 남으며, 역할만 바꾸면 `{roles}`만 가고 다른 관리자가 바꾼 기관이 남고, APPROVED 회원의 변경에는 이메일 검증 예외가 붙어도 `approvalState`가 없음, 09 대조군: c59e039의 네 필드 제출을 같은 경로로 싣고 무변경 저장이 PATCH가 되며 더 새 역할을 덮어쓰는 정확한 출력을 단언한다(01·08이 요청을 못 보는 시험으로 통과하지 않게). F01 이전 7개 사례 판은 로컬 1회 **7/7 통과**(Playwright 1.60.0, `tests/e2e/requirements.txt`와 같은 판)였고, 같은 변경에서 `python -B tests/clinician_policy_test.py` 8/8(역할·경로 목록 불변: 현재 108, clinician-only 거절 102). F01 수정본(9개 사례)의 로컬 결과는 수정 커밋 SHA에 묶인 실행 기록에만 두며 이 문서에는 적지 않는다. 서비스 절반은 `tests/clinician_policy_live.py` test_03(approve [clinician] → 혼합 → 취소)을 hosted에서 재사용하며 이 단위에서 실행하지 않았다. 이 DOM 시험은 `.github/workflows/validate.yml` measurements 작업에서 실행한다(기존 Playwright/Chromium 설치 뒤 단계 하나, 실행 기록 `tmp/workspace-ui-ci/admin-member-roles-dom`은 workspace DOM artifact로 올라간다). 이 단계의 hosted 실행 결과는 아직 없다.

### S5-U1b 임상의 읽기 계약 (REQ-S5-U1b-CLINICIAN-READ → RISK-S5-U1b-DRAFT-LEAK/NONFINAL-BODY/WRITER-FIELD/COUNT-LEAK/TENANT-UID)
- clinician-only 허용 업무 행은 5개다: `GET authz/dicom`·`POST dicom/lookup`(뷰어 읽기 쌍), `GET studies/:uid/viewer-items`(clinician-only는 확정본일 때만 좁힌 모양, 그 전에는 `{final:false, items:null}`; 확정 쪽은 읽은 확정 판 `reportVersion`과 (검사·판·마지막 항목 id)를 서명한 `nextCursor`를 싣고, 다음 쪽은 그 판이 여전히 머리 판일 때만 이어지며 재승인·addendum·reset 뒤나 판을 확인할 수 없는 이어받기 값(판독의 경로의 항목 id, 바뀐 서명, API 재시작 전 값)은 409 `VIEWER_REPORT_CHANGED` — S5-U1b-F04; 판독의 경로의 cursor는 그대로 항목 id), `GET clinician/studies`(listStudies를 그대로 부르고 행을 좁힘, total은 볼 수 있는 검사만), `GET clinician/studies/:uid/report`(RS A이고 머리 판 action이 approve/addendum일 때만 그 판의 본문과 key image, 그 밖은 상태만 — S5-F5 기본값, D-S5-NONFINAL-VIEW 미결). 두 새 경로는 `report-preview.controller.ts`에 있다 — `pacs.controller.ts`는 S4-U5 시험이 바이트를 고정한다.
- 순수(TEST-S5-U1b-PURE): `KIN_CLINICIAN_POLICY_MODULE=api/src/clinician-policy.ts node --test tests/clinician_read_serializer_test.cjs`(Node ≥22.18 type stripping, `@nestjs/common`의 RequestMethod는 fixture enum으로 대체). 값을 주지 않으면 kin-api:ci의 `/app/dist/clinician-policy`를 읽는다. 응답 칸 집합은 `tests/clinician_policy_fixtures.json`의 `read_contract` 하나를 순수·live 시험이 함께 읽는다.
- live(TEST-S5-U1b-LIVE, hosted 합성 전용): `python scripts/run-tests.py --module tests/clinician_read_live.py --mode live --unit s5-u1b-clinician-read --timeout 900` — 8건: test_01 목록 좁힘·볼 수 있는 검사만 total, test_02 머리 판 확정→reset 뒤 상태만, test_03 뷰어 쌍·표시 항목·`/statistics`·원격판독 경계, test_04 회원 회수(회수 전 BFF 세션·Bearer를 보존해 다섯 허용 행 성공 확인 후 같은 인증정보로 재호출: 세션 401, 새 토큰 403 PENDING, 보존 Bearer는 run 소유 120초 클라이언트로 만료 전·후 결과를 `S5-U1B-REVOCATION` marker로 보고하고 즉시 회수로 세지 않음), test_05 같은 사용자 A→B→A, test_06 S5-U1b-F01 경합(kin-api 안 compiled controller/service driver로 두 관문 사이 reset→W 항목→숨김→재승인, 읽기 뒤 변경, 단순 reset → 409 `VIEWER_REPORT_CHANGED`), test_06b S5-U1b-F04 쪽 연쇄(HTTP만: 같은 판 세 쪽 통과와 쪽마다 `reportVersion`·판독문 판 일치, 쪽 사이 addendum → 409, 첫 쪽 뒤 reset(머리 판 없음) → 409, 첫 항목 숨김·둘째 항목 수정·재승인 뒤 옛 이어받기 → 409이고 수정 내용 없음, 새 판으로 바꾼 서명 위조·항목 id cursor → 409, 길이 초과·recheck 동반 → 400, 새 연쇄는 새 판, 판독의 경로는 항목 id cursor 그대로이고 서명 cursor는 400), test_07 S5-U1b-F02 경합(StudyState·Report 읽기 사이 다른 서명자 Addendum, 무페이지·페이지 → 409 `STUDY_LIST_CHANGED`). 소유 계정·합성 검사·run 소유 Keycloak 클라이언트만 쓰고 표시 항목 행은 fixture 정리 전에 자식부터 지운다.
- `tests/clinician_policy_test.py`는 U1b 허용 행·선언된 추가 경로(`allowed_additions`)·`must_stay_denied`와 test_09 소스 고정, test_10 F04 고정(이어받기 판정 → 관문 → 판 비교 → 가림 → 고정 읽기 → 재확인 → 서명 응답 순서, 프로세스 키, HMAC·상수 시간 비교, 판독의 경로 무변경)을 함께 본다(10건, routes 110 / public 4 / allowed 7 / denied 99). 순수 serializer 시험은 15건이다(F04 쪽 연쇄: 같은 판 통과, 머리 판 변경·reset 거절, 판 없는 값·위조·다른 키·다른 검사·비정규 서명 거절).
- `tests/invariants_live.py` ROUTES에 두 새 경로를 REPORT 행(`clinician-studies` collection, `clinician-report`)으로 선언해 기사 무손상·예비 판독 제3자·타 기관 admin 배터리에 넣었다. 시험 수 83은 그대로다.
- 실제 실행: c3173c3(F04 수정 전)에서 hosted 합성 스택 run 36215843994 — `tests/clinician_read_live.py` **7/7**(test_04 `S5-U1B-REVOCATION`: 회수 전 다섯 행 200, 회수 뒤 BFF 세션 401·새 토큰 403 PENDING, 보존 Bearer는 만료 전 허용·만료 뒤 401), `tests/clinician_policy_live.py` **4/4**(routes 110 / denied 99 / clinician 감사 행 0). F04 수정본의 순수 시험(10건·15건) 결과는 수정 커밋 SHA에 묶인 실행 기록에만 두며 이 문서에는 적지 않는다. 미실행: test_06b와 수정본의 live 8건·policy live 재실행, Nest build/tsc, 같은 최종 SHA의 candidate invariants·worklist, hosted CI.

### S5-U1c 전체 route 수용 매트릭스와 역할 합성 (REQ-S5-U1c-ROUTE-COMPLETENESS → RISK-S5-U1c-NEW-ROUTE-LEAK/MIXED-DOWNGRADE → TEST-S5-U1c-INVENTORY/LIVE-MATRIX)
- 매트릭스: `tests/clinician_policy_fixtures.json`의 `route_matrix`가 통합 head 9e7f698의 모든 (method, template) 110개에 S5-U6b의 1개, S5-U4a의 6개, S5-U4c의 5개와 S5-U5b의 1개를 더한 123개를 한 행씩 가진다 — public 4 + session 2 + business 16 + denied 101. denied는 빼기로 만들지 않고 근거 묶음마다 이름을 적는다(report 17·viewer 14·operations 9·workspace 32·admin 11·consultation 5·connect 9·gateway 4, 뜻은 `basis_legend`). 104행 기준선 이후 추가된 19개는 `post_baseline`에 결정·단위·커밋·근거와 함께 따로 적는다: `GET clinician/studies`·`GET clinician/studies/:uid/report`(business, S5-U1b), `POST studies/:uid/dictation`(denied, S3-ASR 판독문 작성 경로), `POST gateway/receipt`·`GET gateway/retry-requests`(denied, Gateway client-credentials 전용), `POST studies/:uid/gateway-retry`(denied, 기사의 Now Retry), `GET admin/metrics`(denied, S5-U6b 운영 지표, need admin), 질문 route 6개 `GET questions`·`GET questions/:id`·`GET studies/:uid/questions`·`POST studies/:uid/questions`·`POST questions/:id/entries`·`POST questions/:id/close`(business, S5-U4a), 영상 요청 route 5개 `GET image-requests`·`GET image-requests/:id`·`GET studies/:uid/image-requests`·`POST studies/:uid/image-requests`·`POST image-requests/:id`(business, S5-U4c), `GET admin/audit`(denied, S5-U5b 감사·보안 기록, need admin).
- 순수(TEST-S5-U1c-INVENTORY, `python -B tests/clinician_policy_test.py`, 23건): test_05는 컨트롤러 − 매트릭스 = ∅, 매트릭스 − 컨트롤러 = ∅, 두 곳에 있는 행 없음, 컨트롤러 = `invariants_live.py` ROUTES(양방향), 개수(`route_matrix.counts`, S5-U6b `GET admin/metrics`·S5-U4a 질문 6행·S5-U4c 요청 5행·S5-U5b `GET admin/audit` 뒤 123/4/2/16/101), 기준선 이후 경로마다 정확히 한 `post_baseline` 행을 단언한다. 새 route는 매트릭스 행(session/business면 live 사례까지)을 붙이기 전에는 이 시험과 live test_01이 실패한다. test_13은 승인 토큰 × 모든 route 모델에서 legacy 역할이 하나라도 있으면 clinician 게이트를 만나지 않고 clinician-only는 denied 행을 하나도 지나지 않음을, 소스에서 `clinicianOnly(` 호출이 guard·`viewer.controller.ts`·`report-preview.controller.ts`·`pacs.service.ts`(`/statistics`) 네 곳뿐이고 역할 존재 검사(`includes('clinician')`)가 없음을 고정한다. test_14는 live 매트릭스 데이터(allow 행마다 positive·wrong_role·wrong_tenant, 설계상 비거절은 pending/invalid logout 예외 하나)와 live 모듈 배선을 본다.
- 쓰기 allow 행(S5-U4a 후속, 지휘 결정 D36): handler가 201로 답하는 allow 행(`@HttpCode` 없는 POST, test_14가 `handler_statuses()`로 컨트롤러 소스에서 읽는다)은 반드시 쓰기 행이고, 그 밖의 행은 쓰기 행일 수 없다. 쓰기 행의 `write`는 `method`(POST)·`body`(합성 값; `requestId`=`$request_id`·`expectedOwner`=`$owner`, `:id` 경로면 `revision`=`$revision` — 실행 시 `fill`의 규칙으로 채운다)·`expected_status`(201 = positive status)·`audit`(`action`은 `source`가 가리키는 제품 상수 값, `rows` ≥ 1, `actor` = positive 신원 clinician, `target` `:uid`, `detail`의 `requestId`는 이 요청의 값)을 가진다. 쓰기 행의 wrong_role은 403·wrong_tenant는 404 거절이고 설계상 예외가 없다. 질문 route 여섯 행의 wrong_role은 같은 기관 doctor/tech에 `QUESTION_ROLE_REQUIRED`, wrong_tenant는 kin-center clinician이다(`GET questions?view=mine`은 목록이라 `absent`). 읽기 행은 기존 계약 그대로다(positive status = handler 선언 status, 200/204, 요청 본문은 `dicom/lookup`뿐, 감사 행 0). live test_05는 사례마다 호출 전후 `AuditLog`를 대조한다 — 쓰기 positive는 선언한 행만(actor·action·target·detail), 읽기와 모든 거절은 clinician 행도 그 검사의 쓰기 action 행도 0. `:id` 질문은 루프 전에 clinician이 `clinician_question_live.ask_question`으로 만들고, close 행은 그 질문을 이름으로 쓰는 다른 행 뒤에 돈다. 질문 행은 클래스 정리에서 같은 모듈의 `drop_study_questions`로 검사 정리 전에 지운다(두 FK RESTRICT). `member_owner`·`read_question_row`·`lit`도 그 모듈 하나에 두고 두 live 모듈이 같이 쓴다(test_14가 정의 1회·위임을 고정). S5-U4c: 요청 route 다섯 행의 wrong_role은 같은 기관에서 거절되는 역할이 있는 세 행(`GET image-requests?view=mine&state=all`·등록·취소)에 doctor/tech의 `IMAGE_REQUEST_ROLE_REQUIRED`, 모든 앱 역할이 읽는 두 행(`GET image-requests/:id`·`GET studies/:uid/image-requests`, OQ-5 a)에 gateway 신원이고, wrong_tenant는 kin-center clinician이다. 쓰기 body 값은 자리표시·`''`·SYNTHETIC 문장 말고는 `live_matrix.literals`에 적은 열거값(요청 종류, 취소 동작, 상대 기관 id의 명시 null)뿐이며, 문자열 값은 그 행 감사 action을 정의한 소스 파일에 적혀 있어야 한다. image-requests 행의 `:id`는 루프 전에 clinician이 `clinician_request_live.make_image_request`로 만든 image-transfer 요청이고(`$revision`도 그 요청의 revision), 등록 행은 다른 종류(external-image)라 활성 요청 규칙과 만나지 않으며, 취소 행은 그 요청을 이름으로 쓰는 다른 행 뒤에 돈다. 요청 행은 클래스 정리에서 `drop_study_image_requests`로 검사 정리 전에 지운다(모든 FK RESTRICT). `read_request_row`와 함께 그 모듈 하나에 둔다(test_14가 고정).
- S5-U1a 검토 노트 종결: D3 — `invariants_live.py`의 `kc_user_summary`가 로컬 세 역할 대신 `policy_app_roles()`(`clinician-policy.ts`의 APP_ROLES를 읽고 형태가 바뀌면 거절)를 쓴다. test_15가 그 함수를 실제로 실행하고 사본으로 거절·새 역할 도달을 대조한다(ROUTES 표와 invariants 83건은 그대로). D5 — 인벤토리가 데코레이터 run 단위로 읽어 `@Public()`을 자기 handler에 붙이고, `@Public()`은 route 데코레이터 바로 위만 허용한다(test_11: 합성 소스 6종 거절, 이전 판독이 아래쪽 `@Public()`을 다음 route에 붙이던 대조). D6 — 컨트롤러의 모든 데코레이터 이름은 읽는 route 데코레이터이거나 `controller_decorators.non_route`여야 하고(WebDAV·`@RequestMapping`·사용자 route 래퍼는 실패), `*.controller.ts` 밖에 controller/route가 없으며, 모델이 모르는 method 번호는 None(거절)이고 allowlist는 GET/POST만 쓰며, `api/package-lock.json`의 `@nestjs/common` 10.4.22가 `request_method_pin`과 같다(test_12). 이 버전 RequestMethod의 전체 멤버는 이 단위에서 읽지 못했다(설치본 없음) — 버전이 바뀌면 enum과 pin을 함께 고친다. D8 — 컨트롤러·ROUTES·매트릭스 세 곳을 양방향으로 대조하고, invariants_live의 데코레이터 표·파일 glob이 순수 파서와 같고 ROUTES 키가 중복되지 않음을 고정한다. 결합은 의도한 병합 시점 조정점으로 남는다: route를 더하는 커밋은 컨트롤러·ROUTES·`route_matrix`(allow면 `live_matrix`)를 함께 바꾼다.
- 소스 계약(S5-U1c-F06, test_21): 인벤토리의 lexer·loader 검사는 닫힌 계약이다. `tests/clinician_policy_fixtures.json`의 `source_contract`가 지원 형태를 적고, 그 밖에 로더·코드 평가·메타데이터 기록·클래스 prototype에 닿는 형태는 모두 거절한다(fail-closed). 지원 형태는 넷이다: (1) `@Name(...)` 데코레이터 직접 적용 — Name은 `decorator_bindings.modules`의 모듈에서 같은 이름 `import { Name }`으로 한 번 묶인다. (2) 평문 문자열 모듈의 ES import/export 문 — api/src 아래 파일 또는 `packages`(10개)의 패키지만, `import 'm'`도 대상이며 프로젝트 파일·`@nestjs/*`의 namespace/default/`import =`/`export *`는 금지. (3) `require('<패키지>')`·`import('<패키지>')` — 맨 이름 호출에 이스케이프 없는 `'...'`/`"..."` 문자열 리터럴 하나만, `loaded_packages`(express, node:crypto)만 받는다(`import x = require()`도 같다). (4) `require` 메서드는 클래스 본문의 선언과 `this.require(...)`·`this.<field>.require(...)` 호출만. `[`·`]` 사이에 홀로 선 문자열·치환 없는 템플릿은 그 이름으로 읽는다(`module['require']`는 `module.require`, `common['Put']`는 `common.Put`). `sealed_words`(eval·Function·Proxy·module·global·globalThis·`__proto__`·defineProperty·defineProperties·setPrototypeOf)는 어디서도 쓰지 않고, 이 이름과 `sealed_strings`를 값으로 가진 문자열은 배열 리터럴 원소로만 둔다. Object·Array는 멤버·타입 인자·`new`로만(선언 키워드 뒤 금지), process는 `.env`·`.exit`, Reflect는 `.ownKeys`, prototype은 `Object.`·`Array.` 뒤, getPrototypeOf는 `===`/`!==` 비교, constructor는 `return` 없는 선언으로만 쓰고, `*.controller.ts`의 class에는 `extends`가 없다(선언부는 아래 `class_heading`이 본문 `{`까지 읽는다). 거절 회귀 `refused`(62건)·허용 대조 `accepted`(10건)·검토자의 로더 네 형태 `f06_loaders`(등록 컨트롤러와 AppModule에 등록한 외부 .ts 양쪽, 로더 단독)를 fixture로 고정하고, test_21이 각 사유 조각을 한 검사 메시지 안에서 대조한다. 인벤토리는 모든 검사를 실행하고 거절 사유를 모두 잇는다. 계약 밖으로 주장하지 않는 것(`outside_the_contract`): 실행 중 계산된 키나 열거로 닿는 속성(`x[key]`, `Reflect.ownKeys` 결과의 인덱스 등; 어떤 값이든 constructor 체인으로 Function에 닿는다 — 실제 등록 route를 매트릭스와 비교하는 live 검사가 닫을 수 있으나 이 단위 범위 밖)과 컨트롤러 route가 아닌 처리기(main.ts `app.use`, `consumer.apply` 미들웨어, `setGlobalPrefix`·RouterModule·버전 경로 재작성).
- 클래스 선언부(S5-U1c-F08, test_23): `controller_heritage`가 `class` 뒤 첫 `{`를 본문으로 잡아, 제네릭 기본값의 타입 리터럴(`<T = {}>`, `<T = { marker: string }>`, `<T = () => { marker: string }>`) 뒤의 `extends PacsController`를 읽지 못했다 — AppModule에 등록한 새 컨트롤러가 PacsController의 public `health`를 `GET unlisted/health`로 물려받는 형태인데도 인벤토리는 110행·public 4였다(Nest 기동·HTTP로는 확인하지 않았다). 이제 `class_heading`이 모든 class 키워드의 선언부를 `class`·이름(식·default export면 없음)·타입 매개변수·`extends`/`implements` 절(순서 무관)·본문 `{` 순서로 읽고, 그 밖의 형태는 사유와 함께 거절한다. 타입 매개변수·인자는 `<`·`>`를 세어 닫는 `>`까지 읽고 `(`·`[`·`{`는 짝까지 통째로 건너며 `=>`는 화살표다. 그 사이에는 이름·숫자·`, . = | & ? : -`만 온다. `.`·`?.`·`#` 뒤의 class는 속성 이름이다. `*.controller.ts`에서는 세 경우를 각각 다른 사유로 거절한다: 상속절의 `extends`, 선언부의 다른 `extends`(타입 매개변수 제약·조건부 타입), 본문까지 읽지 못하는 class 키워드(닫히지 않는 `<`, 타입에 없는 토큰, 객체 키·멤버 이름 class). require 메서드 판정(`class_body`)도 같은 선언부 판독을 쓴다. 본문 `{`는 읽은 선언부의 본문일 때만 인정한다. 이전 판독은 `if (x. class) {` 블록을 클래스 본문으로 보아, 그 안에서 블록이 뒤따르는 `require('../outside')` 호출을 메서드 선언으로 넘겼다. 또 `class Access<T = { marker: string }> {`의 require 메서드를 로더 호출로 거절했다. `source_contract.class_heading`이 지원 형태, 검토자의 세 형태(`forms`), 제네릭 없는 대조(`control`), 거절 9건, 허용 3건을 고정한다. test_23은 세 형태와 대조를 등록 컨트롤러로 넣어 인벤토리와 test_05가 모두 상속 사유로 멈추는지, 이전 판독이 세 형태의 `extends`를 놓쳤는지 확인한다. api/src의 모든 class 키워드가 읽히고 컨트롤러 파일에는 상속이 없는지도 본다. 실제 인벤토리 123 = 4 + 2 + 16 + 101은 그대로다(S5-U6b: `GET admin/metrics` 1행, S5-U4a: 질문 6행, S5-U4c: 요청 5행, S5-U5b: `GET admin/audit` 1행으로 110 → 123). `invariants_live.controller_routes`는 상속 메서드를 열거하지 않으므로, 이런 상속은 이 순수 관문에서 막는다.
- live(TEST-S5-U1c-LIVE-MATRIX, hosted 합성 전용): `python scripts/run-tests.py --module tests/clinician_policy_live.py --mode live --unit s5-u1c-clinician-policy --timeout 900` — 5건. test_01은 public·allow 밖 모든 컨트롤러 경로를 clinician-only로 부른 뒤 부른 목록이 매트릭스 denied 101행과 정확히 같은지 본다(`CLINICIAN_LIVE_SWEEP` routes·denied는 `route_matrix.counts`에서 읽는다: 123, 101). test_04는 합성 hallym 검사 하나로 혼합 사용자(clinician+radiologist, clinician+technician)가 `GET studies`·legacy `viewer-items` 응답(`{items, nextCursor}`)·`report-preview`(`canPreviewEditor`는 legacy 역할이 정함)를 그대로 받고, clinician-only는 같은 검사에서 `GET studies`·`report-preview`가 403 `CLINICIAN_ROUTE_DENIED`이고 `viewer-items`가 좁은 모양이며, kin-center clinician+admin은 기관 규칙의 403임을 본다(`CLINICIAN_LIVE_COMPOSITION`). test_05는 `live_matrix` 54건(18행 × positive/wrong_role/wrong_tenant, 각 1회 호출; wrong_role은 need 행에 같은 기관 radiologist, 질문 행에 같은 기관 radiologist/technician, 요청 행에 같은 기관 radiologist/technician 또는 gateway, 공유 행에 gateway 신원; wrong_tenant는 kin-center clinician, 세션 행은 두 기관 그룹 clinician)을 실행하고 사례마다 감사 행을 대조한다(`CLINICIAN_LIVE_MATRIX`, 쓰기 5행). 검사는 RS W로 두고 판독·항목·설정을 쓰지 않는다. clinician이 쓰는 SYNTHETIC 질문 두 개(`:id` 질문·create 행)와 SYNTHETIC 영상 요청 두 개(`:id` 요청·create 행), 그 영수증·감사 행은 요청·질문 정리 뒤 클래스 정리가 검사와 함께 지운다.
- 미실행: live 5건 전부(hosted 전용), Nest build, 같은 SHA의 candidate invariants·worklist. 순수 결과는 커밋 SHA에 묶인 실행 기록에만 둔다.

### S5-U2a 임상의 홈 (REQ-S5-U2a-CLINICIAN-HOME → RISK-S5-U2a-STALE-A-B-A/HIDE-AS-PERMISSION/STATE-CONFUSION → TEST-S5-U2a-DOM)
- 착지: `KinAuth.home(session)`(`worklist-v0/hpacs-lite/auth.js`)은 승인된 비데모 세션의 역할에 `clinician`이 있고 `radiologist`·`technician`·`admin`이 하나도 없을 때만 `clinician.html`, 그 밖(혼합·판독의·기사·관리자·승인 대기·무효·데모)은 예전처럼 `main.html`이다 — guard의 clinician-only 게이트와 같은 규칙이며 Keycloak 기본 역할은 보지 않는다. `index.html`이 이 값으로 옮긴다. OIDC 콜백은 모든 로그인을 `main.html`로 보내므로(`api/src/auth.controller.ts`), `main.html`에서 `KinAuth.init()`이 clinician-only 세션을 `clinician.html`로 옮기고 끝나지 않아 `main.html`의 boot가 이어지지 않는다. 페이지 선택일 뿐 권한이 아니다.
- 화면(`clinician.html` + `clinician.js`): S5-U1b 두 읽기의 칸만 그린다. 목록은 `GET clinician/studies?limit=100`에서 시작해 `pagination.next`를 그대로 `after`로 넘겨 끝까지 읽고(쪽마다 limit·offset·total·next·UID 순서 확인, 중간 실패면 받은 쪽도 버림), `GET me`로 같은 계정인지 다시 확인한 뒤 검사일 최신순으로 보인다. 행: 이름·ID·성별/검사 시 나이(생년월일과 검사일로만 계산)·생년월일·검사일·장비·설명·기관(원격판독이면 Tele)·판독 상태. 행을 고르면 식별 줄(위 항목과 Accession·Series/Images) 다음에 `GET clinician/studies/:uid/report`의 `report.final`이 참일 때만 판 번호·Approved/Addendum·서명자·서명일·본문 세 칸·key image 목록, 거짓이면 상태(Awaiting Report/In Progress/Preliminary/On Hold/Unknown)만 보인다. 응답의 다른 칸은 읽지 않는다. 거절(403/404/409)과 형식 오류는 서버 문구·HTTP 상태·코드 그대로 보이고 Retry가 있다. 목록 상태 loading/empty/failed는 문구와 `data-state`가 모두 다르다. 판독문 응답은 요청 번호와 UID를 함께 확인한 뒤에만 그린다(A→B→A), 목록도 요청 번호로 늦은 답을 버린다. 역할 목록을 읽지 않으며 브라우저 대화상자·`innerHTML`이 없다. 뷰어 열기는 S5-U2b 범위라 `Open Viewer`는 비활성 안내 자리만 있고 key image는 픽셀 없이 목록(제목·설명·Series/Instance/Frame)으로만 보인다.
- 시험(TEST-S5-U2a-DOM): `python -B tests/clinician_home_dom_test.py` — 12건. 합성 origin에서 배포 파일(`clinician.html`·`clinician.js`·`index.html`·`auth.js`)을 그대로 싣고 `/api/me`·임상의 목록(서명 cursor 모양 쪽 나눔)·판독 읽기를 시험 안 저장소로 답한다. 01 착지(index.html 7역할 조합과 승인 대기, main.html의 clinician-only 넘김과 혼합 세션 boot, 근거 세 줄 고정), 02 식별자·확정본·key image·심은 작성자 칸 미표시·마크업 무실행, 03 확정 전 상태만(본문을 실어도 미표시)·형식 오류·다른 검사 답·404·403·Retry, 04 A→B→A, 05 대조군(UID만 보는 판·확인 없는 판은 늦은 답을 그려야 한다), 06 목록 세 상태·거절 표시·늦은 목록과 그 대조군, 07 세 쪽 cursor 전달·진행 표시·형식 오류·계정 바뀜, 08 영어 컨트롤/한국어 설명·피하는 말·수신 확인 문구 없음·대화상자 없음·12px·24px·키보드, 09 로그아웃·다른 탭 종료·승인 대기/무효·세션 없음, 10 로그아웃 POST를 붙잡은 채 Log out·401(목록·판독)·두 번째 401·다른 탭 종료·BroadcastChannel 없음에서 화면 즉시 비움과 늦은 판독 답 무시, POST·이동 각 1회와 대조군, 11 목록 재확인 중과 403/409 실패 뒤 선택 검사 내림(늦은 판독 답 무시), Retry 성공 때 목록→계정→판독 순 재조회로만 복구와 대조군. CI는 measurements의 S5-U5a DOM 단계 다음 단계가 같은 인터프리터로 돌린다.
- 미실행·한계: hosted CI, 실제 스택의 착지(`tests/e2e/test_report_preview.py` 회귀는 hosted 전용), key image 픽셀·뷰어(S5-U2b), 외부 AI 결과 표시(UXR-SP-33/S5-14, S6 이후), 중요 결과 강조(UXR-S5-15 — 이 화면에는 강조가 없고 전달·수신 확인 문구가 없음만 확인한다).

### S5-U2b 임상의 읽기 전용 뷰어·비교 (REQ-S5-U2b-READONLY-VIEWER → RISK-S5-U2b-WRITE-CONTROL/WRONG-PRIOR → TEST-S5-U2b-DOM)
- 홈(`clinician.js`): 검사를 고르면 `Open Viewer`가 켜지고 고정 OHIF(`/ohif/viewer?StudyInstanceUIDs=<uid>`)를 이름 하나(`kin-clinician-viewer`)의 창에서 연다(opener를 끊는다. 팝업 차단과 뷰어 주소에 실을 수 없는 UID는 문구로 알리고 열지 않는다). 비교 후보는 지금 목록(서버가 허용한 검사)에서 서버 `sourcePatientKey`(기관|원본 DICOM PatientID)가 같은 다른 검사뿐이다 — 이름·생년월일·기사가 화면에서 고친 환자 ID·다른 기관의 같은 원본 ID로 묶지 않는다. 후보가 있을 때만 `Comparison` 칸을 만들고(없거나 키가 없으면 `#viewer-note` 한 줄), `Compare`는 현재 검사와 후보를 `hangingProtocolId=@ohif/hpCompare`로 나란히 연다. 누르는 순간의 선택과 목록으로 키를 다시 확인하므로 목록이 바뀐 뒤 남은 버튼이 다른 환자 쌍을 열지 않는다. 새 API 요청은 없다. U2a 문단의 "`Open Viewer`는 비활성 안내 자리" 문장은 이 단위로 바뀌었다.
- 뷰어(`config/ohif.js`, 즉시 검토 경로): 계정 판정은 뷰어 문서 하나(`kinViewerSession`)가 가진다. 측정 패널·배치 패널·쓰기 모듈 관문(`decide()`, Findings·Job·Tech Note가 `/me` 읽기 하나를 나눔)이 받은 `/me`는 재확인·주기 확인까지 모두 `note()`로 적는다. 역할은 unconfirmed(성공한 답 없음 — 오류·5xx·형식 오류·시간 초과는 허가도 거절도 아님), writer(clinician 밖의 업무 역할이 있음), read-only(업무 역할이 clinician뿐, `kinViewerClinicianOnly` = `api/src/clinician-policy.ts` clinicianOnly)이고 read-only는 writer로 돌아가지 않는다. owner는 문서가 처음 확인한 계정(기관·sub)이며 모드 종료·재진입이 지우지 않는다. `note()`는 역할을 반영하기 전에 답의 계정을 owner와 비교해, 어느 패널이 언제 붙었든 다른 계정이면 곧바로 종료한다(Astra S5-U2b-X4-R-001 F01). 종료(ended, `state()`는 `refused`)는 401·`/me`의 403·로그아웃 방송(storage `kin-session-ended`, BroadcastChannel `kin-session`)·다른 계정·구성원이 아닌 답이며, 역할과 따로 두어 read-only보다 앞서고 문서가 끝날 때까지 남는다(F02). 끝난 문서는 두 패널이 "다시 로그인한 뒤 뷰어를 여세요"로 끝나고, 재진입한 패널과 나중에 붙은 모듈은 종료 상태로 시작해 `/me`·목록을 묻지 않으며, 측정 패널의 요청 관문과 관찰 주기도 종료를 먼저 본다. 작성 경로는 writer로 확인된 뒤에만 열린다. 그 밖의 상태에서는 도구막대의 Measurements 묶음과 More Tools의 작성 항목을 없애고, 도구막대·명령·단축키·도구 그룹(나중에 만든 그룹 포함)·`addNewAnnotation` 직접 호출로 작성 도구를 켜거나 그리지 못하게 하며(영상 조작 도구만 Active), 표식 메뉴·이름·측정 수정·화살표 글·측정 패널의 이름 바꾸기·잠금과 SR 명령을 거절하고, 이 패널이 저장판에서 그리지 않은 표식은 다음 관찰에서 지운다. writer 답이 오면 모드가 정한 도구막대·도구 모드를 되돌린다. Findings·Job·Tech Note는 writer 답에만 붙고, clinician-only가 되는 순간 내려간다. 세 모듈은 문서 세션(`kinViewerSession.writeModule`)을 받는다(Astra S5-U2b-X5-R-001 F01): 자기 `/me`(첫 확인·재확인·주기 확인)는 계정·컨트롤에 반영하기 전에 `answer()`가 owner와 비교해 판정을 적고, 401과 `/me`의 403은 `refuse()`로 문서를 끝낸다(모듈 자원의 403 — Job 목록·작업 읽기, Findings 목록 — 은 이전처럼 그 모듈만 끝내거나 보관한다). 문서가 끝나면 그 요청의 다음 조회·쓰기는 나가지 않는다. 로그인이 끝나면(어느 경로든) 세 모듈은 내려가지 않고 `onEnd()`로 등록한 제자리 종료를 하며 각자 자리에 안내를 남긴다 — Job `#kin-viewer-jobs-status` "세션이 변경되었습니다. 다시 로그인한 뒤 뷰어를 여세요.", Findings "로그인이 종료되었습니다. 다시 로그인한 뒤 뷰어를 여세요.", Tech Note "세션이나 영상창이 변경되었습니다. 뷰어를 새로 여세요."(모드 종료가 내린다). 세션은 이 제자리 종료를 종료 판정을 알리기 전에 돌리므로 MPR 도구의 정리(도구 그룹 되돌리기)가 끝난 뒤 작성 경로 닫기가 적용된다. fix5~fix7은 종료 때 모듈을 내려 안내가 사라졌다(PR #92 hosted e2e `test_volume_mip_job`·`_batch`·`_output`, `test_finding_navigation`). 로그아웃 방송(storage·BroadcastChannel)은 문서 수명의 수신자가 받아, 모든 확장이 모드 종료로 내려간 사이의 로그아웃도 종료로 남고 재진입은 아무것도 묻거나 열지 않는다(F02). clinician-only 문서의 Measurements & Key Images는 `GET studies/:uid/viewer-items?limit=100`과 받은 서명 cursor 그대로만 읽고(includeHidden·recheck 없음, 서버는 숨긴 항목 요청을 400으로 거절), 모든 쪽의 uid·`reportVersion`이 같을 때만 그린다(다르면 받은 쪽도 버림). 확정 전은 withheld, 항목 없음은 empty, 404/409/400은 서버 문구·HTTP 상태·코드로 failed, 403은 denied(`Recheck Access`는 다시 읽기)이고 상태는 `#kin-viewer-history[data-read-only]`에 남는다. 행은 `Read-only`와 `Go to Image`뿐이며 Length·Angle·Ellipse ROI·Add Key Image·Download SR·Store SR·Edit·Save·Hide·History·Recheck Source·Remeasure를 두지 않는다(저장된 측정은 잠근 채 그린다). 확정 판은 원본 프레임 식별과 상관없이 포커스·15초마다 다시 확인해 final:false면 곧바로 내리고, 판이 바뀌면 새 판 전체를 다시 검증한다. writer에서 clinician-only로 바뀌는 순간 진행 중인 작성자 읽기·쓰기는 버리고(늦은 작성자 목록을 그리지 않음), 보관한 writer 작업은 확정 목록·표식·Go to Image를 막지 않는다(보관은 그대로). Recent Layout·Hanging Protocol 대신 `Viewer Status` 상태 줄만 남긴다. 컨트롤을 두지 않는 것은 화면일 뿐 권한이 아니다 — 쓰기 거절은 S5-U1a/U1b 서버가 한다.
- 시험(TEST-S5-U2b-DOM): `python -B tests/clinician_viewer_dom_test.py` — 24건. 홈은 배포 파일(`clinician.html`·`clinician.js`·`auth.js`)을 합성 origin에서 그대로 싣고, 뷰어는 `config/ohif.js` 전체를 cornerstone·도구 그룹·도구막대·명령·grid·서비스 스텁 페이지(고정 longitudinal 모드의 도구 그룹·도구막대를 본뜸)에 싣는다(Findings·Job·Tech Note·Hanging Protocol은 마운트·정지와 쓰기 모듈의 제자리 종료를 기록하는 스텁이고, 22는 실제 모듈을 싣는다). 관찰 창은 고정 900 ms 대기 대신 측정 패널의 관찰 주기(250 ms `scan()`)를 하네스가 즉시 돌리고(`synTick`, 15초 시계 `synAdvance`와 같은 가짜 타이머) 그 요청과 답이 이어 부른 요청이 끝날 때까지 본다. 01 Open Viewer·Compare의 창 이름·opener·주소, 팝업 차단·UID 형식, 02 같은 서버 환자 키만 후보(같은 이름·화면 ID가 같아진 다른 환자·다른 기관 같은 원본 ID·키 없음 제외, 화면에서 고친 같은 키 포함), 03 A→B→A와 목록 새로고침 뒤 남은 Compare 버튼, 04 영어 컨트롤/한국어 설명·피하는 말·12px·24px·Enter, 05 clinician-only 뷰어의 읽기 쿼리·Read-only 행·저장 측정 잠금 표시·쓰기 컨트롤 없음·도구와 SR 거절·모듈 미마운트, 05b 판독의·혼합 세션의 쓰기 패널과 모든 모듈, `/me` 무응답에서 컨트롤·모듈 없음, 06 loading/empty/withheld/failed(404·409·400·다른 검사 답·다른 판 쪽)/denied와 Refresh, 07 비교 검사를 사이에 둔 A→B→A, 08 뷰어 자체 작성 경로 차단, 09 쓰기 모듈 관문(오류는 결정을 미룸, clinician-only는 재진입에도 유지), 10 확정 판의 주기·포커스 확인, 11 writer 확인 전 작성 차단과 writer의 이후 403, 12 원본 프레임 없는 확인, 13 VIEWER_STATE_MATRIX(34행 × 9상태: unconfirmed·refused·read-only·writer·read-only+held·refused+late-writer·refused+layout-account·ended+reenter·read-only+ended), 14·15 clinician-only 전환과 진행 중 요청(SESSION_CHANGE_MATRIX), 16 보관 작업과 확정 목록, 17 거절·세션 종료 유지, 18 배치 패널이 먼저 본 다른 계정, 19 문서 owner(모든 확장이 확인한 writer 문서의 재진입, 측정 패널만 확인한 문서에 나중에 붙은 배치 패널·모듈 관문의 첫 `/me`가 다른 writer·clinician-only 계정이면 역할 반영 전에 종료, 늦은 첫 계정 답·뒤에 붙은 확장·재진입은 아무것도 묻거나 열지 않음), 20 read-only 문서의 401·403·로그아웃·다른 계정 종료가 공유 상태와 두 패널에 남고 `/me`가 다시 성공해도 재진입·포커스·15초가 `/me`·viewer-items를 묻지 않음, 21 같은 계정의 재진입(writer 목록·모듈·계정 컨트롤·작성, 확정 목록 다시 읽기, 보관 작업 뒤 clinician-only), 22 실제 Job·Findings(`finding-link-model.js` 포함)·Tech Note 모듈(Astra S5-U2b-X5-R-001 F01, 한 번에 하나, 나머지는 스텁, Tech Note의 편집기·도크·창 연결·단축키·식별 표시는 스텁): (a) 모듈 관문과 측정 패널이 첫 계정을 확인한 뒤 모듈 자신의 첫 `/me`가 다른 writer·clinician-only 계정 → 곧바로 종료, 모듈은 제자리에서 자기 안내와 함께 끝나고 켜진 컨트롤 없음, 두 패널 종료, 표식·편집·SR·계정 컨트롤 없음, 늦은 첫 계정 답과 재진입은 아무것도 묻거나 열지 않음, (b) 같은 계정으로 붙은 모듈의 재확인(Job 15초 확인·Reload Findings·메모 요청) `/me`만 401·403·다른 계정이고 다른 패널의 `/me`는 보류 → 같은 종료, 그 요청의 다음 읽기·메모 요청은 나가지 않음, (c) 같은 계정의 재확인·재진입은 모듈이 계속 동작, 모듈 목록의 403은 문서 종료가 아님, 23 모든 확장이 모드 종료로 내려간 사이의 storage 로그아웃·BroadcastChannel 로그아웃(각각)(F02) → writer·clinician-only 문서 모두 같은 계정이 다시 답해도 재진입·포커스·15초가 `/me`·목록을 묻지 않고 모듈·표식·계정 컨트롤 없음, 로그아웃 없는 재진입은 정상. 대조군(같은 시험 안의 변형 파일): 이름·표시 ID로 묶는 홈과 누를 때 키를 다시 보지 않는 홈, 읽기 전용 도구막대·모듈 관문·판 번호 고정·세대 검사·확정 판 확인을 뺀 뷰어, 그리고 각 지적 시점의 파일(R-001·R-002·R-003·X-R-001·X2-R-001·X3-R-001·fix6, 그리고 fix7 — 문서 세션을 받지 않는 모듈은 다른 계정을 받아 쓰고 재확인 403에 자기만 끝나며, 문서 수명 로그아웃 수신이 없으면 모드 종료 사이의 로그아웃을 놓침)이 해당 결함을 재현한다. 11·17·18의 "쓰기 모듈이 내려감" 기대는 X5-R-001 F01로 "제자리 종료(컨트롤 비활성, 정지 없음)"로 바뀌었다. `tests/clinician_home_dom_test.py` test_02는 이 단위에서 선택 전 비활성·선택 후 `Open Viewer`가 뷰어 주소를 여는 것으로 바뀌었다. `tests/viewer_hanging_protocol_mount_dom_test.py`의 로그아웃 뒤 재마운트 기대는 X4-R-001 F02로 종료 유지(편집기 없음·추가 `/me` 없음)로 바뀌었고, 지연된 계정 변경 확인은 새 뷰어 문서에서 한다. 순수 시험: `tests/finding_link_model_test.cjs`(배포 `kinViewerSession`을 `config/ohif.js`에서 잘라 Findings 저장소에 연결 — 다른 계정·401·`/me`의 403·목록 401은 문서 종료, 목록 403은 보관만, 종료는 붙은 저장소를 제자리에서 끝내고 진행 중인 다음 읽기를 막음, 정리한 저장소에는 닿지 않음)와 `tests/viewer_note_connection_test.cjs`(Tech Note 관문이 문서 세션을 넘기고, 종료는 모드 종료까지 브리지를 두고 clinician-only는 내림). CI는 measurements의 S5-U2a DOM 단계 다음 세 단계가 같은 인터프리터로 나눠 돌린다(`-k`: 01~13, 14~18, 19 이후 — `test_2`가 뒤에 추가될 시험도 받는다). 한 파일 전체는 로컬 약 190초로 3분 단계 시간을 넘으므로(0707628의 hosted 단계가 test_20 도중 193초에 중단된 원인은 누적 시간이며, test_20에 실제 15초 대기는 없었다) 시간 한도를 올리지 않고 나눴다. 각 단계는 로컬 약 1분이다.
- 미실행·한계: hosted CI, 실제 OHIF·Orthanc 스택(`tests/e2e/test_prior_selection.py`·`tests/e2e/test_related_scope.py` 회귀는 hosted 전용)과 nginx `authz/dicom`·`dicom/lookup` 경로, 기존 뷰어 e2e. 뷰어 시험은 고정 OHIF를 본뜬 스텁 페이지이며, 실제 Findings·Job·Tech Note는 22에서만 계정 변경·세션 종료 경로에 싣고(Tech Note의 편집기·도크·창 연결·단축키·식별 표시는 스텁), 실제 Hanging Protocol 편집기는 싣지 않는다(각 모듈에는 자체 시험이 있다). `tests/e2e/test_volume_preferences.py` 09·14는 로그아웃을 MPR 도구 정리 수단으로 쓰는데, 로그아웃은 문서 종료이고 끝난 문서는 작성 경로를 닫으므로(X2-R-001 F01) 도구 그룹 기대가 "정리한 그룹(09: 더 새로운 네이티브 선택, 14: 원래 바인딩)에 작성 닫기를 적용한 것"(영상 조작 도구가 아닌 Active·Passive 도구만 Enabled)으로 바뀌었다 — hosted e2e 전용. 영상 조작 기능의 `/me` 확인(CT 위치 동기·Cine·3D 커서·원본 PDF)은 각자 계정을 비교하며 `kinViewerSession`에 적지 않는다 — 쓰기 경로가 아니어서 이 단위의 종료 판정 범위 밖이다. 뷰어의 `Open Source PDF`는 `GET studies`(임상의 거절)를 읽어 임상의에게는 실패 문구가 나온다.

### S5-U6a 관리자 Gateway Status (REQ-S5-U6a-GATEWAY-STATUS → RISK-S5-U6a-TENANT/FALSE-DELIVERY/OVERSIZED-CLAIM → TEST-S5-U6a-SERVER·DOM)
- 계약: 새 route가 없다. 회원 콘솔(`worklist-v0/hpacs-lite/admin.html`)의 `Gateway Status` 구역은 기존 `GET /api/studies`(쪽 나눔 없이 한 번)의 행 `gatewayReceipt`와 `notObserved[].gatewayReceipt`만 읽는다. 서버는 자기 기관 검사에만, 자기 기관 자격증명이 쓴 영수증만 싣는다(`listStudies`, S4-U3·S4-F01V). admin에게도 기관 예외가 없고 화면은 따로 거르지 않는다. 재전송은 기존 `POST /api/studies/:uid/gateway-retry`(technician-or-admin, 결정 D10)다. 관리자 전용 route를 새로 두지 않은 이유: `pacs.controller.ts`는 `tests/study_identity_source_test.py`가 바이트(sha256)를 고정하고, 같은 기관 경계가 이미 이 목록에 있다. 그래서 `invariants_live.py` ROUTES·clinician fixture·경로 수는 바뀌지 않는다.
- 화면: 목록은 `Refresh Gateway Status`를 누를 때만 읽고 `study-arrivals.js`도 그때 한 번 싣는다(회원 콘솔을 여는 요청은 여전히 `/api/me`와 회원 목록뿐). 영수증이 있는 항목만 나오고(보고 없음은 정상이며 세지 않음) 실패 → 해석 불가 → 나머지, 최신 보고 순이다. 상태는 글자와 색이 함께 다르다. `Report Received`: 읽을 수 있고 실패가 없는 보고(보고 시점 Gateway 큐의 수이며 영상 수신·전송 종료가 아니므로 단계 이름을 보이지 않음). `Transfer Failed`: `retry`는 자동 재시도 대기·시도 횟수·오류 코드와, 묶을 수 있는 키가 있으면 `Now Retry`. `failed`는 `instance_exceeds_budget`이면 "단일 DICOM 인스턴스가 Gateway 전송 상한(최대 24 MiB)을 넘어 보낼 수 없습니다"와 F-01 문장만 있고 버튼이 없다(H-2). `Retry Requested`: 이 화면이 같은 영수증 키(`epoch|agentSeq`)에 저장한 요청이며 재시도 실행이 아니다. `Unknown`: 해석할 수 없는 보고로, 실패·정상 어느 쪽으로도 세지 않는다. `Query Failed`: 목록 조회 실패(500·형식 오류·409 등, 고정 문장). 마지막 성공 목록을 같은 노드로 두고 `마지막 관측 기준 <시각>`, 첫 조회 실패면 `아직 성공한 조회가 없습니다`이며 그동안 `Now Retry`는 없다. 개수는 성공한 조회 전에는 `Counts Unknown`이고, 부재 목록을 판정하지 못한 조회에는 `Not Observed Unknown`이 붙는다(모름 ≠ 0). 목록 답은 요청 번호로, 재전송 답은 요청 토큰·같은 항목·같은 키로 확인한 뒤에만 그린다. 다른 영수증이 그려지거나 항목이 목록을 떠나면(돌아와도) 토큰을 버린다(A→B→A). 401은 본문을 읽기 전에 판정해 목록을 비우고 로그아웃은 한 번이다. 서버 문구는 보이지 않는다. 영어 컨트롤·상태, 한국어 설명·툴팁·사유, 브라우저 대화상자 없음, 12px 이상.
- 순수 규칙: `study-arrivals.js`의 `gatewayStatus`·`gatewayStatusRows`·`gatewayStatusSummary`·`GATEWAY_STATUS`. U1b `gatewayLabel`·`receiptLabels`·`RETRY_TEXT`·`BANNED`와 기존 고정 문자열은 그대로다.
- 시험: TEST-S5-U6a-SERVER — `gateway_receipt_server_test.cjs`의 `S5-U6a admin view`(kin-api:ci, 기존 runtime 단계). hallym admin과 kin-center admin(`roles: ['admin']`)이 각자 자기 기관 검사의 자기 영수증만 받는다(여섯 단계 투영 그대로, failed의 오류 코드 유지). 원격판독으로 받은 행과 다른 기관 자격증명이 쓴 영수증은 null이고, 영상 없는 자기 검사는 자기 영수증과 함께 온다. 영수증 조회는 매번 `institutionId: me`이며 쓰기가 없다. TEST-S5-U6a-DOM — `python -B tests/admin_gateway_status_dom_test.py` 17건(Playwright Chromium, 서버·네트워크·자격증명 없음, 출하 `admin.html`·`auth.js`·`study-access-admin.js`·`study-arrivals.js`를 합성 origin에서 그대로 실음): 01 부팅 요청은 `/api/me`·회원 목록뿐, 02 다섯 상태·색·재전송 규칙·빈 본문 POST·답 전 표시 없음·409/503 고정 문장, 03 Query Failed가 같은 노드의 마지막 목록을 두고 버튼을 거둠, 04 모름 ≠ 0, 05 목록 A→B→A와 대조군(순번 검사가 없는 판은 늦은 답을 그림), 06 재전송 답 A→B→A·떠났다 돌아온 항목과 대조군(토큰 검사가 없는 판은 `Retry Requested`를 그림), 07 401, 08 문구·12px·키보드·대화상자 없음, 09 순수 규칙 표. CI는 measurements의 S5-U2a DOM 단계 다음 단계가 같은 인터프리터로 돌린다. 결과는 커밋 SHA에 묶인 실행 기록에만 둔다.
- 한계·미실행: Nest build와 서버 시험은 hosted `kin-api:ci`에서만 돈다(로컬에 `/app/dist`가 없음). `Retry Requested`는 이 화면의 POST 답이다. 새로 고침·재방문 때 서버가 요청 여부를 싣지 않으므로 `Now Retry`가 다시 보이고, 같은 영수증에 다시 누르면 서버가 쓰기 없이 `already_requested`(처음 시각)를 답한다. 목록은 기관 전체 `GET /api/studies` 한 번이라 큰 기관에서는 워크리스트 전체 조회와 같은 비용이다. 실제 스택·live·hosted CI는 실행하지 않았다.

### S5-U6b 관리자 운영 지표 (REQ-S5-U6b-OPS-METRICS → RISK-S5-U6b-UNKNOWN-AS-ZERO/INVENTED-THRESHOLD/TENANT-AGGREGATE → TEST-S5-U6b-PURE·DOM)
- 계약: 새 읽기 route `GET /api/admin/metrics` 하나(`admin.controller.ts` → `PacsService.adminMetrics`). `need('admin')`·`inst()` 뒤 검사 접근 조건을 읽고, 제한된 계정은 403 `ADMIN_METRICS_RESTRICTED`(범위 밖 검사를 세어 주는 창이라 `/statistics`와 같은 이유), 읽는 동안 조건이 바뀌면 409 `STUDY_ACCESS_CHANGED`다. clinician-only는 guard 기본 거절(allowlist 밖)이며 clinician 허용 목록은 바꾸지 않았다. S5-U1c 병합 뒤 `route_matrix`에는 `GET admin/metrics`가 denied(admin) 행과 `post_baseline` 행으로 들어가 111 = 4 + 2 + 5 + 100이 되었고, S5-U4a 질문 6행과 합친 뒤에는 117 = 4 + 2 + 11 + 100, S5-U5b `GET admin/audit`까지 합친 뒤에는 118 = 4 + 2 + 11 + 101, S5-U4c 영상 요청 5행까지 합친 뒤에는 123 = 4 + 2 + 16 + 101이다. `AdminController` 접두어를 `admin`으로 올리고 회원 네 경로를 `users…` 핸들러로 옮겼다 — 접두어+핸들러를 이어 붙인 URL과 route key는 그대로다. `invariants_live.py` ROUTES에 `("GET", "admin/metrics"): Route(Kind.TENANT)` 한 행을 더했다.
- 행(`ADMIN_METRIC_KEYS` 11개, 순서 고정): 행마다 `state`(observed·empty·unobservable)·`reason`·`scope`(server·institution)·`source`·`observedAt`(원천이 답한 서버 시각)·`unit`(byte·study·second)·`value`·`max`·`denominator`·`excluded`·`window`를 싣는다. `storage.server`는 Orthanc `GET /statistics`의 `TotalDiskSize`만 읽은 서버 전체 값이다(D11). 같은 답의 서버 전체 검사·영상 수는 싣지 않는다. 전체 용량(분모)은 원천이 없어 늘 관측 불가이고 사용률을 만들지 않는다. `storage.institution`은 원천이 없어 늘 `no_source`다. 검사 행은 KIN DB `StudyState`를 `institutionId = me OR teleInstitutionId = me`(visible 경계)로 읽고 합계만 보낸다. `studies.own`·`studies.tele`·`studies.flag_unreadable`, KIN 등록 시각(`createdAt`, 촬영·수신 시각이 아님) 기준 `studies.registered_24h`·`registered_7d`. `tat.emergency`·`tat.normal`은 최근 7일 안에 첫 승인(ReportVersion `approve`의 가장 이른 `at`, 기관을 JOIN으로 건 한 번의 GROUP BY)된 검사의 등록→첫 승인 시간이고, `waiting.*`는 지금 RS W·T·P·H인 검사의 등록→지금 시간이다. 값은 중앙값(짝수 건이면 가운데 두 값 평균 내림)과 최댓값(초)이고, 분모는 잰 검사 수, `excluded`는 시각을 읽을 수 없거나 순서가 맞지 않거나 RS를 알 수 없는 검사다. 입력은 사람이 입력한 Emergency 표시(`em`)와 서버 시각뿐이고 E·N이 아닌 `em`은 `flag_unreadable`로만 센다. 임계값·색 판정은 없다(C-7). Orthanc 실패·형식 오류·5초 초과는 `source_failed`·`invalid_answer`·`timeout`, DB 실패는 DB 9행 `source_failed`이며 어느 것도 0이 아니다. 분모 0은 `empty`다.
- 화면: 회원 콘솔(`admin.html`) `Operations` 구역. `Refresh Operations`를 누를 때만 읽는다(부팅 요청은 여전히 `/api/me`와 회원 목록뿐, Gateway Status 구역과 서로 요청을 섞지 않음). 표 열은 Metric·State·Value·Unit·Denominator·Scope·Source·Observed At이고 모든 칸에 한국어 설명이 붙는다. 순수 모델 `<script id="admin-metrics-model">`(`KinAdminMetrics`)이 행 모양을 확인한다 — 값을 증명하지 못한 행(누락·문자열 숫자·음수·소수·출처 없음·시각 형식 오류·단위/범위 불일치·지어낸 용량·0분을 실은 empty 등)은 `Unobservable`이고 값·분모·시각 칸에 숫자가 없다. 분모 0은 `No Studies`(분모 칸 `0 Studies`)다. 값의 크기로 색·문구가 바뀌지 않는다. 조회 실패(500·형식 오류·403·409·503)는 고정 문장과 `Query Failed · 마지막 관측 기준 <시각>`, 마지막 표를 흐리게 둔다(첫 실패는 표 없음). 요청 번호로 늦은 답을 버리고(A→B→A), 401은 본문을 읽기 전에 판정해 구역을 비우고 로그아웃은 한 번이다. 서버 문구는 보이지 않는다.
- 시험: TEST-S5-U6b-PURE — `node --test tests/admin_metrics_test.cjs` 19건. 페이지 모델 10건(출하 블록을 vm에서 그대로 실행: 11행 표, 관측 행 문구, 모든 행×네 이유의 `Unobservable`, 분모 0, 값을 증명하지 못한 31가지 행, 읽을 수 없는 답, 단위 서식, 임계값 없음, 요약 줄, 문구)은 어디서나 돈다. 컴파일 서버 9건(`adminMetricRows`가 같은 고정 답을 정확히 만듦·다른 기관 행 무시·DB 실패, `orthancDiskBytes`, `adminMetrics`의 두 질의 기관 조건·원천 행과 서버 수 미유출, admin 외·기관 없음·접근 제한 거절과 그 전 읽기 없음, 접근 조건 변경 409, Orthanc 실패·형식 오류·시간 초과, DB 실패 로그는 코드만)은 `KIN_ADMIN_METRICS_SERVER=/app/dist/pacs.service`일 때만 돌고 그 변수가 있으면 모듈 부재가 실패다(CI runtime 단계, `kin-api:ci`에 `tests`와 `worklist-v0`를 읽기 전용으로 붙임). 변수가 없으면 skip으로 보고되며 통과가 아니다. TEST-S5-U6b-DOM — `python -B tests/admin_metrics_dom_test.py` 24건(Playwright Chromium, 출하 `admin.html`·`auth.js`·`study-access-admin.js`를 합성 origin에서 그대로 실음): 01 부팅 요청 없음, 02 관측 답의 11행×8칸 문구와 한국어 설명, 03 모름 ≠ 0, 04 Query Failed가 마지막 표를 두고 고정 문장·이동 없음, 05 A→B→A와 대조군(순번 검사가 없는 판은 늦은 답을 그림), 06 401, 07 영어/한국어 문구·피하는 말·12px·24px·키보드·임계값 없음(아주 작은 값과 아주 큰 값의 값 칸 스타일이 같음), 08 Gateway Status 구역 무영향. CI는 measurements의 S5-U6a DOM 단계 다음 단계가 같은 인터프리터로 돌린다.
- 한계·미실행: Nest build와 컴파일 서버 9건은 hosted `kin-api:ci`에서만 돈다(로컬에 `/app/dist`가 없음). Orthanc 읽기는 이 단위의 쓰기 범위에 `orthanc.service.ts`가 없어 `OrthancService`의 인증된 `get`을 bracket 접근으로 쓴다(후속에서 공개 메서드로 옮길 자리). `main.html` 하단 `#storage`의 기본값 '0.0GB / -'(실패 시 0처럼 남음)는 이 단위의 쓰기 범위 밖이라 바꾸지 않았다. 큰 기관에서는 기관 범위 StudyState 행 전체와 첫 승인 GROUP BY를 한 번씩 읽는다. 실제 스택·live·hosted CI는 실행하지 않았다.

### S5-U4a 임상의 질문 서버 (REQ-S5-U4p-ATTRIBUTION/TENANT/STUDY-ACCESS-READ/QUESTION-STATE/IDEMPOTENCY/AUDIT-READ → RISK-S5-U4p-* → TEST-S5-U4a-ACCESS·LIVE·POLICY·ROUTES·RESTORE)
- 계약: S5-U4p(Astra S5-U4p-R-002 ACCEPT)와 지휘자 결정 D33. consultation과 별도 모델·서비스이며 consultation·Connect·StudyAccess·guard 코드는 바꾸지 않는다. 새 route 6개(`clinician-question.controller.ts`): `GET questions`(query `view`=mine|inbox, `state`, `cursor`), `GET questions/:id`, `GET studies/:uid/questions`(최신 50), `POST studies/:uid/questions`(`{requestId, expectedOwner, body}`), `POST questions/:id/entries`(`{requestId, expectedOwner, revision, body}`, 작성자면 followup, 아니면 answer), `POST questions/:id/close`(`{requestId, expectedOwner, revision, note}`). 역할은 동작마다 명시한 집합이고 `need()`를 쓰지 않는다: 등록·내 목록은 clinician, 받은편지함·전체 읽기는 radiologist 또는 admin, 답변은 radiologist만(admin 단독 403), 추가 질문·작성자 닫기는 clinician인 작성자, 남의 질문 닫기는 radiologist·admin과 사유, technician만 가진 회원은 모든 질문 route 403. 기관은 소유 기관만(원격판독 기관 404), clinician-only는 자기 질문만 본다. 소유 기관이 바뀐 검사의 질문은 옮기지 않고 양쪽에서 404로 숨긴다. 귀속(sub·actor·표시 이름·기관)은 토큰에서만 오고 body 키는 정확한 집합이다.
- 저장: migration `20260926120000_study_questions` — `StudyQuestion`(revision = entryCount, 닫기 칸은 Closed와 함께만)과 `StudyQuestionEntry`(적용된 요청마다 한 행 = 영수증: id = requestId, fingerprint, appliedRevision = seq, result = 그때 돌려준 `applied`). 두 FK 모두 RESTRICT이고 CHECK는 dump/restore에서 같은 명시 text 형태다. 적용된 어느 requestId든 재전송하면 가시성 판정 뒤·revision/상태 판정 앞에서 저장한 결과를 `201 {owner, applied, replayed:true}`로 돌려주고 행·revision·감사를 바꾸지 않는다. 100칸 중 마지막 한 칸은 닫기 몫이다. StudyAccess 원본 태그는 트랜잭션·잠금 전에 준비하고(UID route `prepare(c,[uid])`, id route `prepare(c)`, 목록 `allowed`), 트랜잭션 안에서는 준비한 태그만 평가한다. 스레드 읽기(#2)와 검사별 목록(#3)은 RepeatableRead 트랜잭션 하나에서 질문 행·검사 상태·항목·현재 판 번호를 행 잠금 없이 읽는다(Astra S5-U4a-F01 — 문장마다 새 스냅샷이면 그 사이의 답변·닫기·판독 취소가 섞여 revision·항목·closed·current가 없었던 조합이 된다). 쓰기 세 경로는 기본 격리 그대로 StudyState → StudyQuestion 순으로 잠그고, #1 목록은 요약·소유 기관을 한 문장으로 읽는다. 서비스는 Prisma와 StudyAccessService만 주입받는다. 감사는 적용마다 같은 트랜잭션 `study.question` 한 행(detail 키 `id, institution, entry, kind, from, to, revision, requestId, role`, 본문·사유 없음).
- `pacs.service.ts` 두 곳: `removeState`는 질문이 있는 검사를 부모 잠금 안에서 409 `STUDY_HAS_QUESTIONS`로 거절한다(cascade 없음). `audits()`의 UID 지정·미지정 두 경로는 `OWNER_ONLY_AUDIT_ACTIONS = ['study.question']` 행을 생성 기관(`detail.institution`) = 현재 소유 기관 = caller 기관일 때만 LIMIT과 같은 SQL의 WHERE에서 남긴다. 다른 action은 그대로이고 같은 시각의 행은 id로 순서를 고정한다.
- 정책·선언: `CLINICIAN_BUSINESS_ROUTES`와 `tests/clinician_policy_fixtures.json`의 `business_routes`·`allowed_additions`에 6행, `route_matrix` 117 = public 4 + session 2 + business 11 + denied 100(이 단위의 거절 수 변화 없음; denied 100은 S5-U6b `GET admin/metrics` 병합 뒤, S5-U5b `GET admin/audit` 병합 뒤 118 = 4 + 2 + 11 + 101), `post_baseline` 6행(b4d9989), `route_metadata_cases` 6건. `invariants_live.py` ROUTES에 6행을 `Kind.TENANT`로 선언했다.
- 시험: TEST-S5-U4a-ACCESS `tests/clinician_question_service_test.cjs`(kin-api:ci, runtime 단계 `tmp/runtime-ci/clinician-question-service`) — 실제 StudyAccessService + stub Prisma + 호출 기록 Orthanc로 (1) 원본 태그 읽기가 `$transaction` 전에 끝나고 콜백 안 Orthanc 호출 0, (2) 읽는 동안 정책 revision 변경 → 409 `STUDY_ACCESS_CHANGED`·트랜잭션·쓰기 0, (3) 읽기 실패 → 503·쓰기 0, (4) 생성자 = PrismaService·StudyAccessService, (5) Orthanc 호출은 `studyAccessMetadata`·`studyIdentities(true)`뿐이고 메타데이터 규칙이 없으면 0, 그리고 Closed 뒤 재전송·재사용 409·감사 키·범위 밖 상태 409 `QUESTION_STATE`. F01 세 건: stub의 RepeatableRead 트랜잭션은 시작 때 저장소 사본을 읽고 다른 트랜잭션은 문장마다 현재 저장소를 읽는다. (a) 스레드 읽기의 질문 행 뒤에 답변·닫기(서비스 쓰기 경로), 검사 상태 뒤에 판독 취소(저장소 직접 변경: A→W, 폐기 v2·취소 v3)가 커밋되어도 응답은 revision 1·Open·항목 1·current `{A, 1}` 한 시점이고 다음 읽기는 Closed·3·`{W, 3}` 전체다. (b) #2·#3은 `{isolationLevel: 'RepeatableRead', maxWait: 4000, timeout: 8000}` 트랜잭션 하나에서 FOR UPDATE/SHARE 없이 읽고, 트랜잭션 밖 읽기는 prepare의 정책 조회 하나뿐이다. (c) 등록·답변·닫기의 옵션은 `{maxWait: 4000, timeout: 8000}` 그대로이고 잠금 순서는 lock_timeout → 정책 공유 잠금 → StudyState FOR UPDATE → (id route) StudyQuestion FOR UPDATE → 쓰기 세 번이다. TEST-S5-U4a-LIVE `tests/clinician_question_live.py`(hosted 합성 전용, `python scripts/run-tests.py --module tests/clinician_question_live.py --mode live --unit s5-u4a-clinician-question --timeout 1800`) — 한 클래스에 계약 이름 그대로 Q01~Q14와 결함 S5-U4a-F01의 Q15, 모듈 상단 `CASES`가 순서와 예상 초(625 + 준비 90)를 고정한다. Q15는 run clinician의 study-access advisory 잠금을 psql 세션이 배타로 잡아 그 clinician의 #2·#3 읽기를 트랜잭션 첫 조회에서 기다리게 하고, 그동안 제품 route로 답변(doctor2)·닫기(doctor)·판독 취소를 커밋한 뒤 풀어 두 응답이 커밋 전 읽기와 같고(revision = entryCount = 항목 1..n, Closed ⇔ closed) 다음 읽기가 커밋 뒤 상태 전체임을 본다. 문장마다 스냅샷을 새로 잡는 읽기는 커밋 뒤 상태를 답해 실패한다. 세 쓰기가 서비스 lock_timeout 3초를 넘기면 두 읽기가 503이다(`CLINICIAN_QUESTION_Q15_WRITES_SECONDS`). 사례마다 run 소유 합성 검사 하나를 쓰고 fixture 정리 전에 질문 행을 지운다(FK RESTRICT). Q07·Q13은 자기 합성 검사의 소유 기관을 psql로 옮겼다가 finally에서 되돌린다(F-21). 복원: `ops_product_transfer_fixture.py`에 Closed 질문 한 개와 영수증 세 행(seq 1 생성·seq 2 답변·seq 3 빈 사유 닫기), 제약 탐침 7개, `ops_product_transfer_test.py` 30 migrations / 41 tables / +4 rows.
- 미실행·한계: 이 host에는 node·tsc가 없어 Nest build·cjs 시험은 hosted `kin-api:ci`에서만 돈다. live 15건, `production_image_test.py`(컨테이너), 복원 producer/consumer, hosted CI는 실행하지 않았다. 복원 receipt는 합성 행이 2,920 bytes 늘었고(정확) 전체는 모델링한 catalog로 약 185 KB/256 KiB로 추정했다(hosted 실측 전). clinician-only 회원의 검사 단위 StudyAccess 철회는 `study-access.service.ts` `target()`이 아직 clinician을 관리하지 않아 시험하지 않았다(D33 OQ-8 b, 후속 카드). U1c `clinician_policy_test.py` test_14와 `clinician_policy_live.py` test_05는 allow 행을 읽기로만 다뤄(positive 200/204, 요청 본문 없음, clinician 감사 행 0) 6행을 담지 못했다. 후속(지휘 결정 D36)에서 U1c 최종본 e378f04를 병합하고 쓰기 allow 행 계약으로 넓혀 6행의 `live_matrix` 사례를 넣었다(위 S5-U1c 절). live Q08은 여섯 행의 역할 표 전체를 따로 확인한다. 새 migration·`removeState` 변경으로 어긋난 다른 단위의 핀은 후속에서 현재 값으로 옮겼다(단언 유지, 핀마다 S5-U4a 근거 주석): `gateway_retry_source_test.py`(migration 29→30, 끝 세 개 U3→U4→`20260926120000_study_questions`, fixture MIGRATIONS 끝, tables 39→41, 행 합 `46 + 1 + 2 + 1 + 1 + 1 + 1 + 3`), `gateway_receipt_source_test.py`(29→30·39→41), `order_reconciliation_source_test.py`·`report_structure_migration_test.py`(29→30), `study_identity_source_test.py`(`removeState` sha256 a5c691d2… → a7809339…, 질문이 있는 검사 409).

### S5-U4c 외부영상·영상전송 요청 서버 (REQ-S5-U4p-ATTRIBUTION/TENANT/STUDY-ACCESS-READ/REQUEST-STATE/IDEMPOTENCY/CONNECT-SEPARATE/AUDIT-READ → RISK-S5-U4p-REQUEST-AS-TRANSFER/REPLAY-HISTORY/AUDIT-READ-LEAK/* → TEST-S5-U4c-ACCESS·LIVE·POLICY·ROUTES·RESTORE)
- 계약: S5-U4p §3~§13(Astra S5-U4p-R-002 ACCEPT)과 지휘자 결정 D33(OQ-1·5·7·10·13·14·15 a). ARCH-143 외부영상 요청과 ARCH-144 영상전송 요청의 **등록과 처리 상태만** 다루는 1/2 단위이고 화면은 2/2다. 요청은 전송이 아니다: 서비스는 Prisma와 StudyAccessService만 주입받고, `Transfer`·`TransferBasis`·`ProcessingAgreement`를 읽거나 쓰지 않으며 FK·연결 칸도 없다(OQ-10 a). Orthanc에 닿는 것은 StudyAccess 판정용 원본 메타데이터 읽기뿐이고 트랜잭션·잠금 전에 끝난다(§4.1). 실제 전송은 직원이 기존 Connect 경로를 따로 쓰며, Connect route 9개는 clinician-only에게 계속 403 `CLINICIAN_ROUTE_DENIED`다. Connect·StudyAccess·guard·orthanc·consultation·S5-U4a 질문 코드는 바꾸지 않았다.
- route 5개(`image-request.controller.ts`, `Kind.TENANT`, 이름에 transfer 없음): `GET image-requests`(query `view`=mine|queue, `state`=active(기본)|closed|declined|cancelled|all, `kind`?, `cursor`?; 총개수 없음), `GET image-requests/:id`, `GET studies/:uid/image-requests`(최신 50), `POST studies/:uid/image-requests`(`{requestId, expectedOwner, kind, counterparty, counterpartyInstitutionId, reason}` — kind는 external-image|image-transfer, counterparty 1~256자, 상대 기관 id는 키 필수·null 또는 등록된 다른 기관(모르는 id·자기 기관 400 `IMAGE_REQUEST_COUNTERPARTY_INVALID`), reason 1~2000자), `POST image-requests/:id`(`{requestId, expectedOwner, revision, action, note}` — action은 accept|close|decline|cancel, accept는 빈 note, 그 밖은 1~2000자). 역할은 동작마다 명시한 집합이고 `need()`를 쓰지 않는다: 등록·내 목록은 clinician, 대기열·전체 읽기는 radiologist·technician·admin(판독의는 읽기만, OQ-5 a), accept·close·decline은 technician·admin, 취소는 요청한 clinician 또는 admin(보이는 비요청자는 403 `IMAGE_REQUEST_ACTION_FORBIDDEN`). 기관은 소유 기관만이고 원격판독 기관과 상대 기관은 어떤 역할로도 읽지 못한다(404). clinician-only는 자기 요청만 본다. 소유 기관이 바뀐 검사의 요청은 옮기지 않고 양쪽에서 404로 숨긴다.
- 상태 기계(state-machine.json `image_request`): Requested → Accepted, Requested|Accepted → Closed(처리 기록)·Declined(거절 사유)·Cancelled(취소 사유), 종결 상태의 새 requestId는 409 `IMAGE_REQUEST_STATE`. Closed는 요청 처리를 마쳤다는 기록일 뿐 영상이 오갔다는 주장이 아니다(화면 문구 "이 기록은 실제 전송 여부를 나타내지 않습니다"는 2/2). 활성(Requested/Accepted) 요청은 `(studyUid, kind, requesterSub)`당 하나다(OQ-7 a, 검사 행 잠금 아래에서 확인해 409 `IMAGE_REQUEST_ACTIVE_EXISTS`, 부분 unique index `StudyImageRequest_active_key`가 마지막 방어선). 처리자(`handler`)는 처리 상태를 바꾼 technician·admin이고 요청자 자신의 취소는 처리자를 바꾸지 않는다. DTO에는 `*Sub`·기관 id·영수증 칸이 없고 키 이름에 transfer·sent·delivered·imported·completed가 없다.
- 저장: migration `20260926130000_study_image_requests` — `StudyImageRequest`와 `StudyImageRequestReceipt`(적용된 요청마다 한 행 = 영수증, 생성 포함: requestId PK·subjectSub·action·fingerprint·appliedRevision(요청 안에서 unique)·result = 그때 돌려준 `applied`; 생성 영수증은 requestId = 요청 id·revision 1). 모든 FK RESTRICT, CHECK는 dump/restore에서 같은 명시 text 형태이고 어느 행에도 NULL로 통과하지 않는다. 적용된 어느 requestId든 재전송하면 가시성 판정 뒤·revision/상태/활성 중복 판정 앞에서 저장한 결과를 `201 {owner, applied, replayed:true}`로 돌려주고 행·revision·영수증·감사를 바꾸지 않는다. 판정 순서는 역할 403 → 입력 400 → 동작 역할 403 → expectedOwner 409 → [트랜잭션 밖] StudyAccess 원본 태그 준비 → [안] 가시성 404 → 재전송 → revision 409 `IMAGE_REQUEST_CHANGED` → 전이 409 → 취소 행위자 403이고, 잠금 순서는 lock_timeout 3s → 정책 공유 잠금 → StudyState FOR UPDATE → StudyAccess 판정 → (id route) StudyImageRequest FOR UPDATE / (등록) 상대 기관 Institution FOR KEY SHARE → 영수증 조회다. #8·#9 읽기는 RepeatableRead 트랜잭션 하나에서 행 잠금 없이 요청 행과 검사 소유 기관을 읽는다(S5-U4a-F01과 같은 방식). 감사는 적용마다 같은 트랜잭션 `study.image-request` 한 행(detail 키 `id, institution, kind, action, from, to, revision, requestId, counterpartyInstitutionId, role`, 사유·note·상대 기관 문장 없음, 읽기 감사 없음 OQ-15 a).
- `pacs.service.ts` 두 곳: `OWNER_ONLY_AUDIT_ACTIONS`에 `study.image-request`를 더해 기존 `audits()` 두 경로가 이 행을 생성 기관 = 현재 소유 기관 = caller 기관일 때만 LIMIT과 같은 SQL의 WHERE에서 남긴다(원격판독·상대 기관은 받지 못한다). `removeState`는 요청이 있는 검사를 부모 잠금 안에서 409 `STUDY_HAS_IMAGE_REQUESTS`로 거절한다(cascade 없음).
- 정책·선언: `CLINICIAN_BUSINESS_ROUTES`와 fixture `business_routes`·`allowed_additions`에 5행, `route_matrix` 122 = public 4 + session 2 + business 16 + denied 100(거절 수 불변; S5-U5b `GET admin/audit` 병합 뒤 123 = 4 + 2 + 16 + 101), `post_baseline` 5행(4749f5f), `route_metadata_cases` 5건, `live_matrix` 5행(쓰기 2행: 등록 201·취소 201, 각 `study.image-request` 감사 1행; 위 S5-U1c 절). `invariants_live.py` ROUTES에 5행을 `Kind.TENANT`로 선언했다.
- 시험: TEST-S5-U4c-ACCESS `tests/image_request_service_test.cjs`(kin-api:ci, runtime 단계 `tmp/runtime-ci/image-request-service`) — 실제 StudyAccessService + stub Prisma + 호출 기록 Orthanc로 (1) 원본 태그 읽기가 `$transaction` 전에 끝나고 콜백 안 Orthanc 호출 0(등록은 `studyAccessMetadata`, 변경과 그 재전송은 `studyIdentities(true)`), (2) 읽는 동안 정책 revision 변경 → 409 `STUDY_ACCESS_CHANGED`·트랜잭션 0·요청/영수증/감사 쓰기 0(등록·변경·재전송), (3) 읽기 실패 → 503·같음, (4) 생성자 = PrismaService·StudyAccessService이고 컴파일본에 Orthanc/Connect 서비스·Connect delegate·표 이름이 없음, (5) Orthanc 호출은 `studyAccessMetadata`·`studyIdentities(true)`뿐이고 메타데이터 규칙이 없으면 0. 그리고 Closed 뒤·새 활성 요청 뒤의 재전송(S-R1~S-R11 모양)·재사용 409·활성 규칙·상대 기관·동작 역할과 행위자 규칙·감사 키·범위 밖 상태 409, #8·#9의 RepeatableRead 한 트랜잭션(행 읽기와 검사 읽기 사이에 커밋된 소유 기관 이동이 응답에 닿지 않음), 쓰기 잠금 순서. TEST-S5-U4c-LIVE `tests/clinician_request_live.py`(hosted 합성 전용, `python scripts/run-tests.py --module tests/clinician_request_live.py --mode live --unit s5-u4c-image-request --timeout 1800`) — 한 클래스에 계약 이름 그대로 R01~R11, 모듈 상단 `CASES`가 순서와 예상 초(555 + 준비 105)를 고정한다. R01은 등록·모든 전이·재전송 전후 Connect 세 표(행 수와 전체 행 digest)와 Orthanc `/statistics`·`/changes?last`·`/jobs`가 같고, 서비스 소스의 주입이 둘뿐이며 Orthanc/Connect 서비스·Connect delegate·표 이름이 없음을 본다. R03은 S-R1~S-R11, R05는 S-R12(technician을 잃은 혼합 회원의 accept 재전송 403)·S-R13(소유 기관 이동 뒤 close 재전송 404)과 요청자의 clinician 제거, R10은 Q13의 (a)~(f)와 tele 없는 두 번째 검사의 상대 기관 D, R11은 메타데이터 규칙(technician modality 일치·불일치, 혼합 요청자 patientId 일치·dateTo 이전)을 확인한다. 사례마다 run 소유 합성 검사를 쓰고 fixture 정리 전에 요청 행을 지운다(RESTRICT, 남이 쓴 행이 있으면 정리를 멈춘다). R05·R10은 자기 합성 검사의 소유 기관을 psql로 옮겼다가 finally에서 되돌린다(F-21). 복원: `ops_product_transfer_fixture.py`에 Closed image-transfer 요청(상대 기관 SYNTHETIC-tele, 생성·accept·close 영수증)과 활성 external-image 요청(상대 기관 id·처리자·note NULL, 생성 영수증), 제약 탐침 11개, `ops_product_transfer_test.py` 31 migrations / 43 tables / +6 rows.
- 다른 단위의 핀은 현재 값으로 옮겼다(단언 유지, 핀마다 S5-U4c 근거 주석): `gateway_retry_source_test.py`(migration 30→31, 끝 네 개 U3→U4→`20260926120000_study_questions`→`20260926130000_study_image_requests`, fixture MIGRATIONS 끝, tables 41→43, 행 합 `46 + 1 + 2 + 1 + 1 + 1 + 1 + 3 + 2 + 4`), `gateway_receipt_source_test.py`(30→31·41→43), `order_reconciliation_source_test.py`·`report_structure_migration_test.py`(30→31), `study_identity_source_test.py`(`removeState` sha256 a7809339… → f5cf8d78…, 요청이 있는 검사 409).
- 미실행·한계: 이 host에는 node·tsc가 없어 Nest build·cjs 시험은 hosted `kin-api:ci`에서만 돈다. live 11건과 policy live, `production_image_test.py`(컨테이너), 복원 producer/consumer, hosted CI는 실행하지 않았다. 복원 receipt는 합성 행이 3,560 bytes 늘었고(34자리 UID로 측정) catalog는 약 13 KB로 모델링해 전체 약 203 KB/256 KiB로 추정했다(hosted 실측 전). clinician-only 회원의 검사 단위 StudyAccess 철회는 시험하지 않았다(`target()` 미확장, D33 OQ-8 b). `tests/clinician_read_serializer_test.cjs`는 clinician 비GET allowlist를 fixture 순서의 정확한 여섯 값(SOP lookup 1 + 질문 스레드 3 + 영상 요청 2 `'POST studies/:uid/image-requests', 'POST image-requests/:id'`)으로 고정한다 — S5-U4a의 네 값 pin이 hosted measurements 단계를 실패시키기 전에 후속 커밋에서 두 요청 쓰기 행을 더했다.

### S5-U5b 관리자 감사·보안 기록 (REQ-S5-U5b-ADMIN-AUDIT/MOVE-PROJECTION/UNCLEAR-HIDDEN/NO-MIGRATION-START → RISK-S5-U5b-CROSS-INSTITUTION/CURRENT-GROUP-ATTRIBUTION/CURRENT-OWNER-ATTRIBUTION/HIDDEN-COUNT/NEW-ACTION-DEFAULT/INVENTED-FIELDS/FAILURE-AS-EMPTY → TEST-S5-U5b-PURE·DOM·LIVE)
- 계약: 새 읽기 route `GET /api/admin/audit` 하나(`admin.controller.ts` → `AdminService.auditEvents`). `admin` 역할이 명시로 필요하고(clinician-only는 guard 기본 거절 `CLINICIAN_ROUTE_DENIED`, technician·radiologist는 403), 읽는 기관은 호출자 기관이다. 기존 검사 범위 `GET /api/audit`은 바꾸지 않았다. migration은 없다(D7 a). `invariants_live.py` ROUTES에 `("GET", "admin/audit"): Route(Kind.TENANT)` 한 행을 더했다. clinician 허용 목록은 바꾸지 않았고, main(S5-U1c·U6b) 병합 뒤 `route_matrix`에 `GET admin/audit`가 denied(admin) 행과 `post_baseline` 행으로 들어가 112 = 4 + 2 + 5 + 101이었다. main(S5-U4a) 병합 뒤 질문 6행(business)과 합쳐 118 = 4 + 2 + 11 + 101이었다. main(S5-U4c) 병합 뒤 영상 요청 5행(business)과 합쳐 123 = 4 + 2 + 16 + 101이다.
- 귀속(`api/src/admin-audit.ts`, 순수): 행을 **쓸 때 남은 값**만 본다. 회원 행(`admin.user.create/create.failed/approve/update/unapprove/suspend/activate/reset-password/patch.failed`)은 detail의 before/after 스냅숏 기관, 검사·기관 행은 카드 표의 칸 하나(`detail.institutionId`·`detail.by`·`detail.institution`·`target`, `report.{addendum,approve,defer,preliminary,reset,save}`는 `detail.by`)로 정한다. 회원의 지금 Keycloak 그룹, 검사의 지금 StudyState 소유·원격판독 기관, 지금 StudyAccess로는 귀속하지 않는다. 기관을 둘 담은 회원 행(이동·부분 이동 실패)은 두 기관에 보이되 각자 자기 쪽 스냅숏만, 반대쪽은 이름 없이 `{withheld:'other_institution'}`이다. 스냅숏은 id·username·name·roles·enabled·approvalState·emailVerified·institution만 싣고 email은 싣지 않는다(모양이 다른 칸은 싣지 않아 화면이 not recorded로 보인다). 숨김(fail closed): 기관을 남기지 않은 행(PENDING 생성·실패 생성·여러 그룹이라 institution이 null인 INVALID 스냅숏·파싱 불가 detail·다른 회원 id·`admin.user.list`·`report.draft*`·`report.hold`·`dictation.request`·gateway receipt/retry·`viewer.*`·`finding.*`·`manualSr.*`·`favorite.*`·`study.tag.*`), Connect 행, 검사 범위 기록(`AUDIT_HIDDEN_STUDY_SCOPED` → `hidden:study_scoped_owner_only`: S5-U4a 임상의 질문 `study.question`은 detail에 생성 기관이 있어도 명시 숨김 행으로 어느 기관에도 싣지 않는다 — 그 행은 검사별 감사 `audits()`의 `OWNER_ONLY_AUDIT_ACTIONS`가 생성 기관 = 현재 소유 기관 = 읽는 기관일 때만 보여 주는 기록이다, Astra S5-U5b-D-F02. main(S5-U4c) 병합으로 들어온 영상 요청 `study.image-request`도 같은 `OWNER_ONLY_AUDIT_ACTIONS` 기록이라 같은 목록·같은 규칙의 명시 숨김 행이다), 표에 없는 action(기본 `hidden:unknown_action`).
- 조회: SQL은 표에 있는 action이면서 detail에 이 기관 문자열의 JSON 표기(`auditMention`)가 있거나 target이 기관인 action이면 target이 이 기관인 행만 후보로 넘기고(보이는 행을 빠뜨리지 않는 넓은 조건), 정확한 판정은 `readAuditPage`가 쪽을 자르기 **전에** 한다. detail 검색은 `$queryRaw`의 `strpos("detail", …) > 0`(글자 그대로의 부분 문자열)이다 — Prisma `contains`(LIKE 패턴)는 JSON 표기의 역슬래시를 이스케이프로 먹어 `\`·`"`가 든 기관의 행을 판정 전에 놓쳤다(Astra S5-U5b-B-F02). 같은 조건의 순수 쌍둥이가 `auditCandidateRow`다. `total`·다음 쪽 여부·빈 목록은 보이는 행만 센다. 다음 쪽 값(`next` → `after`)은 첫 쪽이 읽은 가장 큰 id(기준)와 마지막 행 id를 AES-256-GCM(프로세스 키, 10분)으로 봉인한 값이라 행 id의 틈으로 숨긴 행 수를 알 수 없다. 봉인 전 값은 판 1바이트와 top·after·만료 각 8바이트 big-endian의 고정 25바이트이고 기관+계정은 평문이 아닌 GCM 추가 인증 자료로 묶으므로, 값은 누구에게나 늘 71자다 — 기준 id(기관을 가리지 않은 전체 최댓값)의 자리수가 값의 길이로 드러나지 않는다(Astra S5-U5b-B-F01). 위조·다른 계정·만료·API 재시작 전 값·다른 길이나 판은 409 `ADMIN_AUDIT_CURSOR_EXPIRED`다. `limit`(1–100, 기본 25)·`after` 밖의 질의는 400 `ADMIN_AUDIT_QUERY_INVALID`, DB 읽기 실패는 503 `ADMIN_AUDIT_UNAVAILABLE`(빈 목록이 아님)이다. 검사 접근이 제한된 관리자는 403 `ADMIN_AUDIT_RESTRICTED`(검사 행에 overlay 같은 검사 내용이 실리므로, 운영 지표와 같은 이유 — 읽는 사람의 관문이며 귀속에 쓰지 않음), 읽는 동안 조건이 바뀌면 409 `STUDY_ACCESS_CHANGED`다.
- 화면: 회원 콘솔(`admin.html`) `Audit / Security` 구역. `Refresh Audit`를 누를 때만 읽고 `Load More`는 받은 `next`를 그대로 보낸다(부팅 요청은 여전히 `/api/me`와 회원 목록뿐). 열: Time·Actor·Action·Target·Previous Value·Reason·Recorded Detail. Previous Value는 회원 행의 before 스냅숏뿐이고 나머지는 `not recorded`, Reason은 행이 reason 문자열을 남겼을 때만 그 값이다. 기록되지 않은 값은 `not recorded`, 기록된 null·빈 목록은 `None`, 다른 기관 쪽은 `withheld: other institution`(한국어 설명 툴팁)이다. 고정 문장 둘: `오래된 행 중 기록 시점 기관이 없는 행은 표시되지 않습니다.`(숨긴 수 없음), `접속 이력: 수집되지 않음`(D6: 로그인·로그아웃·세션 만료 감사는 후속). 순수 모델 `<script id="admin-audit-model">`(`KinAdminAudit`)이 답의 모양을 확인하며 스냅숏에 투영 밖 칸(email 포함)이 있으면 답 전체를 읽지 않는다. 조회 실패는 고정 문장과 `Query Failed`, 마지막 목록을 흐리게 두고 빈 문장을 보이지 않는다. Refresh는 요청 번호로(A→B→A), Load More 답은 같은 목록일 때만 붙인다. 401은 본문 전에 페이지의 한 세션 종료로 가며 이 구역도 `KinConsoleSession.onEnd`에 닫는 함수를 등록해 즉시 비운다.
- 시험: TEST-S5-U5b-PURE — `node --test tests/admin_audit_attribution_test.cjs` 14건(기본 `api/src/admin-audit.ts`를 Node ≥22.18 type stripping으로, `KIN_ADMIN_AUDIT_MODULE`로 compiled 모듈 지정 가능): 카드 계약표 대조, 합성 벡터 31개(기록 시점 기관·withheld 쪽. 카드의 30개와 카드 블록은 원문 그대로이며 증거 기록에서 카드와 같음을 확인, 31번 `study.question` 행과 그 계약 행은 카드 블록 뒤에 덧붙였고 detail에 기관이 있어도 어느 기관에도 안 보이며 SQL 후보도 아님), 거절된 현재 그룹·현재 소유 규칙을 부정 대조군으로(카드의 누출 9행을 정확히 재현), 불분명 행, 쪽 읽기(거르기 뒤 자르기·total·원천 실패·순서 위반), 봉인 값, 봉인 값의 길이 불변(B-F01: top·after 9/10·99/100·999/1000·int4 최댓값·안전 정수 최댓값, 시계, 소유자 길이에 71자 고정, 담을 수 없는 값은 봉인 거절, 다른 판·범위 밖·다른 길이·옛 JSON 평문은 같은 키·소유자여도 거절 / 보이는 행 위에 숨긴 행만 더해 기준 id가 5→10000으로 자리수를 넘어도 rows·total·쪽 끝·다음 쪽·길이 모두 같음), SQL 사전 거르기(B-F02: `\`·`"`·`%`·`_`·한글 기관명 7개와 다른 기관 하나에서 보이는 행은 모두 `auditCandidateRow`를 통과하고 거른 읽기가 전체 읽기와 같음, 다른 기관 행은 안 섞임, 없앤 LIKE 형태를 모형으로 돌리면 `\`·`"` 기관에서 정확히 detail 귀속 8행씩을 놓치는 부정 대조군, 서비스 SQL이 같은 mention의 `strpos`·같은 후보/target 목록이고 LIKE·`contains`가 없음을 소스에서 고정), 질의 모양, 완전성(api/src의 감사 쓰기 54곳·helper 정의 5·callback 1에서 action 56개와 동적 접두어 4개가 모두 표에 있고, 표의 모든 행이 어딘가에서 쓰임. action이 이름 하나이면 같은 파일의 모듈 수준 `(export) const 이름 = '리터럴';` 선언 하나만 읽는다 — 지금은 `clinician-question.service.ts`의 `QUESTION_AUDIT_ACTION`과 `image-request.service.ts`의 `IMAGE_REQUEST_AUDIT_ACTION` 두 곳이고, import한 이름·복수 선언·let/var·블록 안 선언·리터럴이 아닌 초기화·같은 이름의 매개변수/구조 분해는 읽지 않아 실패한다), 완전성 부정 대조(선언 값 변경은 미등록 action, 선언 형태 변경 8가지(import로 바꾼 것 포함)와 다른 파일에서 import한 상수를 쓰는 새 쓰기는 읽을 수 없는 쓰기, 새 쓰기의 리터럴·상수 action은 미등록으로 각각 실패). CI는 workspace-ui 작업의 S5-U1b serializer 다음 단계(node:22-alpine, 저장소 읽기 전용). TEST-S5-U5b-DOM — `python -B tests/admin_audit_dom_test.py` 12건(출하 `admin.html`·`auth.js`·`study-access-admin.js`를 합성 origin에서 그대로 실음): 01 부팅 요청 없음·고정 문장, 02 행 문구·withheld·not recorded·None, 03 Load More, 04 빈 답, 05 실패 ≠ 빈 목록, 06 A→B→A와 대조군, 07 대체된 목록의 Load More 답과 대조군, 08 401, 09 Log out, 10 회원 목록 401, 11 문구·12px·thead·마크업 무실행·키보드, 12 모델 거절. CI는 measurements의 S5-U6b DOM 단계 다음. TEST-S5-U5b-LIVE — `python scripts/run-tests.py --module tests/admin_audit_live.py --mode live --unit s5-u5b-admin-audit --timeout 900`(hosted 합성 전용, 한 클래스 6건 선언 순서, 예상 합계 약 260초): 합성 기관 A/B/Z(run 소유 Keycloak 그룹), 회원 m의 A 승인·역할 변경·비밀번호 재설정·A 정책·Z 관리자의 A→B 이동·B 정지·승인 취소, 저장 행으로 넣은 부분 이동 실패와 여러 그룹 스냅숏, m 삭제 뒤에도 A·B 기록 유지, A가 announce·수정·삭제한 검사를 B가 다시 announce해도 B는 A 행을 못 받음, 기관 없는 행은 아무에게도 안 보임(합계 불변), limit=2 쪽 걷기와 봉인 값의 소유자 묶음, 제한된 관리자 403. 합성 기관마다 같은 id의 `Institution` 행을 setUpClass가 psql로 넣고(`StudyAccessPolicy.institution` 외래 키 대상, 없으면 정책 쓰기가 500 — hosted 36244365412), 정리는 run의 Study Access 행을 지운 뒤 이 run이 넣은 id만 지우며 남으면 실패한다. 기관별 DB 정리(기관 대상 감사 행·사이트 Hanging Protocol·Study Access 정책과 이력·StudyState)는 INSERT가 성공해 소유가 확인된 기관(`cls.institutions`)에만 하고 그 안에서도 Study Access는 run의 subject, 감사 행은 run의 actor, StudyState는 run의 UID로 한정한다 — INSERT가 실패한(이미 있던) 기관은 run이 만든 Keycloak 그룹만 지우고 어느 DB DELETE에도 넣지 않는다(Astra S5-U5b-D-F01).
- 회귀 영향: 이 구역이 `KinConsoleSession.onEnd`에 다섯 번째 닫는 함수를 등록하므로 `tests/admin_metrics_dom_test.py` test_13의 등록 수 고정값은 4에서 5로 바뀌었다(U6b 병합 뒤 후속 커밋). 다른 U6a/U6b 시험이 편집하는 문자열 기준점은 모두 페이지에 한 번씩 남는다.
- 한계·미실행: Nest build·tsc와 live 6건은 hosted에서만 돈다. 쪽마다 기관 후보 행 전체를 걸어 total을 센다(큰 기관에서는 쪽 읽기마다 후보 행 전체 파싱). `strpos` 사전 거르기가 실제 PostgreSQL에서 특수문자 기관명의 행을 모두 넘기는지는 순수 모형과 소스 고정으로만 확인했다(live 6건은 특수문자 기관을 만들지 않는다). 감사 기록 읽기 자체는 감사하지 않는다. `study.question`은 main(S5-U4a) 병합으로 들어온 쓰기이고 명시 숨김 행(`hidden:study_scoped_owner_only`)으로 분류했다 — 회원 콘솔에 보이게 하려면 별도 카드의 귀속 규칙이 필요하다. `study.image-request`(main(S5-U4c) 병합으로 들어온 쓰기)도 같은 명시 숨김 행이다.
