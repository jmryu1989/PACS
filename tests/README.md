...
REQ-S3-U2a-CITATION-BACKEND → RISK-S3-CITATION-LEAK/FALSE-PRESENCE/CARRY-FORWARD-WRONG-HEAD/CHECK-AS-500 → TEST-S3-U2a-CITATION-BACKEND: 판독문 인용은 `ReportDraft`·`ReportVersion`의 추가 전용 JSONB 칸 둘뿐이고 `Report` 거울도 연결 표도 만들지 않는다. `python tests/report_citation_vectors_test.py`의 순수 8건은 공용 벡터 파일(`tests/report_citation_vectors.json`, 24+8+6+7+8 벡터)을 계약 규칙의 독립 구현과 대조한다 — 줄 블록은 완전한 줄·연속·겹치지 않음이고, 비교 정규화는 CRLF·홀로 있는 CR·NFC이며, 블록 끝 LF는 종결자라 이어 쳐도 `present`가 뒤집히지 않고, 공백만인 문구는 서버에서도 거절된다. `python tests/report_citation_migration_test.py`의 순수 17건은 추가 전용 DDL, **NULL 허용을 말로 적은 CHECK**(선례를 그대로 베끼면 NULL이 거절된다), canonical `jsonb::text` UTF-8 바이트 한도, 서비스가 409로 옮기는 제약 **이름**의 일치, `versions()`의 명시 select와 `toClient` 무변경, 마이그레이션 부기(`production_image_test`·`ops_product_transfer_fixture`, TABLES 무변경)를 고정한다. `node --test tests/report_citation_test.cjs`의 컴파일 API 18건은 빌드된 이미지에서 위조 차단(`cid`·`insertedBy`·`insertedAt`·`sourceRef`·링크 상태는 서버가 쓴다), 유지 목록의 교집합과 키 부재=무변경, 머리 판이 `ReportVersion(uid, Report.version)`이며 더 큰 번호의 `discarded` 행이 아니라는 것, 이월 건의 작성자·시각 바이트 보존, `removeCitationIds`로만 빠지는 머리 건, reset·빈 확정의 `[]`, 강제 해제의 작성자별 보존과 NULL 생략, 머리40+초안30의 `REPORT_CITATION_LIMIT` 409(문구에 `저장했습니다` 없음), 감사에 `findingId`·`sourceIndex`·문구가 없다는 것, 전용 읽기의 소견 가독 재적용과 `source-unavailable` 축약을 확인한다. **첫 실행은 전부 호스티드 CI다** — 컴파일 API 18건과 실제 PostgreSQL CHECK(`viewer_migration_test.py`의 `test_report_citations_additive_nullable_bounded_and_named`)는 호스트에서 실행할 수 없다. 드라이버가 CHECK 위반을 어떤 예외 모양으로 올리는지는 그 실행에서만 확인되며, 서비스는 **제약 이름**으로만 좁게 가른다. 화면(S3-U2b)은 이 묶음에 없다 — 벡터의 `assembly` 절(R5 문면)과 존재 상태 표시·클라이언트 상태는 U2b가 소비하며 지금은 어떤 제품 코드도 읽지 않는다.
