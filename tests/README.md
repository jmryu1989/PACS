...
REQ-S5-U5a-CLINICIAN-ROLE-ROUNDTRIP → RISK-S5-U5a-SILENT-ROLE-DROP/LABEL-RULE → TEST-S5-U5a-DOM/SERVICE: 회원 콘솔(`worklist-v0/hpacs-lite/admin.html`)의 저장은 PATCH `roles`로 회원 역할 전체를 바꾼다. 체크된 세 역할만 보내던 이전 화면은 `{clinician, radiologist}` 회원의 clinician을 오류 없이 뺐다. 역할 체크박스에 `clinician`(`임상의`)을 더했고, 이 화면이 그리지 않는 역할(서버 `APP_ROLES`가 화면보다 먼저 늘어난 경우)은 대화상자에 바꿀 수 없는 표시(체크·비활성·`name` 없음, 한국어 툴팁)로만 보이고 저장 목록에는 불러온 값 그대로 붙는다. 관리 역할 목록은 마크업의 체크박스 하나에서 읽는다. 같은 단위에서 AGENTS §4에 따라 이 화면의 메뉴·버튼·상태명·패널 제목(진입 패킷 `wording_acceptance.english_required` 24개와 헤더의 `Worklist` 링크)을 영어로 바꾸고, 설명·툴팁·확인·오류 안내와 패킷이 제외한 필드 라벨·표 머리글·`역할` legend·역할 라벨(새 `임상의` 포함)·`전체 기관` 선택지는 한국어로 둔다. `tests/admin_member_roles_dom_test.py`(Playwright Chromium, 서버·네트워크·자격증명 없음)는 출하 `admin.html`·`auth.js`·`study-access-admin.js`를 합성 origin에서 그대로 싣고 `/api/me`·`/api/admin/users`를 시험 안 저장소(역할은 `AdminService.row()`처럼 정렬)로 답하며 모든 PATCH 본문을 기록한다. 답하지 않는 요청과 page error는 실패다. 01 clinician-only·legacy·정지 회원의 불러오기→저장→다시 열기 왕복, 02 혼합 회원의 clinician 유지(그대로 저장·다른 역할 추가)와 관리자가 해제할 때만 제거, 03 모르는 역할의 읽기 전용 표시·되돌려 보냄·다음 회원으로 새지 않음·마크업으로 해석되지 않음, 04 PENDING 회원을 키보드로 열어 clinician 승인, 05 english_required 24개(정적 문구는 첫 목록 요청을 붙잡은 상태에서 읽음)와 버튼·제목·배지·헤더 링크의 한글 0, UXR-SP-34 단어 없음, 역할 라벨 12px 이상, 06 한국어 안내·확인·오류(서버 문구 그대로)와 회원 데이터 비번역, 07 대조군: 이전 역할 처리(clinician 체크박스 없음 + 이전 `admin.html:446` 제출)와 체크값만 보내는 제출을 같은 경로로 싣고 역할이 조용히 빠지는 정확한 출력을 단언한다. 이 단위 변경 파일로 로컬 1회 실제 실행: **7/7 통과**(Playwright 1.60.0, `tests/e2e/requirements.txt`와 같은 판). 같은 변경에서 `python -B tests/clinician_policy_test.py` 8/8(역할·경로 목록 불변: 현재 108, clinician-only 거절 102). 서비스 절반은 `tests/clinician_policy_live.py` test_03(approve [clinician] → 혼합 → 취소)을 hosted에서 재사용하며 이 단위에서 실행하지 않았다. 이 DOM 시험은 `.github/workflows/validate.yml` measurements 작업에서 실행한다(기존 Playwright/Chromium 설치 뒤 단계 하나, 실행 기록 `tmp/workspace-ui-ci/admin-member-roles-dom`은 workspace DOM artifact로 올라간다). 이 단계의 hosted 실행 결과는 아직 없다.
