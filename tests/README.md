...
REQ-S3-U3-STALE-ADDENDUM → RISK-S3-APPROVED-OVERWRITE/DRAFT-LOSS → TEST-S3-U3-STALE-ADDENDUM: `node --test tests/report_rebase_model_test.cjs`의 순수 5건은 main.html에서 그대로 떼어낸 기준 판 기록기·실패 분기·거절 본문 해석기를 검사한다 — 다시 그리지 않은 판 번호는 기준이 되지 못하고, `REPORT_DRAFT_STALE`은 `저장했습니다` 부분 문자열 분기보다 **먼저** 갈리며, 모양이 아닌 본문은 승인본으로 표시되지 않는다(호출부 4곳의 배선도 같은 파일에서 고정한다). `tests/report_stale_draft_test.cjs`의 8건은 빌드된 이미지(`kin-api:ci`, `/app/dist`)에서 `commitReport`의 추가기재 관문이 잠근 `Report` 행 그대로를 409에 싣고 아무 행도 쓰지 않는지, 역할·기관·예비 판독 거절이 먼저 답하고 승인본을 흘리지 않는지, 초안 없는 확정과 다른 action은 그대로인지, `putReport`가 아직 없는 판을 기준으로 거절하고 기준이 올라가는 PUT만 `report.draft.rebase`로 남기며 `Report`를 잠그지 않는지 확인한다. `python tests/report_rebase_dom_test.py`의 격리 Chromium 7건은 실제 `api()`·`loadReport()`·`stashReport()`·`commitReport()`와 실제 모달 표시로, 폴링·PATCH가 올린 판 번호가 확정 기준이 되지 못하고, 거절 뒤 textarea·초안 바이트가 그대로이며 `저장했습니다` 경로로 가지 않고, 화면에 보인 그 판 번호로만 재기준 PUT이 나가고, 검사를 옮기거나 더 새 거절이 온 창은 쓰지 않는 것을 확인한다. 계약 ㉑(실스택에서 두 사람이 겹쳐 승인·추가기재)은 `tests/e2e/test_worklist.py`의 `test_12_stale_draft_addendum_rebase_keeps_both_versions`가 맡는다 — 실제 409 직렬화 → `api()` → 안내 창 → 실제 재기준 PUT → 관문·낙관적 락 통과 → `/report/versions`를 한 번에 지나가며, 안내 창의 승인본 문자열이 그 409 본문의 `head.findings`와 **같은 바이트**인지, 거절 뒤 textarea와 초안 행이 그대로인지, 재기준 PUT이 화면에 보인 판 번호(v2)와 친 글자 그대로인지, 감사에 `report.draft.rebase`가 `{from:1,to:2}`로 한 번만 남는지, 마지막 이력이 approve·addendum·addendum이고 두 본문이 모두 살아 있는지 확인한다. 기존 `hanging-protocols` job의 `ci-hp-worklist`(240초 그대로)와 `candidate_ci`에서 함께 돌며, 이에 따라 선언 건수 핀이 14 → 15(누적 선택 90 → 91)로 옮겨졌다. **로컬에서 실행한 것은 순수 6건뿐이고, 나머지 세 파일(컴파일 API 8건·격리 Chromium 10건·호스티드 ㉑ 1건)의 첫 실행은 호스티드 CI다.**
