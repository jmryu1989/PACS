...
수동 `Focused integration` workflow는 같은 빈 GitHub-hosted runner와 실행 가드를 재사용한다. 여섯 선택지는 `output-integration`, `identity-fields`, `vr-resize-probe`, `hanging-protocols`, `dicom-pdf`, `image-thumbnails`다. 기본 `output-integration` 선택은 비교 판독문 6개 뒤 Job 판독문 4개를 실행한다. `identity-fields` 선택은 표시 설정 API 17개, 위치 API 2개, 필드 API 2개, 실제 위치 2개, 실제 필드 2개를 이 순서로 실행하며 프로필별 정화 로그를 분리한다. runner의 `--profile` 허용 목록은 이 여섯 선택지에 자동 `measurements`, `volume-rendering`을 더한 여덟 프로필을 검사한다. 이는 실제 실행 경로의 등록 설명이며 실행 성공 근거가 아니다. 로컬 원본 fixture에서 실행하지 않는다.
