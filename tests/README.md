...
- 계약: 새 읽기 route `GET /api/admin/metrics` 하나(`admin.controller.ts` → `PacsService.adminMetrics`). `need('admin')`·`inst()` 뒤 검사 접근 조건을 읽고, 제한된 계정은 403 `ADMIN_METRICS_RESTRICTED`(범위 밖 검사를 세어 주는 창이라 `/statistics`와 같은 이유), 읽는 동안 조건이 바뀌면 409 `STUDY_ACCESS_CHANGED`다. clinician-only는 guard 기본 거절(allowlist 밖)이며 clinician 허용 목록은 바꾸지 않았다. S5-U1c 병합 뒤 `route_matrix`에는 `GET admin/metrics`가 denied(admin) 행과 `post_baseline` 행으로 들어가 111 = 4 + 2 + 5 + 100이 되었고, S5-U4a 질문 6행과 합친 뒤에는 117 = 4 + 2 + 11 + 100, S5-U5b `GET admin/audit`까지 합친 뒤에는 118 = 4 + 2 + 11 + 101이다. `AdminController` 접두어를 `admin`으로 올리고 회원 네 경로를 `users…` 핸들러로 옮겼다 — 접두어+핸들러를 이어 붙인 URL과 route key는 그대로다. `invariants_live.py` ROUTES에 `("GET", "admin/metrics"): Route(Kind.TENANT)` 한 행을 더했다.
