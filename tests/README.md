...
REQ-S3-U2a-CITATION-BACKEND → RISK-S3-CITATION-LEAK/FALSE-PRESENCE/CARRY-FORWARD-WRONG-HEAD/CHECK-AS-500/ATTESTATION-LOSS → TEST-S3-U2a-CITATION-BACKEND: 판독문 인용은 `ReportDraft`·`ReportVersion`의 추가 전용 JSONB 칸 둘뿐이고 `Report` 거울도 연결 표도 만들지 않는다. **호스트 순수 31건**: `python tests/report_citation_vectors_test.py` 10건이 공용 벡터 파일(`tests/report_citation_vectors.json` — occurrence 24·blank 8·sameText 7·state 7·equivalence 6·assembly 8)을 계약 규칙의 독립 구현과 대조하고, `python tests/report_citation_migration_test.py` 21건이 추가 전용 DDL·**NULL 허용을 말로 적은 CHECK**·canonical `jsonb::text` UTF-8 바이트 한도·서비스가 409로 옮기는 제약 **이름** 일치·`versions()`의 명시 select와 `toClient` 무변경·전용 읽기가 행마다 따로 묻는다는 것·확정이 지울 초안 행을 먼저 잠근다는 것·재시도 다리까지 같은 매핑 안에 있다는 것·`n`과 `k`가 같은 등식을 쓴다는 것·마이그레이션 부기를 고정한다. **컴파일 API 27건**(`node --test tests/report_citation_test.cjs`, 빌드된 이미지 필요)은 위조 차단·유지 목록 교집합과 키 부재=무변경·머리 판이 `ReportVersion(uid, Report.version)`이라는 것·이월 건의 작성자·시각 바이트 보존·`removeCitationIds`로만 빠지는 머리 건·reset과 빈 확정의 `[]`·강제 해제의 작성자별 보존과 NULL 생략·삽입 시점 예방 한도와 유지 목록 복구·머리40+초안30의 `REPORT_CITATION_LIMIT` 409·머리40+초안30 **읽기**가 행마다 64 한도 안에서 답한다는 것·감사에 `findingId`·`sourceIndex`·문구가 없다는 것·전용 읽기의 소견 가독 재적용과 `source-unavailable` 축약을 확인한다. **실행된 적 없는 호스티드 관문**: 위 27건, `viewer_migration_test.py`의 `test_report_citations_additive_nullable_bounded_and_named`(실제 PostgreSQL DDL·NULL·한도·제약 이름·dump/restore)와 `test_report_citations_runtime_check_translation_and_draft_lock`(**실제 컴파일 `PacsService` + 실제 `PrismaClient`**로 `putReport`·`commitReport`·`forceDiscardDrafts` 세 경로에서 진짜 CHECK 위반이 명명된 409가 되는지, 우리 것이 아닌 실제 CHECK는 그대로 남는지, 확정이 초안 행 잠금을 기다리는지 — 예방 검사를 그 인스턴스에서만 무력화하는 것은 **시험 계측**이며 제품 우회가 아니다), 그리고 `finding_api_test.py` 13(계약 ⑭·⑮ 서버 절반·⑰·⑱·⑲·⑳·㉒를 실스택에서). 드라이버가 CHECK 위반을 어떤 예외 모양으로 올리는지는 **그 실행에서만** 확인된다 — 서비스는 제약 이름으로만 좁게 가른다. 화면(S3-U2b)은 이 묶음에 없다: 벡터의 `assembly` 절(R5 문면)과 존재 상태 표시·클라이언트 상태는 U2b가 소비하며 지금은 어떤 제품 코드도 읽지 않는다. 계약 ⑩·⑫·⑬의 화면 절반·⑯은 U2b, ㉑은 U3에서 닫혔다.
