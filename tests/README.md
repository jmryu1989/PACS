...
REQ-S3-U2b-CITATION-UI → RISK-S3-FALSE-PRESENCE/SILENT-ATTESTATION-LOSS/INVENTED-KEEP-LIST/LOST-TYPED-WORK/CROSS-STUDY-WRITE → TEST-S3-U2b-CITATION-UI: 화면 쪽 인용은 새 순수 모듈 `worklist-v0/hpacs-lite/report-citation.js` 하나와 `main.html`의 미리보기 대화·인용 줄·uid별 상태로 이뤄지고, 소견 패널(`reading-findings.js`)은 출처마다 `Insert into Report`를 붙여 조립된 블록을 건네기만 한다(판독문을 직접 쓰지 않는다). 순서는 **서버 먼저**다 — 확인은 세 칸 + 삽입될 문장이 이미 붙은 값과 `insert{7칸}`을 실은 PUT을 보내고, **200을 받은 뒤에만** 기존 상용구 기전(`el.value += …; input 이벤트`)으로 textarea가 바뀐다. **호스트에서 실제로 실행한 순수 17건**(`node --test tests/report_citation_client_test.cjs`)은 실제 모듈을 실행해 공용 벡터 60건 전부(occurrence 24·blank 8·sameText 7·state 7·equivalence 6·assembly 8)를 서버 검사기와 **같은 답**으로 통과시키고(계약 ⑬), 축약된 건이 본문 대조를 주장하지 않으며(⑥), R5 서식이 제목·본문·특성 순서·`특성: ` 접두사·줄 끝만 LF 정규화·길이 0 칸 생략·LF 하나로 잇기·공백만/4096바이트 초과 거절을 지키고 미리보기=요청=붙은 블록=증언이 같은 바이트임을(⑩), 확인 전 상태에서는 `citationIds` **키 자체가 없고**(`[]`가 아니다, ⑫·B2) 유지 목록이 `absent`·`ambiguous`·축약 건을 **조용히 빼지 않으며** 머리 건은 아직 머리에 있는 명시적 선택으로만 빠지는 것을 확인한다. 같은 파일이 제품 배선을 고정한다 — B1의 `settleStash` → 삽입 PUT 순서, 모든 비-keepalive 저장 경로의 회피, B3의 `reportNeedsWrite()`가 stash·타이머·beforeunload 세 곳에 걸린 것, 확정의 `citationIds`/`removeCitationIds` 분리, 그리고 **DOM 하네스가 잘라 쓰는 제품 구간이 실제로 컴파일된다는 것**(U3에서 단언 이전에 전부 죽은 그 결함의 재발 방지). **호스티드 격리 Chromium 17건**(`python tests/report_citation_dom_test.py`, validate의 workspace-ui job)은 실제 `api()`·`loadReport()`·`stashReport()`·`commitReport()`·실제 모달·실제 `beforeunload` 블록으로 계약 ⑮(거절 뒤 textarea와 초안 행이 바이트 동일)과 ⑯(늦은 200이 검사 이동·A→B→A에서 화면에 쓰이지 않고 수렴 표시가 남는다), B1(나가 있는 저장이 끝난 뒤에 삽입이 떠나고 그 동안 모든 비-keepalive 저장이 비켜선다), B2(확인 전 200은 키를 만들지 않고 전용 읽기 한 번이 그것을 바꾼다), B3(바쁜 동안 Escape·바깥 클릭이 닫지 않고, stash 뒤 dirty가 clean이어도 명시 표시가 다음 저장을 일으킨다), R5(정확한 바이트·명시적 대상·대상 변경 시 새 확인)와 머리 제거가 확정에서만 나간다는 것을 실행한다. **이 17건은 아직 실행된 적이 없다 — 첫 실행은 다음 호스티드 CI다.** 자동 저장 **타이머**(20초 간격)만은 이 파일이 실행하지 않고 소스로 고정하며, 그 타이머가 부르는 `reportNeedsWrite()`는 브라우저에서 실제로 실행된다. **정직하게 남기는 잔여**: ① 삽입이 날아가 있는 동안 탭이 닫히면 keepalive 저장은 응답을 기다릴 수 없어 본문 T0가 나갈 수 있다 — 확인된 상태면 유지 목록이 그 인용도 함께 지우고, 확인 전이면 인용이 남아 `absent`로 보인다(어느 쪽도 글을 잃지 않는다). ② 200을 잃고 재시도해 생긴 중복은 전용 읽기 뒤의 유지 목록에서만 사라지며 그때까지 `ambiguous`로 정직하게 보인다. ③ 머리 건의 제거 의사는 다시 불러오면 남지 않고 이월로 되돌아간다. ④ 확인했으나 화면에서 보지 못한 문장이 기록에 남을 수 있다(현행 상용구 삽입보다 넓은 노출). 소견 패널 쪽 절반은 기존 `finding_command_test.cjs`(빌드된 이미지 없이 도는 순수 실행)에 **2건**을 더해 고정한다 — 출처마다 생긴 `Insert into Report`가 조립된 R5 블록과 `(소견, 판, 출처 번호, 인용 당시 연결 상태·머리 판)`을 워크리스트에 넘기고 **자기는 아무것도 쓰지 않으며**, 숨김·소실 출처, 숨긴 소견, 그 사이 사라진 행, `cite` 입구가 없는 옛 워크리스트는 각각 명시된 문자열로 거절하고 `revised`는 확인 후 넘어간다. 실스택에서 소견 패널 → 서버 → 판독문까지를 한 번에 지나가는 확인은 U2a의 `finding_api_test.py` 13이 서버 절반을 맡고 있으며, 화면까지 포함한 실스택 통합은 남은 호스티드 잔여다.
