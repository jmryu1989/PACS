...
- 미실행·한계: 이 host에는 node·tsc가 없어 Nest build·cjs 시험은 hosted `kin-api:ci`에서만 돈다. live 14건, `production_image_test.py`(컨테이너), 복원 producer/consumer, hosted CI는 실행하지 않았다. 복원 receipt는 합성 행이 2,920 bytes 늘었고(정확) 전체는 모델링한 catalog로 약 185 KB/256 KiB로 추정했다(hosted 실측 전). clinician-only 회원의 검사 단위 StudyAccess 철회는 `study-access.service.ts` `target()`이 아직 clinician을 관리하지 않아 시험하지 않았다(D33 OQ-8 b, 후속 카드). U1c `clinician_policy_test.py` test_14와 `clinician_policy_live.py` test_05는 allow 행을 읽기로만 다뤄(positive 200/204, 요청 본문 없음, clinician 감사 행 0) 6행을 담지 못했다. 후속(지휘 결정 D36)에서 U1c 최종본 e378f04를 병합하고 쓰기 allow 행 계약으로 넓혀 6행의 `live_matrix` 사례를 넣었다(위 S5-U1c 절). live Q08은 여섯 행의 역할 표 전체를 따로 확인한다. 이 단위 경로 밖의 핀도 새 migration·`removeState` 변경으로 어긋난다: `gateway_retry_source_test.py`·`gateway_receipt_source_test.py`·`order_reconciliation_source_test.py`·`report_structure_migration_test.py`(29 migrations·39 tables·행 합·MIGRATIONS 끝), `study_identity_source_test.py`(`removeState` sha256).
