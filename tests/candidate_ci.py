...
BASE = (
    ("invariants_live.py", None, "candidate-invariants", 81),
...
def exact_selection(target, runner):
    ...
    require(len(selected) == 103 and len({(x["file"], x["case"]) for x in selected}) == 103,
            "Candidate selection must contain 103 unique exact cases")
