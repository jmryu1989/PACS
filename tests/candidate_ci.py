...
BASE = (
    ("invariants_live.py", None, "candidate-invariants", 80),
...
def exact_selection(target, runner):
    ...
    require(len(selected) == 102 and len({(x["file"], x["case"]) for x in selected}) == 102,
            "Candidate selection must contain 102 unique exact cases")
