...
BASE = (
    ("invariants_live.py", None, "candidate-invariants", 82),
...
def exact_selection(target, runner):
    ...
    require(len(selected) == 104 and len({(x["file"], x["case"]) for x in selected}) == 104,
            "Candidate selection must contain 104 unique exact cases")
