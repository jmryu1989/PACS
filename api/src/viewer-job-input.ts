...
export function jobCommand(raw: Buffer, create: boolean): any {
  ...
  if (s.version !== 1 || !Array.isArray(s.studies) || ![1, 2].includes(s.studies.length) || new Set(s.studies).size !== s.studies.length) invalid();
  ...
  for (const c of s.cells) {
    if (c === null) continue;
    keys(c, ['study', 'series', 'sop', 'frame', 'camera', 'properties']);
    ...
    keys(c.properties, ['voiRange', 'VOILUTFunction', 'invert']); keys(c.properties.voiRange, ['lower', 'upper']);
